...
#define AVSYNC_IO_GET_VIDEO_SMOOTHLY_PLAY_CFG   0x02  //!< Configure video smooth playback
...
#define AVSYNC_IO_SET_FASTMODE					0x0C  //!< Set avsync in fast switch mode
#define AVSYNC_IO_FLUSH 						0x0D  //!< Set flush event flag
#define AVSYNC_IO_GET_CURRENT_STC_ID			0x0E  //!< Get current STC id
#define AVSYNC_IO_GET_FASTMODE					0x0F  //!< Get avsync fast switch mode
#define AVSYNC_IO_GET_CURRENT_STC_RUNBACK		0x10  //!< Get current STC run back
#define AVSYNC_IO_GET_AUDIO_START_PTS                     0x11
#define AVSYNC_IO_GET_AUDIO_FLUSH_EVENT                0x12
...
typedef enum
{
	...
	AVSYNC_PTS_MODE_CNT,        //!< Not in use
...
/*!
@enum AVSYNC_MODE_E
@brief Audio/video synchronization mode.The default configuration is AVSYNC_MODE_PCR in AVSYNC module,
       AVSYNC_MODE_VIDEO not supported temporarily.
*/
typedef enum
{
	AVSYNC_MODE_INVALID=0,
...
typedef struct
{
	UINT32 vhold_thres;          //!< Video repeat threadhold, with the default value of 80ms.
	UINT32 vdrop_thres;          //!< Video drop threadhold, with the default value of 80ms.
	UINT32 ahold_thres;          //!< Audio repeat threadhold, with the default value of 40ms.
	UINT32 adrop_thres;          //!< Audio drop threadhold, with the default value of 64ms.
	AVSYNC_MODE_E sync_mode;     //!< Audio/video synchronization mode, with the default value of AVSYNC_MODE_PCR.
	AVSYNC_SRCTYPE_E src_type;   //!< Audio/video data source type, with the default value of AVSYNC_SRC_TURNER.
}avsync_cfg_param_t;
...
typedef struct
{
	UINT32 afreerun_thres;               //!< Audio free run threadhold, with the default value of 10s.
	UINT32 vfreerun_thres;               //!< Video free run threadhold, with the default value of 10s.
	UINT8 disable_monitor;               //!< Disable monitor function, with the default value of 1.
	UINT8 disable_first_video_freerun;   //!< Disable free run function of the video first frame, with the default value of 0.
...
struct avsync_video_cb_t
{
	...
	void *sync_info_param;				//!< avsync info param for VIDEO.
};
...
struct avsync_audio_cb_t
{
	...
	void *sync_info_param;				//!< avsync info param for AUDIO.
};
...
/*!
@struct avsync_info_t
@brief  Callback for dmx/snd/decv to get avsync information.
*/
struct  avsync_info_t
{
	UINT8 	sync_mode_unify;	//!< avsync module sync mode.
	UINT8 	source_type;		//!< avsync module source type.
	BOOL	audio_freerun;		//!< audio freerun flag.
	BOOL	video_freerun;    	//!< video freerun flag.
};
...
struct avsync_device
{
	...
	RET_CODE (*get_status)(struct avsync_device*, avsync_status_t *);
	RET_CODE (*get_statistics)(struct avsync_device*, avsync_statistics_t *);
...
struct avsync_dmx_cb
{
	...
	void *sync_info_param;					  //!< avsync info param for DMX.
};

typedef RET_CODE (*AVSYNC_GET_VSYNC_FLAG)(struct avsync_device *dev, struct avsync_vsync_info_t *, AVSYNC_FRAME_SYNCFLAG_E *, BOOL *);
typedef RET_CODE (*AVSYNC_VBV_BUFF_FULL)(struct avsync_device *dev, UINT8);
typedef RET_CODE (*AVSYNC_START_CB)(struct avsync_device *dev);
typedef RET_CODE (*AVSYNC_STOP_CB)(struct avsync_device *dev);
typedef RET_CODE (*AVSYNC_GET_ASYNC_FLAG)(struct avsync_device *dev, struct avsync_async_info_t *, AVSYNC_FRAME_SYNCFLAG_E *);
typedef RET_CODE(*AVSYNC_GET_PAUSE_FLAG)(struct avsync_device *dev, UINT32 *, UINT32 *);
...
@param[in] dev Pointer to AVSYNC device.
@return RET_CODE
@retval  RET_SUCCESS        Operated successfully.
@retval  !RET_SUCCESS       Failed to operate due to parameter error or status error.
*/
...
@retval  RET_SUCCESS        Operated successfully.
@retval  !RET_SUCCESS       Failed to operate due to parameter error or status error.
@note: If this interface is not called by application level, default configuration will be used in AVSYNC module
       internal. Please refer to #AVSYNC_MODE_E definition.
*/
...
@retval  !RET_SUCCESS       Failed to operate due to parameter error or status error.
@note: If this interface is not called by application level, default configuration will be used in AVSYNC module
       internal. Please refer to #AVSYNC_SRCTYPE_E definition.
*/
...
@retval  !RET_SUCCESS       Failed to operate due to parameter error or status error.
@note:  If this interface is not called by application level, default configuration will be used in AVSYNC module
        internal. Please refer to #avsync_cfg_param_t definition.
*/
...
@retval  !RET_SUCCESS       Failed to operate due to parameter error or status error.
@note: If this interface is not called by application level, default configuration will be used in AVSYNC module
       internal. Please refer to #avsync_adv_param_t definition.
*/
...
#define AVSYNC_DBG_PRINT_CURRENT_TIME 		0x100000  //!< Print current time
...
@return RET_CODE
@retval  RET_SUCCESS       Operated successfully.
@retval  !RET_SUCCESS      Failed to operate.
*/
...
/*!
@brief  Get Callback function of AVSYNC module
@param[in/out] struct avsync_callback  Pointer.
@return RET_CODE
@retval  RET_SUCCESS        Operated successfully.
@retval  !RET_SUCCESS       Failed to operate due to parameter error.
*/
RET_CODE avsync_get_callback(struct avsync_callback *pcb);
