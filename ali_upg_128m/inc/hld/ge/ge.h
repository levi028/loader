...
#define GE_IO_SET_ENHANCE_PAR                   (GE_IO_CTRL_BASE_GMA + 0x0F)    /*modified for c3811*/

#define GMA_ENHANCE_BRIGHTNESS        0x01  // value[0, 100], default 50
#define GMA_ENHANCE_CONTRAST          0x02  // value[0, 100], default 50
#define GMA_ENHANCE_SATURATION        0x03  // value[0, 100], default 50
#define GMA_ENHANCE_SHARPNESS         0x04  // value[0, 10], default 5
#define GMA_ENHANCE_HUE               0x05  // value[0, 100], default 50

struct gma_enhance_pars
{
    UINT8 enhance_flag;
...
{
    BOOL   b_is_lq;           //TRUE:LQ,FALSE:HQ
    UINT32 *p_buffer;        //the cmd buffer,aligned withe DW
    UINT32 n_buffer_len;     //the buffer size in DW,at lease 30K
}ge_hw_cmd_buffer;
...
enum GE_PRIMITIVE_MODE
{
    ...
    GE_BOOLEAN_OPERATION_WITHOUT_ALPHA,
...
{
    GE_DISABLE_CKEY,
    GE_ENABLE_EQUAL_TO_CKEY,
    GE_ENABLE_NOT_EQUAL_CKEY,
};

enum GE_BLEND_MODE
{
    GE_BYPASS_SOURCE_DIRECTLY,
    GE_BYPASS_PATTERN_DIRECTLY,
    GE_BLENDING,
};
...
//PTN_MODULATE_MO:pattern alpha value
enum GE_PTN_MODULATE_MODE
{
    GE_NO_MODULATE,//NO_MODULATE_ON,
    GE_GLOBAL_ALPHA,//GLOBAL_COLOR_ALPHA,
    GE_PTNALP_MOD_GLBALP,//PTNALPHA_MOD_GLOBALALPHA,
    GE_PTNALP_MOD_GLBCLR,//PTNALPHA_MOD_GLOBALCOLOR,
    GE_GLBALP_PTNCLR_MOD_GLBCLR,//GLOBALALPHA_PTNCOLOR_MOD_GLOBALCOLOR,
    GE_PTNALP_MOD_GLBALP_PTNCLR_MOD_GLBCLR,//PTNALPHA_MOD_GLOBALALPHA_PTNCOLOR_MOD_GLOBALCOLOR,
};
...
enum GE_RGB_ORDER
{
    ...
    ge_rgb_order_aycb_cr = GE_RGB_ORDER_ARGB,
    ge_rgb_order_acr_cb_y = GE_RGB_ORDER_ABGR,
    ge_rgb_order_ycb_cr_a = GE_RGB_ORDER_RGBA,
    ge_rgb_order_cr_cb_ya = GE_RGB_ORDER_BGRA,
};
...
typedef struct _ge_layer_config_t
{
    ...
    UINT32 b_scale_filter_enable       :1;  // enable/disable GMA scale filter
    UINT32 b_p2nscale_in_normal_play    :1;  // enable/disable PAL/NTSC scale in normal play mode
    UINT32 b_p2nscale_in_subtitle_play  :1;     // enable/disable PAL/NTSC scale in subtitle play mode
    UINT32 b_direct_draw              :1;  // for CPU direct draw, no GE draw
    UINT32 b_cacheable               :1;  // for CPU direct draw, no GE draw
	UINT32 b_newfeature              :1;  // for GE new feature, if use , set it in ge_dev_attach(void)
...
/* add end*/
    ...
    RET_CODE (*set_global_color)(struct ge_device * , UINT32 , UINT32 );
    RET_CODE (*set_ptn_modulate_mode)(struct ge_device * , UINT32 , enum GE_PTN_MODULATE_MODE );
    ...
    RET_CODE (*set_ptn_ckey_mode)(struct ge_device * , UINT32 , enum GE_CKEY_MODE );
    RET_CODE (*set_src_ckey_mode)(struct ge_device * , UINT32 , enum GE_CKEY_MODE );
    RET_CODE (*set_src_colorkey)(struct ge_device * , UINT32 , UINT32 );
    RET_CODE (*set_ptn_colorkey)(struct ge_device * , UINT32 , UINT32 );
    RET_CODE (*set_ptn_ckey_channel)(struct ge_device * , UINT32 , BOOL, BOOL, BOOL, BOOL );
    RET_CODE (*set_src_ckey_channel)(struct ge_device * , UINT32 , BOOL, BOOL, BOOL, BOOL );
    RET_CODE (*set_blend_mode)(struct ge_device * , UINT32 , enum GE_BLEND_MODE );
    RET_CODE (*set_matrix)(struct ge_device * , UINT32 , const float * );
    RET_CODE (*set_scale_param)(struct ge_device * , UINT32 , INT32, INT32, INT32, INT32 );
    RET_CODE (*set_onoff_matrix)(struct ge_device * , UINT32 , BOOL );
    RET_CODE (*set_transform_center)(struct ge_device * , UINT32 , INT32, INT32 );
    void   *gma0_priv;
    void   *gma1_priv;
};
