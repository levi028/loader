...
RET_CODE ge_io_ctrl(struct ge_device *dev,UINT32 dw_cmd,UINT32 dw_param);

RET_CODE ge_region_show_onoff(struct ge_device *dev, ge_surface_desc_t *surface, UINT8 u_region_id,BOOL b_on);
...
RET_CODE ge_region_show_onoff_deo(struct ge_device *dev, ge_surface_desc_t *surface, UINT8 u_region_id,BOOL b_on);
...
RET_CODE ge_io_ctrl_deo(struct ge_device *dev,UINT32 dw_cmd,UINT32 dw_param);
...
*/
INT32 ge_get_blt_status(UINT32 dw_flags);

/*
    Retrieves the color key value
*/
INT32 ge_get_colorkey(ge_surface_desc_t lp_surface, UINT32 dw_flags, ge_color_key_t lp_colorkey);

/*
    Sets the color key value
*/
INT32 ge_set_colorkey(ge_surface_desc_t lp_surface, UINT32 dw_flags, ge_color_key_t lp_colorkey);
...
*/
INT32 ge_flip(ge_surface_desc_t lp_surface, ge_surface_desc_t lp_gl_surface_target_overrid, UINT32 flags);

/*
    The GetFlipStatus method indicates whether the surface
    has finished its flipping process
*/
INT32 ge_get_flip_status(UINT32 dw_flags);
...
/*
    ...
    one from, a surface
*/
INT32 ge_set_clipper(ge_surface_desc_t lp_surface, ge_clip_info_t lp_clipper);

/*
    The GetPalette method retrieves the Palette structure associated with
    this surface.
*/
INT32 ge_get_palette(ge_surface_desc_t lp_surface, ge_palette_info_t lp_palette);
...
*/
INT32 ge_set_palette(ge_surface_desc_t lp_surface, ge_palette_info_t lp_palette);

/*
    CreateSurface method creates a GRAPHICS LIBRARY SURFACE structure
*/
INT32 ge_create_surface(ge_palette_info_t lp_surface_desc,ge_surface_desc_t **lplp_surface);

/*
    ReleaseSurface method release a Surface structure
*/
void ge_release_surface(ge_surface_desc_t lp_surface);

/*
    The GetSurfaceDesc method retrieves a description of the surface in
    its current condition.
*/
INT32 get_surface_desc(ge_surface_desc_t lp_surface, ge_palette_info_t lp_surface_desc);
...
{
    UINT32    dw_caps;        // capabilities of surface wanted
} GL_SCAPS;


typedef struct _GL_SURFACEDESC
{
    UINT32        dw_size;            // size of the GL_SURFACEDESC structure
    UINT32        dw_flags;        // determines what fields are valid
//    UINT32        TV_X;            // start_x position on TV
//    UINT32        TV_Y;            // start_y position on TV
    UINT32        dw_height;        // height of surface to be created
    UINT32        dw_width;        // width of input surface
    union
    {
        INT32        l_pitch;            // distance to start of next line (return value only)
        UINT32       dw_linear_size;           // Formless late-allocated optimized surface size
    };
    UINT32        dw_back_buffer_count;    // number of back buffers requested
    UINT32        dw_alpha_bit_depth;    // depth of alpha buffer requested
    UINT32        dw_reserved;        // reserved
    UINT8 *        lp_surface;        // pointer to the associated surface memory
    UINT8 *        lp_y_top;
    UINT8 *      lp_y_bottom;
    UINT8 *      lp_c_top;
    UINT8 *      lp_c_bottom;
    //    GL_COLORKEY        ddckCKDestOverlay;    // color key for destination overlay use
    GL_COLORKEY        ddck_ckdest_blt;        // color key for destination blt use
    //    GL_COLORKEY        ddckCKSrcOverlay;    // color key for source overlay use
    GL_COLORKEY        ddck_cksrc_blt;        // color key for source blt use
    GL_PIXELFORMAT    ddpf_pixel_format;    // pixel format description of the surface
    GL_SCAPS        dds_caps;        // graphics library surface capabilities
