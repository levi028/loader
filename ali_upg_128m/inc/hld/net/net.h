...
struct packseg
{
    struct packseg *next;           /*next packseg*/
    void           *payload;        /*payload data*/
    UINT16          tot_len;        /*total length of this packet*/    
    UINT16          len;            /*length of this packseg */
};

typedef void (*packseg_free)(struct packseg *);
...
struct net_device
{
	...
	RET_CODE (*attach) (MAC_CONFIG *);
	RET_CODE (*detach) (struct net_device *);
	
	RET_CODE (*open) (struct net_device *, void (*callback) (struct net_device *, UINT32, UINT32));
	...
	RET_CODE (*send_packet_ext) (struct net_device *, struct packseg *, packseg_free);
	void (*callback) (struct net_device *, UINT32, UINT32);
	...
	int (*mt7601_callback)(UINT32); /* Barry add to integrate */
	int customer_pid[10];
	int customer_vid[10];
	int wifi_device_enable;

#if 1//Barry add to support p2p
    int priv_flags;
    int (*set_p2p_enable_status)(struct net_device *, int);
    int (*get_p2p_enable_status)(struct net_device *);
    unsigned char dev_addr[6], addr_len;
    void (*p2p_send_event_callback)(UINT32, UINT32, UINT32, UINT32);//netlink
#endif
	int (*rtl8188_callback)(UINT32); /* Barry add to support 8188 */
#endif
};
