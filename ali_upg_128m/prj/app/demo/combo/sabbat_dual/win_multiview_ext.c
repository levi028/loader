...
*******************************************************************************/
MATRIX_BOX_NODE mb_node_mv_name_ext[MULTI_VIEW_COUNT];

CONTAINER    g_cnt_multiview_ext;
MATRIX_BOX    g_mtb_mv_name_ext;
BITMAP        g_bmp_mv_load_ext;
BITMAP        g_bmp_mv_signal_ext;
TEXT_FIELD g_bmp_mv_signal_txt_ext;
TEXT_FIELD g_mv_pip_txt_ext;

char mv_len_display_str[100];

static BOOL g_key_exit = FALSE;
static BOOL g_key_ok = FALSE;
__MAYBE_UNUSED__ static BOOL g_post_chg = FALSE;
static BOOL g_age_lock = FALSE;
UINT16*    str_mv_prog_name_ext;
static UINT32 lock_play = 0;
static BOOL pwd_open_state = FALSE;

extern void win_multiview_set_lock(BOOL flag);
static VACTION win_mv_mtb_ext_keymap(POBJECT_HEAD obj, UINT32 key);
static PRESULT win_mv_ctn_ext_callback(POBJECT_HEAD p_obj, VEVENT event, UINT32 param1, UINT32 param2);
static PRESULT win_mv_mtb_ext_callback(POBJECT_HEAD p_obj, VEVENT event, UINT32 param1, UINT32 param2);
static VACTION win_com_con_ext_keymap(POBJECT_HEAD obj, UINT32 key);
...
CONTAINER    g_cnt_multiview_ext = {
                                    {    OT_CONTAINER, //object type
                                        ...
                                        win_com_con_ext_keymap,//PFN_KEY_MAP pfnKeyMap;
                                        (PFN_CALLBACK)win_mv_ctn_ext_callback,//PFN_CALLBACK pfnCallback;

                                        NULL,//POBJECT_HEAD pNext;
                                        NULL//POBJECT_HEAD pRoot;
                                    },
                                    (POBJECT_HEAD)&g_mtb_mv_name_ext,
                                    1,
                                    0
};
...
MATRIX_BOX    g_mtb_mv_name_ext = {
                                    {    OT_MATRIXBOX, //object type
                                        ...
                                        win_mv_mtb_ext_keymap,//PFN_KEY_MAP pfnKeyMap;
                                        win_mv_mtb_ext_callback,//PFN_CALLBACK pfnCallback;

                                        (POBJECT_HEAD)&g_bmp_mv_signal_txt_ext,//POBJECT_HEAD pNext;
                                        (POBJECT_HEAD)&g_cnt_multiview_ext//POBJECT_HEAD pRoot;
                                        },
                                        MATRIX_ROW_WRAP,//UINT8 bStyle;
                                        MATRIX_TYPE_WCHAR,//UINT8 bCellType;
                                        MULTI_VIEW_COUNT,//INT16 nCount;
                                        mb_node_mv_name_ext,//    PMATRIX_BOX_NODE pCellTable;

                                        4,//MULTI_VIEW_X_COUNT,//INT16 nRow;
                                        1,//(MULTI_VIEW_COUNT/MULTI_VIEW_X_COUNT),//INT16 nCol;
...
#define LDEF_TXT(root,var_txt,nxt_obj,l,t,w,h,res_id,shidx,str)    \
    DEF_TEXTFIELD(var_txt,root,nxt_obj,C_ATTR_ACTIVE,0, \
        0,0,0,0,0, l,t,w,h, shidx, shidx, shidx,shidx,   \
        NULL, NULL,  \
        C_ALIGN_CENTER | C_ALIGN_VCENTER, 0,0,res_id,str)

LDEF_TXT(&g_cnt_multiview,g_bmp_mv_signal_txt_ext,(POBJECT_HEAD)&g_mv_pip_txt_ext, TXT_L, TXT_T,TXT_W, TXT_H, 0, WSTL_TEXT_10_HD,0)
LDEF_TXT(&g_cnt_multiview,g_mv_pip_txt_ext,NULL, PIP_TXT_L, PIP_TXT_T,PIP_TXT_W, PIP_TXT_H, 0, WSTL_MIXBACK_IDX_04_8BIT,(UINT16*)mv_len_display_str)
...
BOOL win_multiview_ext_get_age_lock(void)
{
    return g_age_lock;
}
...
static void win_mv_load_page(UINT16 first_idx, UINT32 page_cnt)
{
    ...
    mv_para.u_box_active_idx = osd_get_mtrxbox_cur_pos(&g_mtb_mv_name_ext) + 1;
    mv_para.mvmode = MV_4_1_BOX;
    ...
    for(i = 0;  i<page_cnt; i++)
    {
        ...
        com_asc_str2uni(tmp_idx, (UINT16*)mb_node_mv_name_ext[i].str);
        ...
        com_uni_str_cat((UINT16*)mb_node_mv_name_ext[i].str, (const UINT16*)tmp_name);
    }
    ...
    t_mv_init_para.t_mpip_size.u_width=MV_VIDEO_WIDTH*9/16;
    t_mv_init_para.t_mpip_size.u_height=MV_VIDEO_HEIGHT*pipmode/720;
    //}
    //else
    //{
    ...
    t_mv_init_para.play_win.u_start_x = (CNT_MV_L+TXT_MV_X_SHIFT+6)*9/16;
    t_mv_init_para.play_win.u_start_y = (CNT_MV_T+4)*pipmode/720;
    t_mv_init_para.play_win.u_width = (CNT_MV_W-TXT_MV_X_SHIFT)*9/16;
    t_mv_init_para.play_win.u_height = (CNT_MV_H-PIP_TXT_H+4)*pipmode/720;

#ifdef BIDIRECTIONAL_OSD_STYLE
    if (osd_get_mirror_flag() == TRUE)
        t_mv_init_para.play_win.u_start_x = 1280*9/16 - t_mv_init_para.play_win.u_start_x - t_mv_init_para.play_win.u_width;
#endif

    uimultiview_init_ext(&t_mv_init_para,MV_4_1_BOX,TRUE);
...
//    OSD_Scale(OSD_VSCALE_DVIEW, (UINT32)&eOSDSys);
    ...
    osd_draw_fill(CNT_MV_L, CNT_MV_T, CNT_MV_W+6, 4, 0xffff, NULL);
    osd_draw_fill(CNT_MV_L, CNT_MV_T+4, 6,CNT_MV_H+4, 0xffff, NULL);
    osd_draw_fill(CNT_MV_L, CNT_MV_T+CNT_MV_H+4, CNT_MV_W, 4, 0xffff, NULL);
    osd_draw_fill(CNT_MV_L+CNT_MV_W, CNT_MV_T+4, 6,CNT_MV_H+4, 0xffff, NULL);

    osd_draw_fill(CNT_MV_L+TXT_MV_X_SHIFT, CNT_MV_T+4, 6,CNT_MV_H+4, 0xffff, NULL);

    for(i = 1 ; i < MULTI_VIEW_COUNT; i++)
    {
        osd_draw_fill(CNT_MV_L, CNT_MV_T+TXT_MV_Y_SHIFT * i, TXT_MV_X_SHIFT+6, 4, 0xffff, NULL);
    }
}
...
static BOOL win_mv_page_shift(UINT16 start, UINT16 count, INT16 shift)
{
    ...
    else
    {
        ...
        {//when channels after first one is no more than 9
            ...
            {
                enter_mv_mode = FALSE;
                osd_set_mtrxbox_cur_pos(&g_mtb_mv_name_ext, (first_channel + g_mv_page_cnt - count));
            }
            first_channel = count - g_mv_page_cnt;
        }
    }
...
static void win_mv_init()
{
    ...
    {
        mb_node_mv_name_ext[i].str = (UINT32)display_strs[i];//(UINT32)api_alloc_static_str(API_STATIC_STR_LEN);
        mb_node_mv_name_ext[i].num = 0;
        mb_node_mv_name_ext[i].b_attr = C_ATTR_ACTIVE;
    }
    ...
    {
        g_mv_top = 0;
        osd_set_mtrxbox_count(&g_mtb_mv_name_ext, g_mv_total, MULTI_VIEW_COUNT, 1);
        osd_set_mtrxbox_cur_pos(&g_mtb_mv_name_ext, cur_channel);
    ...
    else
    {
        ...
        osd_set_mtrxbox_count(&g_mtb_mv_name_ext, g_mv_page_cnt, MULTI_VIEW_COUNT,1);
        osd_set_mtrxbox_cur_pos(&g_mtb_mv_name_ext, 0);
    }
}
#if 0
static void win_mv_update_prog_info(UINT32 cur_channel)
{
    eit_event_info_t *pe=NULL,*fe=NULL;
    UINT8 *s1=NULL,*s2=NULL;
    BOOL mv_event_update = FALSE;
    UINT8 mv_temp_len,mv_len;
    char mv_string[100];

    sprintf(mv_string," ");
    com_asc_str2uni(mv_string, (UINT16 *)mv_len_display_str);
    osd_draw_object( (OBJECT_HEAD *)&g_mv_pip_txt_ext, C_UPDATE_ALL);

    /*current next epg info */
    pe=epg_get_cur_service_event((INT32)cur_channel, PRESENT_EVENT, NULL,NULL,NULL, /*event_update*/TRUE);
    fe=epg_get_cur_service_event((INT32)cur_channel, FOLLOWING_EVENT, NULL,NULL,NULL, mv_event_update);
    if(mv_event_update==FALSE)
    {
        if(pe==NULL || fe==NULL)
        {
            mv_event_update = TRUE;
        }
    }
    else
    {
        if(pe!=NULL && fe!=NULL)
        {
            mv_event_update = FALSE;
        }
    }
    s1=epg_get_event_name(pe, &mv_len);
    s2=epg_get_event_name(fe, &mv_len);
    mv_len_display_str[0]= '\0';
    if (s1)
    {
                sprintf(mv_string,"Now: ");
                com_asc_str2uni(mv_string, (UINT16 *)mv_len_display_str);
                mv_len =com_uni_str_len(mv_len_display_str);
                com_uni_str_copy_char((UINT8*)&mv_len_display_str[mv_len],s1);
                UINT8 LEN;
                LEN = com_uni_str_len(mv_len_display_str);
                mv_temp_len = LEN;
                if(!osd_get_vscr_state())
                {
                  if(LEN < 15)
                  {
                     ...
                     for(i=0;i<=15-LEN;i++)
                     {
                        ...
                        sprintf(temp_c," ");
                        com_asc_str2uni(temp_c,(UINT16 *)temp_t1);
                        com_uni_str_cat(mv_len_display_str, (UINT16 *)temp_t1);
                     }
                  }
                }
    }
    else
    {
                if(osd_get_vscr_state())
                {
                    osd_set_text_field_str_point(&g_mv_pip_txt_ext,NULL);
                    osd_set_text_field_content(&g_mv_pip_txt_ext,STRING_ID,RS_EPG_NO_INFORMATION);
                }
                else
                {
                    if (mv_temp_len > 15)
                    {
                        sprintf(mv_string,"No Information");
                        com_asc_str2uni(mv_string, (UINT16 *)mv_len_display_str);
                          UINT8 i;
                        for(i=0;i<=mv_temp_len + 5 -15;i++)
                        {
                            char temp_c[2],temp_t1[2];
                            sprintf(temp_c," ");
                            com_asc_str2uni(temp_c,(UINT16 *)temp_t1);
    ...
    if (s2)
    {
                sprintf(mv_string,"Next: ");
                com_asc_str2uni(mv_string, (UINT16 *)mv_len_display_str);
                mv_len =com_uni_str_len(mv_len_display_str);
                com_uni_str_copy_char((UINT8*)&mv_len_display_str[mv_len],s2);
    }
    else
    {
                sprintf(mv_string," ");
...
static void win_mv_update_signal(BOOL onoff,UINT16 idx)
{
    ...
    if((b_lock)&& onoff)
    {
        osd_set_bitmap_content(&g_bmp_mv_signal_ext, IM_GROUP_LOCK);
        osd_set_text_field_content(&g_bmp_mv_signal_txt_ext, STRING_ID,RS_SYSTME_PARENTAL_LOCK);
    }
    else if((SIGNAL_STATUS_UNLOCK == lock_flag)&& onoff)
    {
        osd_set_bitmap_content(&g_bmp_mv_signal_ext, IM_SIGNAL_LEVEL_01);
        osd_set_text_field_content(&g_bmp_mv_signal_txt_ext, STRING_ID, RS_MSG_NO_SIGNAL);
    }
    else if((((p_node.ca_mode == 1)&&(MV_SCAN_SCREEN & uimultiview_get_status()))||(SIGNAL_STATUS_SCRAMBLED == scramble_flag))&& onoff)
    {
        osd_set_bitmap_content(&g_bmp_mv_signal_ext, IM_GROUP_MONEY);
        osd_set_text_field_content(&g_bmp_mv_signal_txt_ext, STRING_ID, RS_SYSTME_CHANNEL_SCRAMBLED);
    }
    else
    {
        osd_set_bitmap_content(&g_bmp_mv_signal_ext, 0);
        osd_set_text_field_content(&g_bmp_mv_signal_txt_ext, STRING_ID, 0);
    }

    x = BMP_SIG_L + ((idx)%MULTI_VIEW_X_COUNT * TXT_MV_X_SHIFT);
    y = BMP_SIG_T + ((idx)/MULTI_VIEW_X_COUNT* TXT_MV_Y_SHIFT) ;
    osd_move_object((POBJECT_HEAD)&g_bmp_mv_signal_ext, x, y, FALSE);
    osd_move_object((POBJECT_HEAD)&g_bmp_mv_signal_txt_ext, x-(TXT_W-BMP_SIG_W)/2, y+BMP_SIG_H, FALSE);

    osd_draw_object((POBJECT_HEAD)&g_bmp_mv_signal_txt_ext, C_UPDATE_ALL);
    osd_draw_object((POBJECT_HEAD)&g_bmp_mv_signal_ext, C_UPDATE_ALL);
}
...
static void win_mv_load_sign_onoff(BOOL onoff, UINT16 idx)
{
    ...
    osd_move_object((POBJECT_HEAD)&g_bmp_mv_load_ext, x, y, FALSE);

    if(onoff)
        osd_draw_object((POBJECT_HEAD)&g_bmp_mv_load_ext, C_UPDATE_ALL);
    else
    {
        osd_hide_object((POBJECT_HEAD)&g_bmp_mv_load_ext, 0);
    ...
    switch(event)
    {
        case EVN_UNKNOWN_ACTION:
        {
            ...
            if((VACT_CURSOR_PGUP == vact) || (VACT_CURSOR_PGDN == vact))
            {
                ...
                sys_data_set_cur_group_channel(g_mv_top+osd_get_mtrxbox_cur_pos(&g_mtb_mv_name_ext));
            }
...
#endif
        api_set_system_state(SYS_STATE_4_1PIC);
        api_stop_play(0);
...
static PRESULT win_mv_message_proc(UINT32 msg_type, UINT32 msg_code)
{
    ...
    case CTRL_MSG_SUBTYPE_STATUS_SIGNAL:
        ...
        if( !(MV_SCAN_SCREEN & multi_status) && !load_status && check_prog_finised && (chg_time_out >= 1))
        {
            ...
            P_NODE p_node;
            p_sys_data = sys_data_get();
            focus = osd_get_mtrxbox_cur_pos(&g_mtb_mv_name_ext);
            get_prog_at(focus + g_mv_top, &p_node);
            ...
            if(p_sys_data->rating_sel)
            {
                if(win_multiview_ext_get_age_lock())
                {
                        age_check = TRUE;
                        b_lock = TRUE;
                        if(get_box_play_status())
