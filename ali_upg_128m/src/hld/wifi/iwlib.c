...
struct	iw15_range
{
	...
	UINT8		num_bitrates;
	INT32		bitrate[IW15_MAX_BITRATES];
	INT32		min_rts;
	INT32		max_rts;
	INT32		min_frag;
	INT32		max_frag;
	...
	UINT16		pmt_flags;
	UINT16		pm_capa;
	UINT16		encoding_size[IW15_MAX_ENCODING_SIZES];
	UINT8		num_encoding_sizes;
	UINT8		max_encoding_tokens;
	UINT16		txpower_capa;
	UINT8		num_txpower;
	INT32		txpower[IW15_MAX_TXPOWER];
...
{
	char		buff[1024];
	void *	cur_dev, *last_dev;
	int		i;

	cur_dev = last_dev = NULL;
	do{
		cur_dev = dev_get_by_type(last_dev, HLD_DEV_TYPE_WIFI);

		if(last_dev != cur_dev) {
			if(cur_dev != NULL)
				(*fn)(skfd, ((struct net_device *)cur_dev)->name, args, count);
		}else
			break;
			 ...
			 int	count)		/* Args count */
{
	...
	* This enable us to treat the SIOCGIWRANGE failure below properly. */
	...
	if(iw_get_ext(skfd, ifname, SIOCGIWRANGE, &wrq) < 0)
	{
		...
		WT_PRINTF("%-8.16s  Driver has no Wireless Extension version information.\n\n", ifname);
		return(0);
	}
	...
	* we use magic. 300 bytes is a also magic number, don't touch... */
	if(wrq.u.data.length >= 300)
	{
		...
		WT_PRINTF("%-8.16s  Recommend Wireless Extension v%d or later,\n",
			ifname, range->we_version_source);
		WT_PRINTF("          Currently compiled with Wireless Extension v%d.\n\n",
			range->we_version_compiled);
	}
	else
	{
		WT_PRINTF("%-8.16s  Wireless Extension version too old.\n\n",
			ifname);
	}
...
iw_print_version_info(const char *	toolname)
{
	...
	{
		WT_DBGPRINT_ERR(("socket"));
		return -1;
	}

	/* Information about the tools themselves */
	if(toolname != NULL)
		WT_PRINTF("%-8.16s  Wireless-Tools version %d\n", toolname, WT_VERSION);
	
	WT_PRINTF("          Compatible with Wireless Extension v11 to v%d.\n\n",
		WE_MAX_VERSION);
	...
	WT_PRINTF("Kernel    Currently compiled with Wireless Extension v%d.\n\n",
		we_kernel_version);
		  ...
		  iwrange *	range)
{
	...
	* will break... */
	if(!iw_ignore_version)
	{
		...
		{
			WT_DBGPRINT_ERR(("Warning: Driver for device %s has been compiled with an ancient version\n", ifname));
			WT_DBGPRINT_ERR(("of Wireless Extension, while this program support version 11 and later.\n"));
			WT_DBGPRINT_ERR(("Some things may be broken...\n\n"));
		}
		...
		* the unknown */
		...
		{
			WT_DBGPRINT_ERR(("Warning: Driver for device %s has been compiled with version %d\n", ifname, range->we_version_compiled));
			WT_DBGPRINT_ERR(("of Wireless Extension, while this program supports up to version %d.\n", WE_MAX_VERSION));
			WT_DBGPRINT_ERR(("Some things may be broken...\n\n"));
		}
		...
		{
			WT_DBGPRINT_ERR(("Warning: Driver for device %s recommend version %d of Wireless Extension,\n", ifname, range->we_version_source));
			WT_DBGPRINT_ERR(("but has been compiled with version %d, therefore some driver features\n", range->we_version_compiled));
			WT_DBGPRINT_ERR(("may not be available...\n\n"));
		}
		 ...
		 iwprivargs **	ppriv)
{
	...
	int			maxpriv = 64;	/* Minimum for compatibility WE<13 */ /* Barry add to 64, default 16 */
	iwprivargs *		newpriv;
		    ...
		    wireless_config *	info)
{
	...
	*/
	...
	if(info->has_key)
	{
		...
		if((flags & IW_ENCODE_INDEX) > 0)
		{
			...
			{
				WT_DBGPRINT_ERR(("SIOCSIWENCODE(%d): %s\n",
	...
	*/
	if(info->has_essid)
	{
		...
		{
			WT_DBGPRINT_ERR(("SIOCSIWESSID: %s\n", strerror(errno)));
			ret = -1;
		}
	}
...
{
	input = input; key = key;
	WT_DBGPRINT_ERR(("Error: Passphrase not implemented\n"));
	return(-1);
}
	       ...
	       UINT16 *		flags)
{
	...
	if(!strncmp(input, "l:", 2))
	{
		...
		if(p == NULL)
		{
			WT_DBGPRINT_ERR(("Error: Invalid login format\n"));
			return(-1);
		}
		*p = '\0';
	    ...
	    int			macmax)
{
	...
	while(*p != '\0')
	{
		...
		if(*p == '\0')
		{
			...
			WT_DBGPRINT_ERR(("iw_mac_aton(%s): %s\n", orig, buf));
#endif
		...
		{
#ifdef DEBUG
			WT_DBGPRINT_ERR(("iw_mac_aton(%s): trailing junk!\n", orig));
#endif
			errno = E2BIG;
	...
	/* Error... */
#ifdef DEBUG
	WT_DBGPRINT_ERR(("iw_mac_aton(%s): invalid ether address!\n", orig));
#endif
	errno = EINVAL;
	return(0);
}
	   ...
	   struct sockaddr *sap)
{
	...
	if(strchr(bufp, ':') == NULL)
	{
		...
		if(iw_check_if_addr_type(skfd, ifname) < 0)
		{
			WT_DBGPRINT_ERR(("%-8.16s  Interface doesn't support IP addresses\n", ifname));
			return(-1);
		}
		...
		{
			WT_DBGPRINT_ERR(("Invalid interface address %s\n", bufp));
			return(-1);
		}
		...
		{
			WT_DBGPRINT_ERR(("Arp failed for %s on %s... (%d)\nTry to ping the address before setting it.\n",
				bufp, ifname, errno));
			return(-1);
		}
		...
		{
		char buf[20];
		WT_PRINTF("IP Address %s => Hw Address = %s\n",
	...
	else	/* If it's an hardware address */
	{
		...
		{
			WT_DBGPRINT_ERR(("Invalid hardware address %s\n", bufp));
			return(-1);
		}
	}
...
#ifdef DEBUG
	WT_PRINTF("DBG - stream->current = %p, stream->value = %p, stream->end = %p\n",
...
#ifdef DEBUG
	WT_PRINTF("DBG - iwe->cmd = 0x%X, iwe->len = %d\n",
		iwe->cmd, iwe->len);
#endif
...
#ifdef DEBUG
	WT_PRINTF("DBG - event_type = %d, event_len = %d, pointer = %p\n",
		...
		if(extra_len > 0)
		{
			...
			else
			{
				...
				* Fixing that in the kernel would break 64 bits userspace. */
				...
				{
					UINT16		alt_dlen = *((UINT16 *) pointer);
					unsigned int	alt_token_len = alt_dlen * descr->token_size;
					
					if((alt_token_len + 8) == extra_len)
					{
#ifdef DEBUG
						WT_PRINTF("DBG - alt_token_len = %d\n", alt_token_len);
#endif
				...
				* produce buffer overflow to our caller... */
				...
				if(iwe->u.data.length < descr->min_tokens)
					iwe->u.data.pointer = NULL;	/* Discard paylod */
#ifdef DEBUG
				WT_PRINTF("DBG - extra_len = %d, token_len = %d, token = %d, max = %d, min = %d\n",
		...
		wireless_scan_head *	context)
{
	...
	if(wrq.u.data.length)
	{
		...
		int	i;
		WT_PRINTF("Scan result [%02X", buffer[0]);
