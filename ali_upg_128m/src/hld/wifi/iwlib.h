...
//#define DEBUG 1
#define wtdebug_level	8

#define WT_PRINTF		libc_printf

#define WT_DBGPRINT(level, fmt)    \
do{                                   \
    if (level <= wtdebug_level)      \
    {                               \
        WT_PRINTF fmt;               \
    }                               \
}while(0)
...
//#ifndef IW_EV_LCP_PK_LEN   //LYH mark
#if 0   //LYH move to wireless.h
	   ...
	   struct iwreq *	pwrq)		/* Fixed part of the request */
{
	...
	dev = (struct net_device *)dev_get_by_name((INT8 *)ifname);
	if(dev == NULL)
		return -1;

    if(dev->ioctl)
	    ret = dev->ioctl(dev, (UINT32)request, (UINT32)pwrq);
...
{
  return MEMCMP((void *)eth1, eth2, sizeof(*eth1));
}
