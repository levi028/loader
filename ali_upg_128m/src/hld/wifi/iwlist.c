		   ...
		   const char *			sep)
{
	...
	for(i = 0; i < num_names; i++)
	{
		...
		{
			/* Print out */
			WT_PRINTF("%s%s", sep, names[i].name);
...
{
	if(value >= num_names)
		WT_PRINTF(" unknown (%d)", value);
	else
		WT_PRINTF(" %s", names[value]);
}
		...
		int		buflen)
{
	...
	WT_PRINTF("IE raw value %d [%02X", buflen, iebuf[0]);
	for(i = 1; i < buflen; i++)
		WT_PRINTF(":%02X", iebuf[i]);
	WT_PRINTF("]\n");
#endif
	...
	if(iebuf[0] == 0xdd)
		WT_PRINTF("WPA Version %d\n", ver);
	if(iebuf[0] == 0x30)
		WT_PRINTF("IEEE 802.11i/WPA2 Version %d\n", ver);
	...
	if(ielen < (offset + 4))
	{
		...
		WT_PRINTF("                        Group Cipher : TKIP\n");
		WT_PRINTF("                        Pairwise Cipher : TKIP\n");
		return;
	}
	...
	{
		WT_PRINTF("                        Group Cipher : Proprietary\n");
	}
	else
	{
		WT_PRINTF("                        Group Cipher :");
	...
	if(ielen < (offset + 2))
	{
		...
		WT_PRINTF("                        Pairwise Ciphers : TKIP\n");
		return;
	}
	...
	offset += 2;
	WT_PRINTF("                        Pairwise Ciphers (%d) :", cnt);
	...
	cnt = iebuf[offset] | (iebuf[offset + 1] << 8);
	offset += 2;
	WT_PRINTF("                        Authentication Suites (%d) :", cnt);
	...
	* allowed.) 8-) */
	...
	{
		WT_PRINTF("                       Preauthentication Supported\n");
	}
}
		     ...
		     int		has_range)
{
	...
	{
		case SIOCGIWAP:
			WT_PRINTF("          Cell %02d - Address: %s\n", state->ap_num,
			iw_saether_ntop(&event->u.ap_addr, buffer));
			state->ap_num++;
			break;
		case SIOCGIWNWID:
			if(event->u.nwid.disabled)
				WT_PRINTF("                    NWID:off/any\n");
			else
				WT_PRINTF("                    NWID:%X\n", event->u.nwid.value);
			break;
		case SIOCGIWFREQ:
		...
		case SIOCGIWESSID:
		{
			...
			if(event->u.essid.flags)
			{
				...
				WT_PRINTF("                    ESSID:off/any/hidden\n");
		}
			break;
		case SIOCGIWENCODE:
		{
			...
			else
			{
				...
				if((event->u.data.flags & IW_ENCODE_INDEX) > 1)
					WT_PRINTF(" [%d]", event->u.data.flags & IW_ENCODE_INDEX);
				if(event->u.data.flags & IW_ENCODE_RESTRICTED)
					WT_PRINTF("   Security mode:restricted");
				if(event->u.data.flags & IW_ENCODE_OPEN)
					WT_PRINTF("   Security mode:open");
				WT_PRINTF("\n");
			}
		}
			break;
			...
			for(i = 0; i < IW_SIZE_MODUL_LIST; i++)
			{
				if((modul & iw_modul_list[i].mask) == iw_modul_list[i].mask)
				{
					if((n++ % 8) == 7)
						...
						WT_PRINTF(" ; ");
					WT_PRINTF("%s", iw_modul_list[i].cmd);
				}
			}
			WT_PRINTF("\n");
		}
		...
		case IWEVCUSTOM:
		{
			...
			break;
		default:
			WT_PRINTF("                    (Unknown Wireless Token 0x%04X)\n",
				event->cmd);
	}	/* switch(event->cmd) */
}
		    ...
		    int		count)		/* Args count */
{
	...
	{
		WT_DBGPRINT_ERR(("*** Please report to jt@hpl.hp.com your platform details\n"));
		WT_DBGPRINT_ERR(("*** and the following line :\n"));
		WT_DBGPRINT_ERR(("*** IW_EV_LCP_PK2_LEN = %zu ; IW_EV_POINT_PK2_LEN = %zu\n\n",
	...
	{
		WT_DBGPRINT_ERR(("%-8.16s  Interface doesn't support scanning.\n\n",
			ifname));
		return(-1);
	}
	...
	* from iwconfig... */
	while(count > 0)
	{
		...
		*/
		if(!strncmp(args[0], "essid", 5))
		{
			if(count < 1)
			{
				WT_DBGPRINT_ERR(("Too few arguments for scanning option [%s]\n",
					args[0]));
				return(-1);
			}
			args++;
			...
			{
				WT_DBGPRINT_ERR(("Invalid scanning option [%s]\n", args[0]));
				return(-1);
			}
	...
	else
	{
		...
		if(iw_set_ext(skfd, ifname, SIOCSIWSCAN, &wrq) < 0)
		{
			...
			{
				WT_DBGPRINT_ERR(("%-8.16s  Interface doesn't support scanning : %s\n\n",
			...
			* But, don't wait !!! */
			...
			WT_DBGPRINT_ERR(("%-8.16s  (Could not trigger scanning, just reading left-over results)\n", ifname));
	...
	while(1)
	{
		...
		if(ret < 0)
		{
			if(errno == EAGAIN || errno == EINTR)
				continue;
			WT_DBGPRINT_ERR(("Unhandled signal - exiting...\n"));
			return(-1);
		}
		...
		if(ret == 0)
		{
			...
			if(newbuf == NULL)
			{
				if(buffer)
					FREE(buffer);
				WT_DBGPRINT_ERR(("%s: Allocation failed\n", __FUNCTION__));
				return(-1);
			}
			buffer = newbuf;
			...
			if(iw_get_ext(skfd, ifname, SIOCGIWSCAN, &wrq) < 0)
			{
				...
				* various increasing sizes. Jean II */
				...
				/* Bad error */
				FREE(buffer);
				WT_DBGPRINT_ERR(("%-8.16s  Failed to read scan data : %s\n\n",
					ifname, strerror(errno)));
				return(-2);
			}
			else
	...
	if(wrq.u.data.length)
	{
		...
		WT_PRINTF("Scan result %d [%02X", wrq.u.data.length, buffer[0]);
		for(i = 1; i < wrq.u.data.length; i++)
			WT_PRINTF(":%02X", buffer[i]);
		WT_PRINTF("]\n");
#endif
		WT_PRINTF("%-8.16s  Scan completed :\n", ifname);
		...
		WT_PRINTF("%-8.16s  No scan results\n\n", ifname);
		...
		int		count)		/* Args count */
{
	...
	if(iw_get_range_info(skfd, ifname, &range) < 0) {
		WT_DBGPRINT_ERR(("%-8.16s  no frequency information.\n\n",
			ifname));
	}
	else
	{
		...
		{
			WT_PRINTF("%-8.16s  %d channels in total; available frequencies :\n",
			...
			for(k = 0; k < range.num_frequency; k++)
			{
				...
				WT_PRINTF("          Channel %.2d : %s\n",
					range.freq[k].i, buffer);
			}
		}
		else
			WT_PRINTF("%-8.16s  %d channels\n",
				ifname, range.num_channels);
		   ...
		   int		count)		/* Args count */
{
	...
	/* Extract range info */
	if(iw_get_range_info(skfd, ifname, &range) < 0) {
		WT_DBGPRINT_ERR(("%-8.16s  no bit-rate information.\n\n",
			ifname));
	}
	else
	{
		...
		{
			WT_PRINTF("%-8.16s  %d available bit-rates :\n",
		...
		if(range.bitrate_capa & IW_BITRATE_BROADCAST)
		{
			...
			{
				iw_print_bitrate(buffer, sizeof(buffer), wrq.u.bitrate.value);
				WT_PRINTF("          Broadcast Bit Rate%c%s\n",
					(wrq.u.bitrate.fixed ? '=' : ':'), buffer);
			}
		}
		...
		int		count)		/* Args count */
{
	...
	/* Extract range info */
	if(iw_get_range_info(skfd, ifname, &range) < 0) {
		WT_DBGPRINT_ERR(("%-8.16s  no encryption keys information.\n\n",
			ifname));
	}
	else
	{
		WT_PRINTF("%-8.16s  ", ifname);
		/* Print key sizes */
		if((range.num_encoding_sizes > 0) &&
			(range.num_encoding_sizes < IW_MAX_ENCODING_SIZES))
		{
			WT_PRINTF("%d key sizes : %d", range.num_encoding_sizes,
			range.encoding_size[0] * 8);
			/* Print them all */
			for(k = 1; k < range.num_encoding_sizes; k++)
				WT_PRINTF(", %d", range.encoding_size[k] * 8);
			WT_PRINTF("bits\n          ");
		}
		/* Print the keys and associate mode */
		WT_PRINTF("%d keys available :\n", range.max_encoding_tokens);
		for(k = 1; k <= range.max_encoding_tokens; k++)
		{
			...
			{
				WT_DBGPRINT_ERR(("Error reading wireless keys (SIOCGIWENCODE): %s\n", strerror(errno)));
				break;
			}
			...
			else
			{
				...
				WT_PRINTF("\t\t[%d]: %s", k, buffer);

				/* Other info... */
				WT_PRINTF(" (%d bits)", wrq.u.data.length * 8);
				WT_PRINTF("\n");
			}
		}
		...
		if(iw_get_ext(skfd, ifname, SIOCGIWENCODE, &wrq) >= 0)
		{
			...
			* message int the loop above */
			WT_PRINTF("          Current Transmit Key: [%d]\n",
	      ...
	      int		count)		/* Args count */
{
	...
	if((iw_get_range_info(skfd, ifname, &range) < 0) ||
		...
		WT_DBGPRINT_ERR(("%-8.16s  no power management information.\n\n",
			ifname));
	}
	else
	{
		...
		{
			WT_PRINTF("Supported modes :\n          ");
			if(range.pm_capa & (IW_POWER_UNICAST_R | IW_POWER_MULTICAST_R))
				WT_PRINTF("\t\to Receive all packets (unicast & multicast)\n          ");
			if(range.pm_capa & IW_POWER_UNICAST_R)
				WT_PRINTF("\t\to Receive Unicast only (discard multicast)\n          ");
			if(range.pm_capa & IW_POWER_MULTICAST_R)
				WT_PRINTF("\t\to Receive Multicast only (discard unicast)\n          ");
			if(range.pm_capa & IW_POWER_FORCE_S)
				WT_PRINTF("\t\to Force sending using Power Management\n          ");
			if(range.pm_capa & IW_POWER_REPEATER)
				WT_PRINTF("\t\to Repeat multicast\n          ");
		}
		   ...
		   int		count)		/* Args count */
{
	...
	if((iw_get_range_info(skfd, ifname, &range) < 0) ||
		(range.we_version_compiled < 10)) {
		WT_DBGPRINT_ERR(("%-8.16s  no transmit-power information.\n\n",
			ifname));
	}
	else
	{
		if((range.num_txpower <= 0) || (range.num_txpower > IW_MAX_TXPOWER))
			WT_PRINTF("%-8.16s  unknown transmit-power information.\n\n", ifname);
		else
		{
			WT_PRINTF("%-8.16s  %d available transmit-powers :\n",
		 ...
		 int		count)		/* Args count */
{
	...
	if((iw_get_range_info(skfd, ifname, &range) < 0) ||
		...
		WT_DBGPRINT_ERR(("%-8.16s  no retry limit/lifetime information.\n\n",
			ifname));
	}
	else
	{
	      ...
	      int	count)		/* Args count */
{
	...
	{
		WT_DBGPRINT_ERR(("%-8.16s  Interface doesn't have a list of Peers/Access-Points\n\n", ifname));
		return(-1);
	}
...
#if 0   //LYH (not support)
	/* Check if we have valid mac address type */
	if(iw_check_mac_addr_type(skfd, ifname) < 0)
	{
		WT_DBGPRINT_ERR(("%-8.16s  Interface doesn't support MAC addresses\n\n", ifname));
		return(-2);
	}
#endif
	...
	/* Display it */
	if(n == 0)
		WT_PRINTF("%-8.16s  No Peers/Access-Point in range\n", ifname);
	else
		WT_PRINTF("%-8.16s  Peers/Access-Points in range:\n", ifname);
	for(i = 0; i < n; i++)
	{
		if(has_qual)
		{
			/* Print stats for this address */
			WT_PRINTF("    %s : ", iw_saether_ntop(&hwa[i], temp));
		      ...
		      int		count)		/* Args count */
{
	...
	if((iw_get_range_info(skfd, ifname, &range) < 0) ||
		(range.we_version_compiled < 10)) {
		WT_DBGPRINT_ERR(("%-8.16s  no wireless event capability information.\n\n",
			ifname));
	}
	else
	{
		...
		WT_PRINTF("%-8.16s  Wireless Events supported :\n", ifname);
		...
		int		count)		/* Args count */
{
	...
	if((iw_get_range_info(skfd, ifname, &range) < 0) ||
		...
		WT_DBGPRINT_ERR(("%-8.16s  no authentication information.\n\n",
			ifname));
	}
	else
	{
		...
		if(!range.enc_capa)
		{
			WT_PRINTF("%-8.16s  unknown authentication information.\n\n", ifname);
		}
		else
		{
			/* Display advanced encryption capabilities */
			WT_PRINTF("%-8.16s  Authentication capabilities :", ifname);
			...
			{ 
				...
				{
					WT_PRINTF("          Current %s :", iw_auth_settings[k].label);
					if(iw_auth_settings[k].names != NULL)
						...
						WT_PRINTF((wrq.u.param.value) ? " yes" : " no");
					WT_PRINTF("\n");
				}
			}
		}
		   ...
		   int		count)		/* Args count */
{
	...
	/* Extract range info */
	if(iw_get_range_info(skfd, ifname, &range) < 0) {
		WT_DBGPRINT_ERR(("%-8.16s  no wpa key information.\n\n",
			ifname));
	}
	else
	{
		...
		for(k = 1; k <= range.max_encoding_tokens; k++)
		{
			...
			{
				WT_DBGPRINT_ERR(("Error reading wpa keys (SIOCGIWENCODEEXT): %s\n", strerror(errno)));
				break;
			}
			...
			else
			{
				...
				/* Key size */
				WT_PRINTF(" (%d bits)", extinfo->key_len * 8);
				WT_PRINTF("\n");
		  ...
		  int		count)		/* Args count */
{
	...
	if(iw_get_ext(skfd, ifname, SIOCGIWGENIE, &wrq) < 0) {
		WT_DBGPRINT_ERR(("%-8.16s  no generic IE (%s).\n\n",
			ifname, strerror(errno)));
	}
	else
	{
		WT_DBGPRINT_ERR(("%-8.16s\n", ifname));
		if(wrq.u.data.length == 0)
			WT_PRINTF("          empty generic IE\n");
		else
		 ...
		 int		count)		/* Args count */
{
	...
	if((iw_get_range_info(skfd, ifname, &range) < 0) ||
		(range.we_version_compiled < 11)) {
		WT_DBGPRINT_ERR(("%-8.16s  no modulation information.\n\n",
			ifname));
	}
	else
	{
		if(range.modul_capa == 0x0)
			WT_PRINTF("%-8.16s  unknown modulation information.\n\n", ifname);
		else
		{
			int i;
			WT_PRINTF("%-8.16s  Modulations available :\n", ifname);
			...
			if(iw_get_ext(skfd, ifname, SIOCGIWMODUL, &wrq) >= 0)
			{
				...
				WT_PRINTF("          Current modulations %c",
					wrq.u.param.fixed ? '=' : ':');
...
find_command(const char *	cmd)
{
	...
	if(found == NULL)
	{
		WT_DBGPRINT_ERR(("iwlist: unknown command `%s' (check 'iwlist --help').\n", cmd));
		return NULL;
	}

	if(ambig)
	{
		WT_DBGPRINT_ERR(("iwlist: command `%s' is ambiguous (check 'iwlist --help').\n", cmd));
		return NULL;
	}
...
iwlist(int argc, char ** argv)
{
	...
	{
		WT_DBGPRINT_ERR(("iwlist: command `%s' needs fewer arguments (max %d)\n",
