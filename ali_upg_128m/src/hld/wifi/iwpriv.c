...
iw_usage(void)
{
	WT_DBGPRINT_ERR(("Usage: iwpriv interface [private-command [private-arguments]]\n"));
}
...
{
	struct iwreq	wrq;
	UINT8	buffer[4096];	/* Only that big in v25 and later */
	...
	* don't have to bother with it later on... */
	...
	if(k == priv_num)
	{
		WT_DBGPRINT_ERR(("Invalid command : %s\n", cmdname));
		return(-1);
	}
	...
	if(priv[k].cmd < SIOCDEVPRIVATE)
	{
		...
		if(j == priv_num)
		{
			WT_DBGPRINT_ERR(("Invalid private ioctl definition for : %s\n",
				cmdname));
			return(-1);
		}
		...
		(priv[k].set_args & IW_PRIV_SIZE_MASK))
	{
		switch(priv[k].set_args & IW_PRIV_TYPE_MASK)
		{
			...
			default:
				WT_DBGPRINT_ERR(("Not implemented...\n"));
				return(-1);
		}
		...
		if((priv[k].set_args & IW_PRIV_SIZE_FIXED) &&
			(wrq.u.data.length != (priv[k].set_args & IW_PRIV_SIZE_MASK)))
		{
			WT_PRINTF("The command %s needs exactly %d argument(s)...\n",
	...
	* will have to handle the data */
	...
	if(iw_set_ext(skfd, ifname, priv[k].cmd, &wrq))   //LYH WiFi (doesn't matter its set or get)
	{
		WT_DBGPRINT_ERR(("Interface doesn't accept private ioctl...\n"));
		WT_DBGPRINT_ERR(("%s (%X): %s\n", cmdname, priv[k].cmd, strerror(errno)));
		return(-1);
	}
		...
		(priv[k].get_args & IW_PRIV_SIZE_MASK))
	{
		...
		WT_PRINTF("%-8.16s  %s:", ifname, cmdname);
		...
		switch(priv[k].get_args & IW_PRIV_TYPE_MASK)
		{
			...
			case IW_PRIV_TYPE_FLOAT:
			{
				...
				for(j = 0; j < n; j++)
				{
					...
					if(freq >= GIGA)
						WT_PRINTF("%gG  ", freq / GIGA);
					else
						if(freq >= MEGA)
							WT_PRINTF("%gM  ", freq / MEGA);
						else
							WT_PRINTF("%gk  ", freq / KILO);
				}
				WT_PRINTF("\n");
			}
				break;
			...
			default:
				WT_DBGPRINT_ERR(("Not yet implemented...\n"));
				return(-1);
		}
	}	/* if args to set */
	    ...
	    char *	ifname)		/* Dev name */
{
	...
	if(number <= 0)
	{
		...
		WT_DBGPRINT_ERR(("%-8.16s  no private ioctls.\n\n",
			ifname));
		if(priv)
			FREE(priv);
		return(-1);
		...
		int		count)
{
	...
	{
		WT_PRINTF("%-8.16s  Available private ioctls :\n", ifname);
		
		/* Print them all */
		for(k = 0; k < n; k++)
			if(priv[k].name[0] != '\0')
				WT_PRINTF("          %-16.16s (%.4X) : set %3d %s & get %3d %s\n",
	       ...
	       int		count)
{
	...
	{
		WT_PRINTF("%-8.16s  Available read-only private ioctl :\n", ifname);
	    ...
	    char *	ifname)		/* Dev name */
{
  ...
  int		number;
  int		roamcmd;
  char		roam_state;		/* buffer to hold new roam state */
  char		change_roam_state=0;	/* whether or not we are going to
					   change roam states */
