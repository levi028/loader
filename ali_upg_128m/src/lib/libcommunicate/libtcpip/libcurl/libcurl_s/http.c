...
static INT64 m_cur_content_range = -1;

static INT64 http_atold(char *str)
{
    INT64 r = 0;
    int i;
    int l = strlen(str);
...
static int http_isDigit(const char a)
{
    return ((int)a >= '0' && (int)a <= '9');
}

// it can transfer from "bytes 1258-100000/100001" to 1258
static INT64 http_string2lld(char *input)
{
	//char       *r;
    char *p = input;
    ...
    for (p = input; (*p) && !http_isDigit(*p);  p++);
    {
    }
...
static INT64 http_set_content_range(char *content_range)
{
    char *pFind = NULL;
    m_cur_content_range = -1;
    
    if (NULL == content_range)
        return 0;
    pFind = strstr(content_range, "/");
    if (pFind)
    {
        m_cur_content_range = http_atold(pFind);
    }
    return m_cur_content_range;
}

INT64 http_get_content_range()
{
    return m_cur_content_range;
}
...
#define HEADER1 k->p /* no conversion needed, just use k->p */
    ...
    /* check for Content-Type: header lines to get the MIME-type */
      ...
      */
      ...
      range = http_set_content_range(ptr);
      offset_set = http_string2lld(ptr);
      //libc_printf("range: %lld, offset: %lld\n", range, offset_set);
      range = range >= offset_set ? (range - offset_set) : range;
      Curl_pgrsSetDownloadSize(data, range);
