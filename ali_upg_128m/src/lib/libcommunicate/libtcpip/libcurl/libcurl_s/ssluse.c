...
#if 1
#define SSL_PRINTF	libc_printf
#else
#define SSL_PRINTF(...)	do{}while(0)
#endif
  ...
  if(1 != err) {
    ...
    if(SSL_ERROR_WANT_READ == detail) {
      ...
      case 0x14090086:
        ...
        if(lerr != X509_V_OK) {
        #if 1
        	SSL_PRINTF("SSL certificate problem: %d\n", lerr);
        #else
  ...
  if(rc) {
    ...
    if(data->set.ssl.certverifyresult != X509_V_OK) {
      if(data->set.ssl.verifypeer) {
        ...
        if(strict)
        #if 1
	        SSL_PRINTF("SSL certificate verify result: %d\n", lerr);
        #else
      ...
      {
      #if 1
      	SSL_PRINTF("SSL certificate verify result: %d, continuing anyway.\n", lerr);
      #else
