...
#if 0 // For Debug
        libc_printf("Response nread:%d\n", nread);
        for (i = 0; i < nread; i++)
        {
            libc_printf("%c", k->buf[i]);
        }
        libc_printf("\n");
#endif
...
static CURLcode Curl_do_perform(struct SessionHandle *data)
{
  ...
  for(;;) {
    ...
    if(NULL != conn && ((conn->handler->protocol& CURLPROTO_RTMP) ||
		(conn->handler->protocol& CURLPROTO_RTMPT) || (conn->handler->protocol& CURLPROTO_RTMPE)
		||(conn->handler->protocol& CURLPROTO_RTMPS) ||(conn->handler->protocol& CURLPROTO_RTMPTE)
		||(conn->handler->protocol& CURLPROTO_RTMPTS))) /* save proto generic handle to urlfile handle. add by ALi*/
    {
		data->state.proto.generic = conn->proto.generic;
		b_rtmp = TRUE;
	}
    if(res == CURLE_OK) {
      bool do_done;
	...
	/* clear urlfile proto handle. in this case, generic is only void* pointer
	 * no valid memory, just set NULL is OK.
	*/
	if(b_rtmp && data->state.proto.generic ) 
    {
