                     ...
                     va_list param)
{
  ...
  case CURLOPT_RTMP_LIVESTREAM:
    data->set.rtmp_livestream = (0 != va_arg(param, long))?TRUE:FALSE;
    break;
  case CURLOPT_RTMP_STARTTIME:
    data->set.rtmp_starttime = va_arg(param, long);
    break;
  case CURLOPT_RTMP_STOPTIME:
    data->set.rtmp_stoptime = va_arg(param, long);
    break;
  case CURLOPT_RTMP_TIMEOUT:
    data->set.rtmp_timeout = va_arg(param, long);
    break;

  case CURLOPT_SEEK_FLAG:
    data->set.ulseekflag = va_arg(param,long);
    break;

  case CURLOPT_SEEK_INFO:
    data->set.ulseekinfo = va_arg(param, long);
    break;

  case CURLOPT_TIME_LENGTH:
    data->set.dtimelength = va_arg(param,double);
    break;

  case CURLOPT_SEEK_TIME:
    data->set.dseektime = va_arg(param, double);
    break;

  case CURLOPT_CURRENT_POS:
    argptr = va_arg(param,char *);
    data->set.sllcurrentpos = (long long)(*(long long *)argptr);
    break;
    
  case CURLOPT_NOT_STRIP_URLPARAM:
    data->set.not_strip_urlparam = (0 != va_arg(param, long))?TRUE:FALSE;
    break;
                                    ...
                                    char *passwd)
{
  ...
  *prot_missing = FALSE;
  ...
  if(!data->set.not_strip_urlparam)
  {
