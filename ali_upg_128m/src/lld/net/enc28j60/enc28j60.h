...
//#define ENC_TX_STATUS
//#define ENC_DEBUG_BUF
//#define ENC_DBG_MAC
//#define ENC_DBG_ARP
//#define ENC_DBG_RARP
//#define ENC_DBG_IP
//#define ENC_DBG_TCP
//#define ENC_DBG_UDP
#define NET_PRINTF        libc_printf
#else
#define NET_PRINTF(...) do{}while(0)
#endif

#ifdef ENC_DEBUG_TX
#define DUMP_TSM_FRAME    1
#else
#define DUMP_TSM_FRAME    0
#endif

#ifdef ENC_DEBUG_RX
#define DUMP_RCV_FRAME        1
#else
#define DUMP_RCV_FRAME        0
#endif

#ifdef ENC_SIMPLE_DEBUG
#define NET_SIM_PRINTF        libc_printf
#else
#define NET_SIM_PRINTF(...) do{}while(0)
#endif

#define MAX_MAC_FRM        (1514ul)
#define HW_RAMSIZE            (8192ul)
#define TXSIZE                (1ul+MAX_MAC_FRM+7ul)
#define TXSTART                 (HW_RAMSIZE - TXSIZE)
#define RXSTART                (0ul)                        // Should be an even memory address; must be 0 for errata
#define RXSTOP                ((TXSTART-2ul) | 0x0001ul)    // Odd for errata workaround
#define RXSIZE                (RXSTOP-RXSTART+1ul)
#define RXPKTSIZE            (1514+4+6)
#define RXTHLD                (RXSIZE/RXPKTSIZE)
#define ENC_RX_BUF_LEN         (2048)
#define ENC_TX_BUF_LEN         ENC_RX_BUF_LEN
...
typedef struct
{
    ...
    UINT32    int_number;
    UINT8     int_enable;
    UINT8    enc_clk_div;        /*ENC working clk shall no more than 20MHz*/
    UINT32    flash_bak;        /*0xb8000098 bak up*/
    UINT32    base_addr;
    OSAL_ID    tx_rx_mutex;
    OSAL_ID tsk_id;
    UINT32     tsk_sleep;
...
typedef union {
    ...
    struct {
        ...
        unsigned char    packet_defer:1;
        unsigned char    excessive_defer:1;
        unsigned char    maximum_collisions:1;
        unsigned char    late_collision:1;
        unsigned char    giant:1;
        unsigned char    underrun:1;
        UINT16                bytes_transmitted_on_wire;
        unsigned char    control_frame:1;
        unsigned char    pausecontrol_frame:1;
        unsigned char    backpressure_applied:1;
        unsigned char    vlantagged_frame:1;
...
typedef union {
    ...
    struct {
        UINT16               byte_count;
        unsigned char    previously_ignored:1;
        unsigned char    rxdcpreviously_seen:1;
        unsigned char    carrier_previously_seen:1;
        unsigned char    code_violation:1;
        unsigned char    crcerror:1;
        unsigned char    length_check_error:1;
        unsigned char    length_out_of_range:1;
        ...
        unsigned char    unsupported_opcode:1;
