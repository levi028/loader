...
//    }
    ...
    {
    	CXD2856_LOG(pDemod,"==i2c read error\n");
        result = SONY_RESULT_ERROR_I2C;
    }
    //CXD2856_LOG_I2C(pDemod, result, 1, deviceAddress, &subAddress, 1);
    //CXD2856_LOG_I2C(pDemod, result, 0, deviceAddress, pData, size);
...
sony_result_t nim_cxd2856_i2c_CommonWriteRegister(sony_i2c_t* pI2c, uint8_t deviceAddress, uint8_t subAddress, const uint8_t* pData, uint32_t size)
{
    ...
    if(size > BURST_WRITE_MAX){
        /* Buffer is too small... */
        CXD2856_LOG(pDemod, "s%(): buffer[%d] is too small: pData[%d]\r\n", __FUNCTION__, BURST_WRITE_MAX, size);
        SONY_TRACE_I2C_RETURN(SONY_RESULT_ERROR_ARG);
    }
    osal_mutex_lock(pDemod->i2c_mutex_id, OSAL_WAIT_FOREVER_TIME);
...
//    result = pI2c->Write(pI2c, deviceAddress, buffer, size+1, SONY_I2C_START_EN | SONY_I2C_STOP_EN);
	...
	{
    	CXD2856_LOG(pDemod,"==i2c write error\n");
        result = SONY_RESULT_ERROR_I2C;
    }
    
    //CXD2856_LOG_I2C(pDemod, result, 1, deviceAddress, buffer, size+1);
...
sony_result_t nim_cxd2856_TunerGateway(void* nim_dev_priv, UINT8	tuner_address , UINT8* wdata , int wlen , UINT8* rdata , int rlen)
{
    ...
    if( (0 == wlen) && (0 == rlen) )
    {
        CXD2856_LOG(pDemod, "s%(): No data for read/write.\r\n", __FUNCTION__);
        SONY_TRACE_I2C_RETURN(SONY_RESULT_ERROR_ARG);
    }
	...
	if ( wlen != 0 ) //write to tuner
	{
		...
		{
			CXD2856_LOG(pDemod, "s%(): buffer[%d] is too small: Write wdata[%d]\r\n", __FUNCTION__, BURST_WRITE_MAX, wlen);
			SONY_TRACE_I2C_RETURN(SONY_RESULT_ERROR_ARG);
		}
	...
	if ( rlen != 0 ) //read from tuner
	{
		...
		{
			CXD2856_LOG(pDemod, "s%(): buffer[%d] is too small: Read rdata[%d]\r\n", __FUNCTION__, BURST_WRITE_MAX, rlen);
			SONY_TRACE_I2C_RETURN(SONY_RESULT_ERROR_ARG);
		}
