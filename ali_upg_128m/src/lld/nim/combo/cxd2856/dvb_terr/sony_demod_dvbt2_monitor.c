...
sony_result_t sony_demod_dvbt2_monitor_BBHeader (sony_demod_t * pDemod,
                                                 sony_dvbt2_plp_btype_t type,
                                                 sony_dvbt2_bbheader_t * pBBHeader)
{
    SONY_TRACE_ENTER ("sony_demod_dvbt2_monitor_BBHeader");
...

    {
        ...
        pBBHeader->streamInput = (sony_dvbt2_stream_t)((data[1] >> 6) & 0x03);
        ...
        pBBHeader->plpMode = (data[4] & 0x01) ? SONY_DVBT2_PLP_MODE_HEM : SONY_DVBT2_PLP_MODE_NM;
...
sony_result_t sony_demod_dvbt2_monitor_InBandBTSRate (sony_demod_t * pDemod,
                                                      sony_dvbt2_plp_btype_t type,
                                                      uint32_t * pTSRateBps)
{
    SONY_TRACE_ENTER ("sony_demod_dvbt2_monitor_InBandBTSRate");
...

    {
        ...
        if (pDemod->pI2c->ReadRegister (pDemod->pI2c, pDemod->i2cAddressSLVT, addr, &data, 1) != SONY_RESULT_OK) {
            SLVT_UnFreezeReg (pDemod);
            SONY_TRACE_RETURN (SONY_RESULT_ERROR_I2C);
        }
...

    {
        ...
        if (pDemod->pI2c->ReadRegister (pDemod->pI2c, pDemod->i2cAddressSLVT, addr, &data[0], 4) != SONY_RESULT_OK) {
            SLVT_UnFreezeReg (pDemod);
            SONY_TRACE_RETURN (SONY_RESULT_ERROR_I2C);
        }
...
static sony_result_t dvbt2_CalcQuality (sony_demod_t * pDemod,
                                        uint32_t ber, int32_t snr, uint8_t * pQuality)
{
    ...
    SONY_TRACE_ENTER ("dvbt2_CalcQuality");
...
sony_result_t sony_demod_dvbt2_monitor_AveragedPreBCHBER (sony_demod_t * pDemod,
                                                          uint32_t * pBER)
{
    uint8_t rdata[32];
    ...
    SONY_TRACE_ENTER ("sony_demod_dvbt2_monitor_AveragedPreBCHBER");
...

    {
         ...
         * <SLV-T>   22h     5Eh     [1:0]        IL1POST_PLP_FEC_TYPE[1:0]
         */
        if (pDemod->pI2c->ReadRegister (pDemod->pI2c, pDemod->i2cAddressSLVT, 0x5E, rdata, 1) != SONY_RESULT_OK) {
            SLVT_UnFreezeReg (pDemod);
            SONY_TRACE_RETURN (SONY_RESULT_ERROR_I2C);
        }

        plpFecType = (sony_dvbt2_plp_fec_t) (rdata[0] & 0x03);
         ...
         * <SLV-T>   22h     5Bh     [2:0]        IL1POST_PLP_COD[2:0]
         */
        if (pDemod->pI2c->ReadRegister (pDemod->pI2c, pDemod->i2cAddressSLVT, 0x5B, rdata, 1) != SONY_RESULT_OK) {
            SLVT_UnFreezeReg (pDemod);
            SONY_TRACE_RETURN (SONY_RESULT_ERROR_I2C);
        }

        plpCr = (sony_dvbt2_plp_code_rate_t) (rdata[0] & 0x07);
    ...
    if (periodType == 0) {
         ...
         * <SLV-T>   20h     72h     [3:0]      OREG_BBER_MES[3:0]
         */
        if (pDemod->pI2c->ReadRegister (pDemod->pI2c, pDemod->i2cAddressSLVT, 0x72, rdata, 1) != SONY_RESULT_OK) {
            SONY_TRACE_RETURN (SONY_RESULT_ERROR_I2C);
        }
...
sony_result_t sony_demod_dvbt2_monitor_AveragedSNR (sony_demod_t * pDemod,
                                                    int32_t * pSNR)
{
    uint8_t rdata[24];
    int hNum = 0;
    ...
    SONY_TRACE_ENTER ("sony_demod_dvbt2_monitor_AveragedSNR");
     ...
     */
    if (pDemod->pI2c->ReadRegister (pDemod->pI2c, pDemod->i2cAddressSLVT, 0x30, rdata, 24) != SONY_RESULT_OK) {
        SONY_TRACE_RETURN (SONY_RESULT_ERROR_I2C);
    }
    ...
    /* Same as sony_demod_dvbt2_monitor_SNR
     *
     * IREG_SNMON_OD is clipped at a maximum of 10876.
     *
     * SNR = 10 * log10 (IREG_SNMON_OD / (12600 - IREG_SNMON_OD)) + 32.0
...
sony_result_t sony_demod_dvbt2_monitor_AveragedQuality (sony_demod_t * pDemod,
                                                        uint8_t * pQuality)
{
    sony_result_t result = SONY_RESULT_OK;
    uint32_t ber = 0;
    int32_t snr = 0;
    SONY_TRACE_ENTER ("sony_demod_dvbt2_monitor_AveragedQuality");
    ...
    /* Get Pre-BCH BER. */
    result = sony_demod_dvbt2_monitor_AveragedPreBCHBER (pDemod, &ber);
    if (result != SONY_RESULT_OK) {
        SONY_TRACE_RETURN (result);
    }

    /* Get SNR value. */
    result = sony_demod_dvbt2_monitor_AveragedSNR (pDemod, &snr);
    if (result != SONY_RESULT_OK) {
        SONY_TRACE_RETURN (result);
    }
