...
static sony_result_t dvbt_CalcQuality (sony_demod_t * pDemod,
                                       uint32_t ber, int32_t snr, uint8_t * pQuality)
{
    ...
    */
    ...
    sony_result_t result = SONY_RESULT_OK;
    SONY_TRACE_ENTER ("dvbt_calcQuality");
                                               ...
                                               uint8_t * pQuality)
{
    ...
    /* Get SNR value. */
    result = sony_demod_dvbt_monitor_SNR (pDemod, &snr);
    if (result != SONY_RESULT_OK) {
        SONY_TRACE_RETURN (result);
    }
...
sony_result_t sony_demod_dvbt_monitor_AveragedPreRSBER (sony_demod_t * pDemod,
                                                        uint32_t * pBER)
{
    uint8_t rdata[32];
    ...
    SONY_TRACE_ENTER ("sony_demod_dvbt_monitor_AveragedPreRSBER");
...
sony_result_t sony_demod_dvbt_monitor_AveragedSNR (sony_demod_t * pDemod,
                                                   int32_t * pSNR)
{
    uint8_t rdata[24];
    int hNum = 0;
    ...
    SONY_TRACE_ENTER ("sony_demod_dvbt_monitor_AveragedSNR");
    ...
    /* Same as sony_demod_dvbt_monitor_SNR
     *
     * IREG_SNMON_OD is clipped at a maximum of 4996.
     *
     * SNR = 10 * log10 (IREG_SNMON_OD / (5350 - IREG_SNMON_OD)) + 28.5
...
sony_result_t sony_demod_dvbt_monitor_AveragedQuality (sony_demod_t * pDemod,
                                                       uint8_t * pQuality)
{
    sony_result_t result = SONY_RESULT_OK;
    uint32_t ber = 0;
    int32_t snr = 0;
    SONY_TRACE_ENTER ("sony_demod_dvbt_monitor_AveragedQuality");
    ...
    /* Get Pre-RS (Post-Viterbi) BER. */
    result = sony_demod_dvbt_monitor_AveragedPreRSBER (pDemod, &ber);
    if (result != SONY_RESULT_OK) {
        SONY_TRACE_RETURN (result);
    }

    /* Get SNR value. */
    result = sony_demod_dvbt_monitor_AveragedSNR (pDemod, &snr);
    if (result != SONY_RESULT_OK) {
        SONY_TRACE_RETURN (result);
    }
