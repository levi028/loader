                                    ...
                                    sony_dvbt2_tune_param_t * pTuneParam)
{
	...
	/*********add by leo.liu of ALI***********/
	sony_demod_t *pdemod = NULL; 
	pdemod = pInteg->pDemod;
	/************************************/
    ...
    /* Tune the RF part */
	if(NULL == pdemod->dvbt_config.nim_tuner_control)
	{
	...
	{	
		...
		//tune tuner
		if(pdemod->dvbt_config.nim_tuner_control(pdemod->tuner_id,pTuneParam->centerFreqKHz,pTuneParam->bandwidth,0,0,0) < 0)
		{
...
/****************************ali add ***********************************/
	...
	{	
		...
		NIM_MUTEX_ENTER(pdemod);
		//tune tuner
		if(pdemod->dvbt_config.nim_tuner_control(pdemod->tuner_id,centerFreqKHz,bandwidth,0,0,0) < 0)
		{
			NIM_MUTEX_LEAVE(pdemod);
			result = SONY_RESULT_ERROR_I2C;
			SONY_TRACE_RETURN (result);
...
sony_result_t sony_integ_dvbt2_WaitTSLock (sony_integ_t * pInteg, sony_dvbt2_profile_t profile)
{
    ...
    for (;;) {
		...
		/*original company debug function,Ali doesn't need*/
        /* Check cancellation. 
        result = sony_integ_CheckCancellation (pInteg);
        if (result != SONY_RESULT_OK) {
            SONY_TRACE_RETURN (result);
...
/*
This api was modified by ali;add a para:rfLevel.the unit of rfLevel must be 0.001dbm
*/

sony_result_t sony_integ_dvbt2_monitor_SSI (sony_integ_t * pInteg, uint8_t * pSSI,int32_t rfLevel)
{
    //int32_t rfLevel; //ali add
...
static sony_result_t dvbt2_WaitDemodLock (sony_integ_t * pInteg, sony_dvbt2_profile_t profile)
{
    ...
    for (;;) {
		...
		/*query the demod lock status repeatedly*/
        /*case SONY_DEMOD_LOCK_RESULT_UNLOCKED:
            SONY_TRACE_RETURN (SONY_RESULT_ERROR_UNLOCK);*/
