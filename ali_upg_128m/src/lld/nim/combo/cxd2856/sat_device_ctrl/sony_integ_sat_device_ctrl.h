...
/**
 ...
 In common, LNB controller needs time to output stable volage after waking up.
...
typedef struct {
    ...
    sony_tuner_sat_t * pTunerReal;          /**< Tuner instance. */
...
*/
typedef struct {
    sony_tuner_sat_t tunerSinglecable;                        /**< Tuner instance for single cable. */
...
------------------------------------------------------------------------------*/
/**
 @brief Disable satellite device control function.

 @note  LNB controller will be set to Sleep state.
        To enable satellite device control function, simply call
        satellite device control functions like ::sony_integ_sat_device_ctrl_SetVoltageTone.
