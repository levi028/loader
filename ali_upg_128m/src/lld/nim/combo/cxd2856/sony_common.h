...
#define SONY_SLEEP(n) osal_task_sleep(n)

#define NIM_MUTEX_ENTER(pdemod)  \
	if(pdemod->though_mutex != OSAL_INVALID_ID) \
	{ \
		do{ \
			pdemod->though_ret = osal_semaphore_capture(pdemod->though_mutex, 1000);\
			if (pdemod->though_ret!= OSAL_E_OK) \
				{libc_printf("nim_cxd2856_sema[%d] capture timeout, retry...\n", \
					pdemod->though_mutex);} \
		}while(pdemod->though_ret != OSAL_E_OK); \
	}

#define NIM_MUTEX_LEAVE(pdemod) \
	if (pdemod->though_mutex != OSAL_INVALID_ID) \
	{ \
		osal_semaphore_release(pdemod->though_mutex); \
	}
...
#define SONY_TRACE_ENTER(func) libc_printf("\n%s,%s,%d\n", __FUNCTION__, __FILE__, __LINE__)
#define SONY_TRACE_RETURN(result) \
    SONY_MACRO_MULTILINE_BEGIN \
        libc_printf("result=%d,%s,%d\n", result, __FUNCTION__, __LINE__); \
        return (result); \
    SONY_MACRO_MULTILINE_END
