...
MXL_STATUS MxLWare603_OEM_WriteRegister(UINT32 tuner_id, UINT8 regaddr, UINT8 regdata)
{
    ...
    {
        status = nim_i2c_write(MxL603_Config[tuner_id].i2c_type_id, MxL603_Config[tuner_id].c_tuner_base_addr, (UINT8 *)&Cmd, 2);
    }
	if(status!=SUCCESS)
	{
		MXL603_PRINTF("==tuner w error %d 0x%x\n",MxL603_Config[tuner_id].i2c_type_id,MxL603_Config[tuner_id].c_tuner_base_addr);
	}
...
MXL_STATUS MxLWare603_OEM_ReadRegister(UINT32 tuner_id, UINT8 regaddr, UINT8 *dataptr)
{
    ...
    if (run_on_through_mode(tuner_id))
    {
        ...
        if(status != SUCCESS)
    	{
    		MXL603_PRINTF("==tuner r 00 error %d 0x%x\n",MxL603_Config[tuner_id].i2c_type_id,MxL603_Config[tuner_id].c_tuner_base_addr);
            return MXL_FALSE;
    	}
        status = m_ThroughMode[tuner_id].dem_write_read_tuner( m_ThroughMode[tuner_id].nim_dev_priv,
                                                           MxL603_Config[tuner_id].c_tuner_base_addr,
        ...
        if(status != SUCCESS)
    	{
    		MXL603_PRINTF("==tuner r 11 error %d 0x%x\n",MxL603_Config[tuner_id].i2c_type_id,MxL603_Config[tuner_id].c_tuner_base_addr);
            return MXL_FALSE;
    	}
    }
    else
    {
        ...
        if(status != SUCCESS)
    	{
    		libc_printf("==tuner r 0 error %d 0x%x\n",MxL603_Config[tuner_id].i2c_type_id,MxL603_Config[tuner_id].c_tuner_base_addr);
            return MXL_FALSE;
    	}
        status = nim_i2c_read(MxL603_Config[tuner_id].i2c_type_id,
                          MxL603_Config[tuner_id].c_tuner_base_addr,
                          dataptr, 1);*/
        status = i2c_write_read(MxL603_Config[tuner_id].i2c_type_id,MxL603_Config[tuner_id].c_tuner_base_addr,Read_Cmd,2,1);              
        if(status != SUCCESS)
    	{
    		MXL603_PRINTF("==tuner r 1 error %d 0x%x\n",MxL603_Config[tuner_id].i2c_type_id,MxL603_Config[tuner_id].c_tuner_base_addr);
            return MXL_FALSE;
    	}
		*dataptr = Read_Cmd[0];
    }
...
INT32 tun_mxl603_init(UINT32 *tuner_id, struct COFDM_TUNER_CONFIG_EXT * ptrtuner_config)
{
    ...
    ret = tun_mxl603_init_common(tuner_id,ctuner_reopen);
...
INT32 tun_mxl603_init_DVBC(UINT32 *tuner_id, struct QAM_TUNER_CONFIG_EXT * ptrtuner_config)
{
    ...
    MXL603_PRINTF("[%s]line=%d,success!\n",__FUNCTION__,__LINE__);
...
INT32 tun_mxl603_get_rf_level(UINT32 tuner_id, UINT16 *rf_level)// return level in dbuV.
{
	...
	MXL603_PRINTF("mxl603 get RF level: %d * 0.01dbm !\n", rxPower);
...
* Return Value: INT32            : Result
*****************************************************************************/
INT32 tun_mxl603_control(UINT32 tuner_id, UINT32 freq, UINT8 bandwidth,
                         UINT8 agc_time_const,UINT8 *data,UINT8 _i2c_cmd)
{
    ...
    switch(bandwidth)
    {
        ...
        case 8:
               ...
               break;
        default:
           MXL603_PRINTF("Error! bandwidth=%d\n",bandwidth);
           break;
    }
...
INT32 tun_mxl603_command(UINT32 tuner_id, INT32 cmd, UINT32 param)
{
    ...
    switch(cmd)
    {
	    ...
	    case NIM_TUNER_GET_RF_POWER_LEVEL:
	        ret = MxLWare603_API_ReqTunerRxPower(tuner_id, &rx_power);
			if (rx_power < 0)
			{
				rx_power = 0 - rx_power;
			}

			rx_power = (rx_power - 350)/10;//unit is 0.1dbm
			*(REAL32 *)param = rx_power;
	        break;
