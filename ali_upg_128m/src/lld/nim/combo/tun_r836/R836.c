...
static UINT8 R840_IMR_Cal_Result = 0;  //1: fail, 0: ok
static UINT8 R840_TF_Check_Result = 0; //1: fail, 0: ok
...
R840_ErrCode R840_IQ_Tree5(UINT8 FixPot, UINT8 FlucPot, UINT8 PotReg, R840_SectType* CompareTree);
...
Sys_Info_Type R840_Sys_Sel(R840_Standard_Type R840_Standard)
{
	...
	switch(R840_Standard)
	{
		...
		case R840_DVB_C_6M_IF_5M:
		case R840_J83B_IF_5M:
			 if(R840_TF_Check_Result==1) //fail
			 {
...
R840_ErrCode R840_Init(void)
{
	...
	if(R840_Initial_done_flag==FALSE)
	{
		  ...
		  R840_IMR_Cal_Result = 0; 
		  R840_TF_Check_Result = 0;
...
R840_ErrCode R840_InitReg(void)
{
	...
	R840_Array[9] = R840_Array[9] & 0x7F;    
	...
	R840_Array[4] = R840_Array[4] & 0xF7;    
	...
	R840_Array[23] = R840_Array[23] & 0xFE;    
	...
	R840_I2C.RegAddr = 0x17;
	R840_Array[23] = (R840_Array[23] & 0x27) | 0x90;    
	R840_I2C.Data = R840_Array[23];
	if(I2C_Write(&R840_I2C) != RT_Success)
	    return RT_Fail;
	...
	R840_Array[17] = (R840_Array[17] & 0x3F) | 0x00;    
...
R840_ErrCode R840_Xtal_Check(void)
{
	...
	R840_Array[20] = (R840_Array[20] & 0x00) | 0x0B;  
	...
	R840_Array[16] = (R840_Array[16] & 0x1B) | 0x60;  
...
R840_ErrCode R840_IQ(R840_SectType* IQ_Pont)
{
	...
	//Compare_IQ[0].Iqcap = R840_iniArray[3] & 0xFC;
		...
		{	    
		   ...
		   if(R840_IQ_Tree5(Compare_IQ[0].Gain_X, Compare_IQ[0].Phase_Y, 0x08, &Compare_IQ[0]) != RT_Success) //Y
		     return RT_Fail;
		...
		else
		{
		   ...
		   if(R840_IQ_Tree5(Compare_IQ[0].Phase_Y, Compare_IQ[0].Gain_X, 0x09, &Compare_IQ[0]) != RT_Success) //X
		     return RT_Fail;
...
R840_ErrCode R840_IQ_Tree5(UINT8 FixPot, UINT8 FlucPot, UINT8 PotReg, R840_SectType* CompareTree)
{
	...
	for(TreeCunt = 0;TreeCunt < TreeTimes;TreeCunt ++)
	{
		...
		if(R840_Muti_Read(&CorTemp[TreeCunt].Value) != RT_Success)
			return RT_Fail;
...
{	 
    ...
    if(R840_Fil_Cal_flag[RT_Standard] == FALSE)
	{
		...
		//protection
		if(R840_IMR_Cal_Result==1) //fail
		{
			if((R840_Fil_Cal_BW[RT_Standard]==0x60) && (R840_Fil_Cal_code[RT_Standard]==15))  //6M/15
			{
				R840_Fil_Cal_BW[RT_Standard] = R840_Fil_Cal_BW_def[RT_Standard];
				R840_Fil_Cal_code[RT_Standard] = R840_Fil_Cal_code_def[RT_Standard];
				R840_Fil_Cal_LpfLsb[RT_Standard] = 0;
			}
		}
...
R840_ErrCode R840_Standby(R840_LoopThrough_Type R840_LoopSwitch)
{
	...
	//set LNA gain
     R840_I2C.RegAddr = 0x05;
     R840_Array[5] = (R840_Array[5] & 0xE0) | 31;
	 R840_I2C.Data = R840_Array[5];
     if(I2C_Write(&R840_I2C) != RT_Success)
	       return RT_Fail;
     ...
     R840_I2C.RegAddr = 0x1A;
     R840_Array[26] = (R840_Array[26] & 0xF0) | 0;
	 R840_I2C.Data = R840_Array[26];
     if(I2C_Write(&R840_I2C) != RT_Success)
	       return RT_Fail;
	 ...
	 //set Mixer gain
     R840_I2C.RegAddr = 0x07;
     R840_Array[7] = (R840_Array[7] & 0xF0) | 0;
	 R840_I2C.Data = R840_Array[7];
     if(I2C_Write(&R840_I2C) != RT_Success)
	       return RT_Fail;
	 ...
	 //set VGA pin, +32.5dB
	 R840_I2C.RegAddr = 0x0C;
	 R840_Array[12] = (R840_Array[12] & 0xE0) | 0x0F;   
	 R840_I2C.Data = R840_Array[12];
	 if(I2C_Write(&R840_I2C) != RT_Success)
		 return RT_Fail;
	 ...
	 //set LNA band = high
	 R840_I2C.RegAddr = 0x05;
	 R840_Array[5] = (R840_Array[5] & 0x9F) | 0x00; //R5[6:5]=00
	 R840_I2C.Data = R840_Array[5];
	 if(I2C_Write(&R840_I2C) != RT_Success)
		 return RT_Fail;
	 ...
	 //set Byp_lpf=bypass
	 R840_I2C.RegAddr = 0x01;
	 R840_Array[1] = (R840_Array[1] & 0xFB) | 0x00; //R1[2]=0
	 R840_I2C.Data = R840_Array[1];
	 if(I2C_Write(&R840_I2C) != RT_Success)
		 return RT_Fail;
...
R840_ErrCode R840_WakeUp(void)
{
	...
	//PLL_LDO_D (LDO1=200R, LDO2=200R), R23[7:6]=10; R23[4:3]=10
	R840_I2C.RegAddr = 0x17;
	R840_Array[23] = (R840_Array[23] & 0x27) | Pll_Dldo;
	R840_I2C.Data = R840_Array[23];
	if(I2C_Write(&R840_I2C) != RT_Success)
	    return RT_Fail;
	...
	//PLL_LDO_A=2.35V, R17[7:6]=00
	R840_I2C.RegAddr = 0x11; 
	R840_Array[17] = (R840_Array[17] & 0x3F) | Pll_Aldo;  
	R840_I2C.Data = R840_Array[17];
	if(I2C_Write(&R840_I2C) != RT_Success)
	    return RT_Fail;
...
R840_ErrCode R840_AGC_Slow(void)  //Set AGC clock to 60Hz
{
		 //AGC CLK to 60hz		 
		 R840_I2C.RegAddr = 0x1B;			
...
//---------------Test Function------------------------//
UINT8 R840_RingPll_Check(void) //for CTMR testing
{
    UINT8  result = 0;   //1:fail;  0: ok
	UINT8  R840_RingCheckArray[R840_REG_NUM] = {  //16MHz

                           0x0E, 0x0D, 0x40, 0x40, 0x65,   
					  //   0x00, 0x01, 0x02, 0x03, 0x04 

	                       0xC0, 0x97, 0x60, 0x40, 0x00, 0x0F, 0x60, 0x30, 0xA4,
					//     0x05  0x06  0x07  0x08  0x09  0x0A  0x0B  0x0C  0x0D                                                    	   
													  
						   0x95, 0x94, 0x73, 0x02, 0x72, 0x10, 0x15, 0x99, 0x59, 
					//     0x0E  0x0F  0x10  0x11  0x12  0x13  0x14  0x15  0x16                                                    	   

						   0xB0, 0x29, 0x42, 0x90, 0xC2, 0x25, 0x32, 0xAE, 0x47};
					//     0x17  0x18  0x19  0x1A  0x1B  0x1C  0x1D  0x1E  0x1F 
	...
	if(R840_Xtal==16000)
	{
		R840_RingCheckArray[20]=0x15;
		R840_RingCheckArray[21]=0x99;
		R840_RingCheckArray[22]=0x59;
		R840_RingCheckArray[24]=0x29;
	}
	else //24M
	{
		R840_RingCheckArray[20]=0x4D;
		R840_RingCheckArray[21]=0xBB;
		R840_RingCheckArray[22]=0x3B;
		R840_RingCheckArray[24]=0x21;
	}
	
	R840_I2C_Len.RegAddr = 0;
	R840_I2C_Len.Len = R840_REG_NUM;
	for(InitArrayCunt = 0; InitArrayCunt<R840_REG_NUM; InitArrayCunt ++)
	{
		R840_I2C_Len.Data[InitArrayCunt] = R840_RingCheckArray[InitArrayCunt];
		//R840_Array[InitArrayCunt] = R840_RingCheckArray[InitArrayCunt];
	}
	if(I2C_Write_Len(&R840_I2C_Len) != RT_Success)
		return RT_Fail;    
	...
	ADC_read = (R840_I2C_Len.Data[1] & 0x3F);
	if(ADC_read<=20)
		result = 0;
	else
		result = 1;
...
UINT8 R840_UL_TF_Check(void)
{
	...
	if(R840_Xtal==16000)  //16M	
	 ...
	 for(VGA_Count=0; VGA_Count < 16; VGA_Count ++)
	 {
		...
		if(VGA_Read > 20*ADC_READ_COUNT)
			break;
	 }

	//Mixer Amp = 15
	R840_I2C.RegAddr = 0x07;
	R840_Array[7] = (R840_Array[7] & 0xE0) | 15; 
	R840_I2C.Data = R840_Array[7];
	if(I2C_Write(&R840_I2C) != RT_Success)
		return RT_Fail;
	...
	//if VGA_Read<30, Mixer Amp_Fail
	if((VGA_Read<30*ADC_READ_COUNT))
		result = 5;  //Mixer fail
	else
		result = 0;
	...
	//test RF gain
	R840_I2C.RegAddr = 0x07;
	R840_Array[7] = (R840_Array[7] & 0xE0) | 0x00;  //mixer = 0
	R840_I2C.Data = R840_Array[7];
	if(I2C_Write(&R840_I2C) != RT_Success)
		return RT_Fail;

	R840_I2C.RegAddr = 0x1A;
	R840_Array[26] = (R840_Array[26] & 0xE0) | 0x1F;  //RF buf = 15
	R840_I2C.Data = R840_Array[26];
	if(I2C_Write(&R840_I2C) != RT_Success)
		return RT_Fail;
...
//	UINT8  u1MixerGain = 8;
	...
	//Mixer Amp Gain=8
	R840_I2C.RegAddr = 0x07;
	R840_Array[7] = (R840_Array[7] & 0xE0) | 8; 
	R840_I2C.Data = R840_Array[7];
	if(I2C_Write(&R840_I2C) != RT_Success)
		return RT_Fail;
	...
	IMR_Gain = IMR_POINT.Gain_X & 0x2F;
	IMR_Phase = IMR_POINT.Phase_Y & 0x2F;
...
UINT8 R840_IC_Check(void) //for CTMR testing
{
	...
	//IMR check
	CheckResult = R840_RingPll_Check();
	if(CheckResult!=0)  
	{
		return CheckResult;   //IMR fail or ADC too high
	}

	//IMR cal check
	if(R840_InitReg() != RT_Success)        //write initial reg before doing IMR Cal
	{
		CheckResult=4;   //I2C fail
		return CheckResult;
	}
	...
	CheckResult = R840_UL_TF_Check();
	if(CheckResult!=0)  
	{
		return CheckResult;  //TF fail or ADC too low or Gain fail
	}
...
UINT8 R840_Dump_Data(void) 
{
	...
	printf("LNA=%d, RF=%d, Mixer=%d\n", u1Gain1, u1Gain2, u1Gain3);
	printf("RF_flag=%d, Pulse_flag=%d\n", bRfFlag, bPulseFlag);

	for(i=0;i<32;i++)
	{
		printf("R%d=0x%X\n",i,R840_I2C_Len.Data[i]);
	}
	printf("\n");
