...
//===========Xtal Frequency===========//
 UINT16 R850_Xtal = 24000;					 //Xtal clock rate.
...
R850_SectType R850_IMR_Data[MAX_TUNER_NUM][R850_IMR_POINT_NUM] ={{{0,0,0,0}}};
...
//----------------------------------------------------------//
//------------extern interface;be defined in tun_r850.c ------------//
//----------------------------------------------------------//

extern R850_ErrCode I2C_Write_Len(UINT32 tuner_id, I2C_LEN_TYPE *I2C_Info);
extern R850_ErrCode I2C_Read_Len(UINT32 tuner_id, I2C_LEN_TYPE *I2C_Info);
extern R850_ErrCode I2C_Write(UINT32 tuner_id, I2C_TYPE *I2C_Info);
...
//                   Internal Functions                            //
//----------------------------------------------------------//
R850_ErrCode R850_Xtal_Check(R850_TUNER_NUM R850_Tuner_Num);
R850_ErrCode R850_InitReg(R850_TUNER_NUM R850_Tuner_Num);
R850_ErrCode R850_Cal_Prepare(R850_TUNER_NUM R850_Tuner_Num, UINT8 u1CalFlag);
R850_ErrCode R850_IMR(R850_TUNER_NUM R850_Tuner_Num, UINT8 IMR_MEM, UINT8 IM_Flag, UINT8 Rev_Mode);
R850_ErrCode R850_PLL(R850_TUNER_NUM R850_Tuner_Num, UINT32 LO_Freq, R850_Standard_Type R850_Standard);
R850_ErrCode R850_MUX(R850_TUNER_NUM R850_Tuner_Num, UINT32 LO_KHz, UINT32 RF_KHz, R850_Standard_Type R850_Standard);
R850_ErrCode R850_IQ(R850_TUNER_NUM R850_Tuner_Num, R850_SectType* IQ_Pont);
R850_ErrCode R850_IQ_Tree(R850_TUNER_NUM R850_Tuner_Num, UINT8 FixPot, UINT8 FlucPot, UINT8 PotReg, R850_SectType* CompareTree);
R850_ErrCode R850_CompreCor(R850_SectType* CorArry);
R850_ErrCode R850_CompreStep(R850_TUNER_NUM R850_Tuner_Num, R850_SectType* StepArry, UINT8 Pace);
R850_ErrCode R850_Muti_Read(R850_TUNER_NUM R850_Tuner_Num, UINT8* IMR_Result_Data);
R850_ErrCode R850_Section(R850_TUNER_NUM R850_Tuner_Num, R850_SectType* SectionArry);
R850_ErrCode R850_F_IMR(R850_TUNER_NUM R850_Tuner_Num, R850_SectType* IQ_Pont);
R850_ErrCode R850_IMR_Cross(R850_TUNER_NUM R850_Tuner_Num, R850_SectType* IQ_Pont, UINT8* X_Direct);
R850_ErrCode R850_IMR_Iqcap(R850_TUNER_NUM R850_Tuner_Num, R850_SectType* IQ_Point);   
R850_ErrCode R850_SetStandard(R850_TUNER_NUM R850_Tuner_Num, R850_Standard_Type RT_Standard);
R850_ErrCode R850_SetFrequency(R850_TUNER_NUM R850_Tuner_Num, R850_Set_Info R850_INFO);
...
UINT8 R850_Filt_Cal_ADC(R850_TUNER_NUM R850_Tuner_Num, UINT32 IF_Freq, UINT8 R850_BW, UINT8 FilCal_Gap);
...
R850_Sys_Info_Type R850_Sys_Sel(R850_Standard_Type R850_Standard)
{
	...
	case R850_ISDB_T_IF_5M: 
		R850_Sys_Info.IF_KHz=5000;
		R850_Sys_Info.BW=1;                      //BW=7M   R23[6:5] [8M(0), 7M(1), 6M(2)]
	//joey, 20160112, for ISDB-T CNR optimization.
		//R850_Sys_Info.FILT_CAL_IF=7780;          //CAL IF  
		R850_Sys_Info.FILT_CAL_IF=7880;          // Enlarge around 100K.
		//R850_Sys_Info.HPF_COR=3;                 //R24[7:4] [0~15 ; input: "0~15"]
		R850_Sys_Info.HPF_COR=5;                 // Change from "3" to "5".
		if(R850_Chip == R850_MP)
...
R850_ErrCode R850_Init(R850_TUNER_NUM R850_Tuner_Num, R850_Standard_Type R850_Standard)
{
	...
	//R850_Sys_Info_Type R850_Sys_Init;//warning: variable 'R850_Sys_Init' set but not used

	//R850_Sys_Init = R850_Sys_Sel (R850_Standard);	//Get R850_Mixer_Mode by different standard //warning: variable 'R850_Sys_Init' set but not used
	...
	while(check_i2c_count < 8)
	{
		...
		if(I2C_Read_Len(R850_Tuner_Num, &R850_I2C_Len) == RT_Success)
		{
...
R850_ErrCode R850_InitReg(R850_TUNER_NUM R850_Tuner_Num)//ok
{
	...
	if(I2C_Write_Len(R850_Tuner_Num, &R850_I2C_Len) != RT_Success)
		return RT_Fail;
...
R850_ErrCode R850_Xtal_Check(R850_TUNER_NUM R850_Tuner_Num)
{
	...
	if(I2C_Write(R850_Tuner_Num,&R850_I2C) != RT_Success)
		return RT_Fail;
/*
...
*/
	...
	for(i=0; i<XTAL_CHECK_SIZE; i++) //0~3
	{
		...
		R850_Delay_MS(R850_Tuner_Num, XTAL_CHK_DELAY);  //10 or 20ms??

		R850_I2C_Len.RegAddr = 0x00;
		R850_I2C_Len.Len = 3;
		I2C_Read_Len(R850_Tuner_Num, &R850_I2C_Len);
...
R850_ErrCode R850_PLL(R850_TUNER_NUM R850_Tuner_Num, UINT32 LO_Freq, R850_Standard_Type R850_Standard)//ok
{
	UINT8  MixDiv = 2;
	UINT8  DivBuf = 0;
	UINT8  Ni = 0;
	...
	{
		u2XalDivJudge = (UINT16) ((LO_Freq+R850_IF_GOLOBAL)/1000/13.5);
		//  48, 120 ,264 ,288, 336
		if((u2XalDivJudge==4)||(u2XalDivJudge==10)||(u2XalDivJudge==22)||(u2XalDivJudge==24)||(u2XalDivJudge==28))
		{
			CP_OFFSET = 0x02;  //30u,     R37[1]=1
	...
	if(R850_XtalDiv == XTAL_DIV1)
		R850_Delay_MS(R850_Tuner_Num, PLL_LOCK_DELAY);
	else if((R850_XtalDiv == XTAL_DIV1_2) || (R850_XtalDiv == XTAL_DIV2_1))
	    R850_Delay_MS(R850_Tuner_Num, PLL_LOCK_DELAY*2);
	else
	    R850_Delay_MS(R850_Tuner_Num, PLL_LOCK_DELAY*4);  
...
// output: TRUE or FALSE
//-----------------------------------------------------------------------------------/
R850_ErrCode R850_Muti_Read(R850_TUNER_NUM R850_Tuner_Num, UINT8* IMR_Result_Data) //ok
{
	
	R850_Delay_MS(R850_Tuner_Num, ADC_READ_DELAY);//2
	R850_I2C_Len.RegAddr = 0;
 ...
 //ok
R850_ErrCode R850_Section(R850_TUNER_NUM R850_Tuner_Num, R850_SectType* IQ_Pont)
{
	R850_SectType Compare_IQ[3];
...
UINT8  R850_Filt_Cal_ADC(R850_TUNER_NUM R850_Tuner_Num, UINT32 IF_Freq, UINT8 R850_BW, UINT8 FilCal_Gap)
{
	 ...
	 for(LPF_Count=5; LPF_Count < 16; LPF_Count ++)  
	 {
		...
		R850_Delay_MS(R850_Tuner_Num, FILTER_GAIN_DELAY); //
	 ...
	 for(i=initial_cnt; i<3; i++)
	 {
		 ...
		 R850_Delay_MS(R850_Tuner_Num, FILTER_CODE_DELAY); //delay ms
	 ...
	 {		 
		 ...
		 R850_Delay_MS(R850_Tuner_Num, FILTER_GAIN_DELAY); //delay ms
...
R850_ErrCode R850_SetFrequency(R850_TUNER_NUM R850_Tuner_Num, R850_Set_Info R850_INFO) //Write Multi byte 
{
...
//Optimize value
	...
	if(I2C_Write(R850_Tuner_Num, &Dlg_I2C) != RT_Success)
		return RT_Fail;
	...
	Dlg_I2C_Len.Len = 2;
	if(I2C_Read_Len(R850_Tuner_Num, &Dlg_I2C_Len) != RT_Success) // read data length
	{
		I2C_Read_Len(R850_Tuner_Num, &Dlg_I2C_Len);
	}
