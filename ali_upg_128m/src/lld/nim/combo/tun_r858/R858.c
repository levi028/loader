...
08/10/2016     Vincent          v1.4        add DVBT/T2 J83B
*/
...
extern R858_ErrCode r858_i2c_write(UINT32 tuner_id, I2C_TYPE *I2C_Info);
extern R858_ErrCode r858_i2c_write_len(UINT32 tuner_id, I2C_LEN_TYPE *I2C_Info);
extern R858_ErrCode r858_i2c_read_len(UINT32 tuner_id, I2C_LEN_TYPE *I2C_Info);

#define R858_Delay_MS(R858_ExtTunerNum_Type, x_ms)	osal_task_sleep(x_ms)
#define I2C_Write		r858_i2c_write
#define I2C_Write_Len	r858_i2c_write_len
#define I2C_Read_Len	r858_i2c_read_len
...
UINT8 R858_iniArray[2][R858_REG_NUM] = {     //Tuner1/2
		...
		//  0x12  0x13  0x14  0x15  0x16  0x17  0x18  0x19  0x1A  0x1B 
			0x00, 0xE1, 0x5A, 0x38, 0xDB, 0x87, 0x3D, 0x49, 0x39, 0x49,
		//  0x1C  0x1D  0x1E  0x1F  0x20  0x21  0x22  0x23  0x24  0x25
			0x45, 0x04, 0xA5, 0x2A, 0x0B, 0xE8, 0xD8, 0x00, 0xD2, 0x80,
		//  0x26  0x27  0x28  0x29  0x2A  0x2B  0x2C  0x2D  0x2E  0x2F
			0x22, 0x02, 0x00, 0x57, 0xA5, 0xF6, 0x77, 0x1A},
		...
		//  0x12  0x13  0x14  0x15  0x16  0x17  0x18  0x19  0x1A  0x1B  
			0x00, 0xE1, 0x5A, 0x38, 0xD8, 0x87, 0x3D, 0x49, 0x39, 0x49,
		//  0x1C  0x1D  0x1E  0x1F  0x20  0x21  0x22  0x23  0x24  0x25
			0x45, 0x04, 0xA5, 0x2A, 0x0B, 0xE8, 0xD8, 0x00, 0xD2, 0x80,
		//  0x26  0x27  0x28  0x29  0x2A  0x2B  0x2C  0x2D  0x2E  0x2F
...
UINT8   R858_ADDRESS[R858_MAX_NUM][2] = {{0xD4, 0xF4},{0x14, 0x34}};   
...
R858_SectType R858_IMR_Data[R858_MAX_NUM][2][R842_IMR_POINT_NUM] = {{{{0,0,0,0}}}};
...
*/
	...
	case R858_J83B_IF_4300:  
		...
		R858_Sys_Info.BW=0;                       //BW=7M   R17[5:4] [8M(0), 7M(1), 6M(2)]
		R858_Sys_Info.FILT_CAL_IF=6950;          //CAL IF  
	...
	case R858_J83B_IF_4000:  
		...
		R858_Sys_Info.BW=0;                       //BW=7M   R17[5:4] [8M(0), 7M(1), 6M(2)]
		R858_Sys_Info.FILT_CAL_IF=6650;          //CAL IF  
	...
	if((R858_Standard==R858_J83B) || (R858_Standard==R858_J83B_IF_5M) ||(R858_Standard==R858_J83B_IF_5500) || (R858_Standard==R858_J83B_IF_4000) || (R858_Standard==R858_J83B_IF_4300))  //J83B
...
*/
	...
	case R858_DVB_T2_1_7M_IF_5M:
		...
		//AGC
		R858_SysFreq_Info.AGC_CLK_CRTL = 0;				//R8[4]    ["on" (0), "off" (1)] v 
		...
		if(RF_freq>=650000 && RF_freq < 800000 )
		...
		R858_SysFreq_Info.DET_BW_LPF = 0;               //R34[0]       ["normal" (0), "enhance" (1)] v
		R858_SysFreq_Info.DET_BW_SEL = 0;               //R34[3]       ["NrbBW by manual" (0), "NrbBW ctrl by img_flag" (1)] v
		break;
	...
	case R858_DVB_C_6M_IF_5M:	
		...
		R858_SysFreq_Info.DET_BW_LPF = 1;               //R34[0]       ["normal" (0), "enhance" (1)] v
		R858_SysFreq_Info.DET_BW_SEL = 1;               //R34[3]       ["NrbBW by manual" (0), "NrbBW ctrl by img_flag" (1)] v
		break;
	case R858_J83B_IF_5M:	
	case R858_J83B_IF_5500:
	case R858_J83B_IF_4300:
	...
	case R858_ISDB_T_IF_5M:	
		if(RF_freq<=340000)
		{
			...
			R858_SysFreq_Info.RF_VTL_H=0x49;				//R31[7:0] RF VTL/H = 0.64(3) / 1.34(A)
		...
		R858_SysFreq_Info.LNA_TOP=1;					//R45[2:0] ["7~0" (0~7) ; input: "7~0"] ; co-use ; tuner 1 control
		R858_SysFreq_Info.LNA_VTL_H=0x49;				//R30[7:0] LNA VTL/H=0.64(3) / 1.14(8)
		R858_SysFreq_Info.RF_LTE_PSG=1;					//R32[3]  ["no psg" (0), "7.5dB(5~8)" (1)]
...
R858_ErrCode R858_PLL(R858_ExtTunerNum_Type R858_Num, R858_IntTunerNum_Type IntTunerNum, UINT32 LO_Freq, R858_Standard_Type R858_Standard)
{
	...
	UINT16 SDM8to1 = 0;
	UINT8   XTAL_POW = 0;
	UINT32  u2XalDivJudge;
	UINT8   CP_CUR = 0x00;
	UINT8	CP_CPI = 0x00;
	UINT8   CP_OFFSET = 0x00;
	UINT8   SDM_RES = 0x00;
	//UINT8   NS_RES = 0x00;   //warning: variable 'NS_RES' set but not used
	...
	*/
	...
	if(R858_Num ==0)
	{
		...
		//Xtal_cap resol
		R858_I2C.RegAddr = 29;
		R858_Array[R858_Num][IntTunerNum][R858_I2C.RegAddr] = (R858_Array[R858_Num][IntTunerNum][R858_I2C.RegAddr] & 0x86) | ((XTAL_CAP_RESOL_1 & 0x1E)<<2) | (XTAL_CAP_RESOL_1 & 0x01);//R29[6:3] R29[0]
		R858_I2C.Data = R858_Array[R858_Num][IntTunerNum][R858_I2C.RegAddr];
		if(I2C_Write(R858_Num, &R858_I2C) != RT_Success)
			return RT_Fail;
...
*/
	...
	{	if(IntTunerNum == 0) //Tuner1
		{
			if((u2XalDivJudge>=48000) && (u2XalDivJudge<114000) && (u2XalDivJudge!=96000))
			{
		...
		{
			if(((u2XalDivJudge>=48000) && (u2XalDivJudge<114000)) || (u2XalDivJudge==216000) || (u2XalDivJudge==360000) || (u2XalDivJudge==384000) || (u2XalDivJudge==408000) || (u2XalDivJudge==432000))
			{
				CP_OFFSET = 0x02;  //on,     R38[1]=1
			}
			else
	...
	if(R858_Xtal == 24000)
	{
		if(IntTunerNum == 0) //Tuner1
		{
			if((u2XalDivJudge>=48000) && (u2XalDivJudge<114000) && (u2XalDivJudge!=72000) )
			{
		...
		{
			if(((u2XalDivJudge>=48000) && (u2XalDivJudge<114000) && (u2XalDivJudge!=72000) ) || (u2XalDivJudge==360000) || (u2XalDivJudge==384000) || (u2XalDivJudge==432000))
			{
				CP_CPI = 0x01;  //on,		R27[5]=1
			}
			else
	...
	if(R858_Xtal == 24000)
	{
		...
		if(IntTunerNum == 0) //Tuner1
		{
			if((u2XalDivJudge>=48000) && (u2XalDivJudge<114000) && (u2XalDivJudge!=72000) ) //48 to 198MHz
			{
				CP_CUR = 0x00;  //0.7mA,		R27[2:0]=0
			}
			else if(u2XalDivJudge==72000)
			{
		...
		else //tuner2
		{
			...
			{
				CP_CUR = 0x04;  //0.3mA,		R27[2:0]=0
			}
			else if(u2XalDivJudge==72000)
			{
				CP_CUR = 0x05;  //0.2mA,		R27[2:0]=5
			}
			else if(u2XalDivJudge==216000 || u2XalDivJudge==360000)
			{
				CP_CUR = 0x03;  //0.4mA,		R27[2:0]=3
			}
			else if((u2XalDivJudge==384000) || (u2XalDivJudge==408000) || (u2XalDivJudge==432000))
			{
	...
	//RF_TEMP = LO_Freq + 5000;//warning: variable 'RF_TEMP' set but not used
	...
	if(MixDiv <= 4)  //Div=2,4
	{
		...
		//BiasHf = 2;           //90u
		SDM_RES = 0;     //short  
		//NS_RES = 0;        //0R    warning: variable 'NS_RES' set but not used
	...
	else if(MixDiv == 8) 
	{
		...
		//BiasHf = 2;           //90u
		SDM_RES = 0;     //short
		//NS_RES = 1;        //800R   warning: variable 'NS_RES' set but not used
		//Hfdiv_cur = 2;	//90
	...
	else
	{
		...
		{		
			...
			if(IntTunerNum == 0) //Tuner1
			{
				...
				else if(((u2XalDivJudge>=114000) && (u2XalDivJudge<198000) ) || (u2XalDivJudge==72000) || (u2XalDivJudge==216000) || (u2XalDivJudge==360000))   // 114MHz to 198MHz
				{
...
R858_ErrCode R858_SetFrequency(R858_ExtTunerNum_Type R858_Num, R858_IntTunerNum_Type IntTunerNum, R858_Set_Info R858_INFO)
{
	...
	R858_I2C_Len.I2cAddr=R858_ADDRESS[R858_Num][IntTunerNum];
	 ...
	 // lna current
	if((R858_INFO.RF_KHz>=330000) && (R858_INFO.RF_KHz<810000))
	{
        R858_I2C.RegAddr = 51;
	...
	default:
	 ...
	 if((R858_INFO.RF_KHz>=620000) && (R858_INFO.RF_KHz<801000))
		SysFreq_Info1.FILT_3TH_LPF_GAIN = 3;
	else
		SysFreq_Info1.FILT_3TH_LPF_GAIN = 2;
	...
	//R9
	//R858_I2C.RegAddr = 8;
	//R858_Array[R858_Num][IntTunerNum][R858_I2C.RegAddr] = (R858_Array[R858_Num][IntTunerNum][R858_I2C.RegAddr] & 0xEF) | 0x10;
   ...
   /* R858_I2C.Data = R858_Array[R858_Num][IntTunerNum][R858_I2C.RegAddr];
	...
	//DET BW LPF[0]
	R858_I2C.RegAddr = 34;
	R858_Array[R858_Num][IntTunerNum][R858_I2C.RegAddr] = (R858_Array[R858_Num][IntTunerNum][R858_I2C.RegAddr] & 0xFE) | (SysFreq_Info1.DET_BW_LPF<<1);  
    //detBW_sel[3]
	R858_Array[R858_Num][IntTunerNum][R858_I2C.RegAddr] = (R858_Array[R858_Num][IntTunerNum][R858_I2C.RegAddr] & 0xF7) | (SysFreq_Info1.DET_BW_SEL<<1); 
    /*R858_I2C.Data = R858_Array[R858_Num][IntTunerNum][R858_I2C.RegAddr];
    if(I2C_Write(R858_Num, &R858_I2C) != RT_Success)
       return RT_Fail;*/
