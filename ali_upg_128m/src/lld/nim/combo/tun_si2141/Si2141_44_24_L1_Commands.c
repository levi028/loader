 ...
 ***********************************************************************************************************************/
unsigned char Si2141_44_24_convert_to_byte (const unsigned char* buffer, unsigned char shift, unsigned char mask) {
  unsigned int rspBuffer = *buffer;
  return ((rspBuffer >> shift) & mask);
}
unsigned long Si2141_44_24_convert_to_ulong(const unsigned char* buffer, unsigned char shift,unsigned long mask) {
  return ((( ( (unsigned long)buffer[0]) | ((unsigned long)buffer[1] << 8) | ((unsigned long)buffer[2]<<16) | ((unsigned long)buffer[3]<<24)) >> shift) & mask );
}
unsigned int  Si2141_44_24_convert_to_uint (const unsigned char* buffer, unsigned char shift,unsigned int mask) {
  return (( ( (unsigned int)buffer[0]) | (((unsigned int)buffer[1]) << 8) >> shift) & mask);
}
short         Si2141_44_24_convert_to_int  (const unsigned char* buffer, unsigned char shift,unsigned int mask) {
  return (( ( (unsigned int)buffer[0]) | (((unsigned int)buffer[1]) << 8) >> shift) & mask);
}
/***********************************************************************************************************************
  Si2141_44_24_CurrentResponseStatus function
  Use:        status checking function
              Used to fill the Si2141_44_24_COMMON_REPLY_struct members with the ptDataBuffer byte's bits
...
{
    api->status->tunint =     Si2141_44_24_convert_to_byte(&ptDataBuffer, 0, 0x01);
    api->status->atvint =     Si2141_44_24_convert_to_byte(&ptDataBuffer, 1, 0x01);
    api->status->dtvint =     Si2141_44_24_convert_to_byte(&ptDataBuffer, 2, 0x01);
    api->status->err    =     Si2141_44_24_convert_to_byte(&ptDataBuffer, 6, 0x01);
    api->status->cts    =     Si2141_44_24_convert_to_byte(&ptDataBuffer, 7, 0x01);
  return (api->status->err ? ERROR_Si2141_44_24_ERR : NO_Si2141_44_24_ERROR);
}

 /***********************************************************************************************************************
  Si2141_44_24_pollForResetState function
  Use:        ResetState checking function
              Used to check the status byte for the reset code (0xFE).
  Comments:   Only required for the Si2141_44_24_L1_POWER_DOWN_HW Command
...
unsigned char Si2141_44_24_pollForResetState (L1_Si2141_44_24_Context *api)
{
  ...
  while (system_time() - start_time <1000)  { /* wait a maximum of 1000ms */
    if (L0_ReadCommandBytes(api->i2c, 1, rspByteBuffer) != 1) {
      SiTRACE("Si2141_44_24_pollForResetState ERROR reading byte 0!\n");
      return ERROR_Si2141_44_24_POLLING_CTS;
    }
  ...
  SiTRACE("Si2141_44_24_pollForResetState ERROR status Timeout!\n");
  return ERROR_Si2141_44_24_CTS_TIMEOUT;
}
/***********************************************************************************************************************
  Si2141_44_24_GetStatus function
  Use:        POWER_UP checking function
              Used to check the status byte the reset code (0xFE) to drop.
  Comments:   Only required for the Si2141_44_24_L1_POWER_UP Command
...
unsigned char Si2141_44_24_GetStatus (L1_Si2141_44_24_Context *api)
{
    ...
    if (L0_ReadCommandBytes(api->i2c, 1, rspByteBuffer) != 1)
    {
      SiTRACE("Si2141_44_24_GetStatus ERROR reading byte 0!\n");
      return ERROR_Si2141_44_24_POLLING_CTS;
    }
...
/***********************************************************************************************************************
  Si2141_44_24_pollForCTS function
...
unsigned char Si2141_44_24_pollForCTS (L1_Si2141_44_24_Context *api)
{
  ...
  while (system_time() - start_time <1000)  { /* wait a maximum of 1000ms */
    if (L0_ReadCommandBytes(api->i2c, 1, rspByteBuffer) != 1) {
      SiTRACE("Si2141_44_24_pollForCTS ERROR reading byte 0!\n");
      return ERROR_Si2141_44_24_POLLING_CTS;
    }
  ...
  SiTRACE("Si2141_44_24_pollForCTS ERROR CTS Timeout!\n");
  return ERROR_Si2141_44_24_CTS_TIMEOUT;
}

/***********************************************************************************************************************
  Si2141_44_24_pollForResponse function
...
unsigned char Si2141_44_24_pollForResponse (L1_Si2141_44_24_Context *api, unsigned int nbBytes, unsigned char *pByteBuffer)
{
  ...
  while (system_time() - start_time <1000)  { /* wait a maximum of 1000ms */
    if ((unsigned int)L0_ReadCommandBytes(api->i2c, nbBytes, pByteBuffer) != nbBytes) {
      SiTRACE("Si2141_44_24_pollForResponse ERROR reading byte 0!\n");
    ...
    if (pByteBuffer[0] & 0x80)  {
     if (pByteBuffer[0] & 0x40)  {
        ...
        SiTRACE("Si2141_44_24 debug bytes 0x%02x 0x%02x 0x%02x 0x%02x 0x%02x 0x%02x 0x%02x\n", debugBuffer[0], debugBuffer[1], debugBuffer[2], debugBuffer[3], debugBuffer[4], debugBuffer[5], debugBuffer[6]);
      }
      return Si2141_44_24_CurrentResponseStatus(api, pByteBuffer[0]);
    }
  }

  SiTRACE("Si2141_44_24_pollForResponse ERROR CTS Timeout!\n");
  return ERROR_Si2141_44_24_CTS_TIMEOUT;
}

#ifdef    Si2141_44_24_AGC_OVERRIDE_CMD
/*---------------------------------------------------*/
/* Si2141_44_24_AGC_OVERRIDE COMMAND                         */
/*---------------------------------------------------*/
                                         ...
                                         unsigned char   force_top_gain)
{
    ...
    SiTRACE("Si2141_44_24 AGC_OVERRIDE ");
  #ifdef   DEBUG_RANGE_CHECK
    if ((force_max_gain > Si2141_44_24_AGC_OVERRIDE_CMD_FORCE_MAX_GAIN_MAX) ) {error_code++; SiTRACE("\nOut of range: ");}; SiTRACE("FORCE_MAX_GAIN %d ", force_max_gain );
    if ((force_top_gain > Si2141_44_24_AGC_OVERRIDE_CMD_FORCE_TOP_GAIN_MAX) ) {error_code++; SiTRACE("\nOut of range: ");}; SiTRACE("FORCE_TOP_GAIN %d ", force_top_gain );
    if (error_code) {
    ...
    SiTRACE("\n");
    cmdByteBuffer[0] = Si2141_44_24_AGC_OVERRIDE_CMD;
    cmdByteBuffer[1] = (unsigned char) ( ( force_max_gain & Si2141_44_24_AGC_OVERRIDE_CMD_FORCE_MAX_GAIN_MASK ) << Si2141_44_24_AGC_OVERRIDE_CMD_FORCE_MAX_GAIN_LSB|
                                         ( force_top_gain & Si2141_44_24_AGC_OVERRIDE_CMD_FORCE_TOP_GAIN_MASK ) << Si2141_44_24_AGC_OVERRIDE_CMD_FORCE_TOP_GAIN_LSB);
...
/*---------------------------------------------------*/
/* Si2141_44_24_CONFIG_CLOCKS COMMAND                       */
/*---------------------------------------------------*/
                                         ...
                                         unsigned char   en_xout)
{
    ...
    SiTRACE("Si2141_44_24 CONFIG_CLOCKS ");
  #ifdef   DEBUG_RANGE_CHECK
    if ((subcode    > Si2141_44_24_CONFIG_CLOCKS_CMD_SUBCODE_MAX   ) ) {error_code++; SiTRACE("\nOut of range: ");}; SiTRACE("SUBCODE %d "   , subcode    );
    if ((clock_mode > Si2141_44_24_CONFIG_CLOCKS_CMD_CLOCK_MODE_MAX)  || (clock_mode < Si2141_44_24_CONFIG_CLOCKS_CMD_CLOCK_MODE_MIN) ) {error_code++; SiTRACE("\nOut of range: ");}; SiTRACE("CLOCK_MODE %d ", clock_mode );
    if ((en_xout    > Si2141_44_24_CONFIG_CLOCKS_CMD_EN_XOUT_MAX   ) ) {error_code++; SiTRACE("\nOut of range: ");}; SiTRACE("EN_XOUT %d "   , en_xout    );
    if (error_code) {
      SiTRACE("%d out of range parameters\n", error_code);
      return ERROR_Si2141_44_24_PARAMETER_OUT_OF_RANGE;
    }
    ...
    SiTRACE("\n");
    cmdByteBuffer[0] = Si2141_44_24_CONFIG_CLOCKS_CMD;
    cmdByteBuffer[1] = (unsigned char) ( ( subcode    & Si2141_44_24_CONFIG_CLOCKS_CMD_SUBCODE_MASK    ) << Si2141_44_24_CONFIG_CLOCKS_CMD_SUBCODE_LSB   );
    cmdByteBuffer[2] = (unsigned char) ( ( clock_mode & Si2141_44_24_CONFIG_CLOCKS_CMD_CLOCK_MODE_MASK ) << Si2141_44_24_CONFIG_CLOCKS_CMD_CLOCK_MODE_LSB|
                                         ( en_xout    & Si2141_44_24_CONFIG_CLOCKS_CMD_EN_XOUT_MASK    ) << Si2141_44_24_CONFIG_CLOCKS_CMD_EN_XOUT_LSB   );
...
/*---------------------------------------------------*/
/* Si2141_44_24_CONFIG_I2C COMMAND                             */
/*---------------------------------------------------*/
                                         ...
                                         unsigned char   i2c_broadcast)
{
    ...
    SiTRACE("Si2141_44_24 CONFIG_I2C ");
  #ifdef   DEBUG_RANGE_CHECK
    if ((subcode       > Si2141_44_24_CONFIG_I2C_CMD_SUBCODE_MAX      )  || (subcode       < Si2141_44_24_CONFIG_I2C_CMD_SUBCODE_MIN      ) ) {error_code++; SiTRACE("\nOut of range: ");}; SiTRACE("SUBCODE %d "      , subcode       );
    if ((i2c_broadcast > Si2141_44_24_CONFIG_I2C_CMD_I2C_BROADCAST_MAX) ) {error_code++; SiTRACE("\nOut of range: ");}; SiTRACE("I2C_BROADCAST %d ", i2c_broadcast );
    if (error_code) {
      SiTRACE("%d out of range parameters\n", error_code);
      return ERROR_Si2141_44_24_PARAMETER_OUT_OF_RANGE;
    }
    ...
    SiTRACE("\n");
    cmdByteBuffer[0] = Si2141_44_24_CONFIG_I2C_CMD;
    cmdByteBuffer[1] = (unsigned char) ( ( subcode       & Si2141_44_24_CONFIG_I2C_CMD_SUBCODE_MASK       ) << Si2141_44_24_CONFIG_I2C_CMD_SUBCODE_LSB      );
    cmdByteBuffer[2] = (unsigned char) ( ( i2c_broadcast & Si2141_44_24_CONFIG_I2C_CMD_I2C_BROADCAST_MASK ) << Si2141_44_24_CONFIG_I2C_CMD_I2C_BROADCAST_LSB);
...
/*---------------------------------------------------*/
/* Si2141_44_24_CONFIG_PINS COMMAND                           */
/*---------------------------------------------------*/
                                         ...
                                         unsigned char   xout_mode)
{
    ...
    SiTRACE("Si2141_44_24 CONFIG_PINS ");
  #ifdef   DEBUG_RANGE_CHECK
    if ((gpio1_mode > Si2141_44_24_CONFIG_PINS_CMD_GPIO1_MODE_MAX) ) {error_code++; SiTRACE("\nOut of range: ");}; SiTRACE("GPIO1_MODE %d ", gpio1_mode );
    if ((gpio1_read > Si2141_44_24_CONFIG_PINS_CMD_GPIO1_READ_MAX) ) {error_code++; SiTRACE("\nOut of range: ");}; SiTRACE("GPIO1_READ %d ", gpio1_read );
    if ((gpio2_mode > Si2141_44_24_CONFIG_PINS_CMD_GPIO2_MODE_MAX) ) {error_code++; SiTRACE("\nOut of range: ");}; SiTRACE("GPIO2_MODE %d ", gpio2_mode );
    if ((gpio2_read > Si2141_44_24_CONFIG_PINS_CMD_GPIO2_READ_MAX) ) {error_code++; SiTRACE("\nOut of range: ");}; SiTRACE("GPIO2_READ %d ", gpio2_read );
    if ((agc1_mode  > Si2141_44_24_CONFIG_PINS_CMD_AGC1_MODE_MAX ) ) {error_code++; SiTRACE("\nOut of range: ");}; SiTRACE("AGC1_MODE %d " , agc1_mode  );
    if ((agc1_read  > Si2141_44_24_CONFIG_PINS_CMD_AGC1_READ_MAX ) ) {error_code++; SiTRACE("\nOut of range: ");}; SiTRACE("AGC1_READ %d " , agc1_read  );
    if ((agc2_mode  > Si2141_44_24_CONFIG_PINS_CMD_AGC2_MODE_MAX ) ) {error_code++; SiTRACE("\nOut of range: ");}; SiTRACE("AGC2_MODE %d " , agc2_mode  );
    if ((agc2_read  > Si2141_44_24_CONFIG_PINS_CMD_AGC2_READ_MAX ) ) {error_code++; SiTRACE("\nOut of range: ");}; SiTRACE("AGC2_READ %d " , agc2_read  );
    if ((xout_mode  > Si2141_44_24_CONFIG_PINS_CMD_XOUT_MODE_MAX ) ) {error_code++; SiTRACE("\nOut of range: ");}; SiTRACE("XOUT_MODE %d " , xout_mode  );
    if (error_code) {
      SiTRACE("%d out of range parameters\n", error_code);
      return ERROR_Si2141_44_24_PARAMETER_OUT_OF_RANGE;
    }
    ...
    SiTRACE("\n");
    cmdByteBuffer[0] = Si2141_44_24_CONFIG_PINS_CMD;
    cmdByteBuffer[1] = (unsigned char) ( ( gpio1_mode & Si2141_44_24_CONFIG_PINS_CMD_GPIO1_MODE_MASK ) << Si2141_44_24_CONFIG_PINS_CMD_GPIO1_MODE_LSB|
                                         ( gpio1_read & Si2141_44_24_CONFIG_PINS_CMD_GPIO1_READ_MASK ) << Si2141_44_24_CONFIG_PINS_CMD_GPIO1_READ_LSB);
    cmdByteBuffer[2] = (unsigned char) ( ( gpio2_mode & Si2141_44_24_CONFIG_PINS_CMD_GPIO2_MODE_MASK ) << Si2141_44_24_CONFIG_PINS_CMD_GPIO2_MODE_LSB|
                                         ( gpio2_read & Si2141_44_24_CONFIG_PINS_CMD_GPIO2_READ_MASK ) << Si2141_44_24_CONFIG_PINS_CMD_GPIO2_READ_LSB);
    cmdByteBuffer[3] = (unsigned char) ( ( agc1_mode  & Si2141_44_24_CONFIG_PINS_CMD_AGC1_MODE_MASK  ) << Si2141_44_24_CONFIG_PINS_CMD_AGC1_MODE_LSB |
                                         ( agc1_read  & Si2141_44_24_CONFIG_PINS_CMD_AGC1_READ_MASK  ) << Si2141_44_24_CONFIG_PINS_CMD_AGC1_READ_LSB );
    cmdByteBuffer[4] = (unsigned char)0x00;
    cmdByteBuffer[5] = (unsigned char) ( ( agc2_mode  & Si2141_44_24_CONFIG_PINS_CMD_AGC2_MODE_MASK  ) << Si2141_44_24_CONFIG_PINS_CMD_AGC2_MODE_LSB |
                                         ( agc2_read  & Si2141_44_24_CONFIG_PINS_CMD_AGC2_READ_MASK  ) << Si2141_44_24_CONFIG_PINS_CMD_AGC2_READ_LSB );
    cmdByteBuffer[6] = (unsigned char) ( ( xout_mode  & Si2141_44_24_CONFIG_PINS_CMD_XOUT_MODE_MASK  ) << Si2141_44_24_CONFIG_PINS_CMD_XOUT_MODE_LSB );
    ...
    error_code = Si2141_44_24_pollForResponse(api, 7, rspByteBuffer);
    if (error_code) {
      SiTRACE("Error polling CONFIG_PINS response\n");
      return error_code;
    }

    api->rsp->config_pins.gpio1_mode  = Si2141_44_24_convert_to_byte  (&rspByteBuffer[ 1] , Si2141_44_24_CONFIG_PINS_RESPONSE_GPIO1_MODE_LSB   , Si2141_44_24_CONFIG_PINS_RESPONSE_GPIO1_MODE_MASK  );
    api->rsp->config_pins.gpio1_state = Si2141_44_24_convert_to_byte  (&rspByteBuffer[ 1] , Si2141_44_24_CONFIG_PINS_RESPONSE_GPIO1_STATE_LSB  , Si2141_44_24_CONFIG_PINS_RESPONSE_GPIO1_STATE_MASK );
    api->rsp->config_pins.gpio2_mode  = Si2141_44_24_convert_to_byte  (&rspByteBuffer[ 2] , Si2141_44_24_CONFIG_PINS_RESPONSE_GPIO2_MODE_LSB   , Si2141_44_24_CONFIG_PINS_RESPONSE_GPIO2_MODE_MASK  );
    api->rsp->config_pins.gpio2_state = Si2141_44_24_convert_to_byte  (&rspByteBuffer[ 2] , Si2141_44_24_CONFIG_PINS_RESPONSE_GPIO2_STATE_LSB  , Si2141_44_24_CONFIG_PINS_RESPONSE_GPIO2_STATE_MASK );
    api->rsp->config_pins.agc1_mode   = Si2141_44_24_convert_to_byte  (&rspByteBuffer[ 3] , Si2141_44_24_CONFIG_PINS_RESPONSE_AGC1_MODE_LSB    , Si2141_44_24_CONFIG_PINS_RESPONSE_AGC1_MODE_MASK   );
    api->rsp->config_pins.agc1_state  = Si2141_44_24_convert_to_byte  (&rspByteBuffer[ 3] , Si2141_44_24_CONFIG_PINS_RESPONSE_AGC1_STATE_LSB   , Si2141_44_24_CONFIG_PINS_RESPONSE_AGC1_STATE_MASK  );
    api->rsp->config_pins.agc2_mode   = Si2141_44_24_convert_to_byte  (&rspByteBuffer[ 5] , Si2141_44_24_CONFIG_PINS_RESPONSE_AGC2_MODE_LSB    , Si2141_44_24_CONFIG_PINS_RESPONSE_AGC2_MODE_MASK   );
    api->rsp->config_pins.agc2_state  = Si2141_44_24_convert_to_byte  (&rspByteBuffer[ 5] , Si2141_44_24_CONFIG_PINS_RESPONSE_AGC2_STATE_LSB   , Si2141_44_24_CONFIG_PINS_RESPONSE_AGC2_STATE_MASK  );
    api->rsp->config_pins.xout_mode   = Si2141_44_24_convert_to_byte  (&rspByteBuffer[ 6] , Si2141_44_24_CONFIG_PINS_RESPONSE_XOUT_MODE_LSB    , Si2141_44_24_CONFIG_PINS_RESPONSE_XOUT_MODE_MASK   );
    api->rsp->config_pins.xout_state  = Si2141_44_24_convert_to_byte  (&rspByteBuffer[ 6] , Si2141_44_24_CONFIG_PINS_RESPONSE_XOUT_STATE_LSB   , Si2141_44_24_CONFIG_PINS_RESPONSE_XOUT_STATE_MASK  );
...
/*---------------------------------------------------*/
/* Si2141_44_24_EXIT_BOOTLOADER COMMAND                   */
/*---------------------------------------------------*/
                                         ...
                                         unsigned char   ctsien)
{
    ...
    SiTRACE("Si2141_44_24 EXIT_BOOTLOADER ");
  #ifdef   DEBUG_RANGE_CHECK
    if ((func   > Si2141_44_24_EXIT_BOOTLOADER_CMD_FUNC_MAX  )  || (func   < Si2141_44_24_EXIT_BOOTLOADER_CMD_FUNC_MIN  ) ) {error_code++; SiTRACE("\nOut of range: ");}; SiTRACE("FUNC %d "  , func   );
    if ((ctsien > Si2141_44_24_EXIT_BOOTLOADER_CMD_CTSIEN_MAX) ) {error_code++; SiTRACE("\nOut of range: ");}; SiTRACE("CTSIEN %d ", ctsien );
    if (error_code) {
      SiTRACE("%d out of range parameters\n", error_code);
      return ERROR_Si2141_44_24_PARAMETER_OUT_OF_RANGE;
    }
    ...
    SiTRACE("\n");
    cmdByteBuffer[0] = Si2141_44_24_EXIT_BOOTLOADER_CMD;
    cmdByteBuffer[1] = (unsigned char) ( ( func   & Si2141_44_24_EXIT_BOOTLOADER_CMD_FUNC_MASK   ) << Si2141_44_24_EXIT_BOOTLOADER_CMD_FUNC_LSB  |
                                         ( ctsien & Si2141_44_24_EXIT_BOOTLOADER_CMD_CTSIEN_MASK ) << Si2141_44_24_EXIT_BOOTLOADER_CMD_CTSIEN_LSB);
...
/*---------------------------------------------------*/
/* Si2141_44_24_FINE_TUNE COMMAND                               */
/*---------------------------------------------------*/
                                                   ...
                                                   int   offset_500hz)
{
    ...
    SiTRACE("Si2141_44_24 FINE_TUNE ");
  #ifdef   DEBUG_RANGE_CHECK
    if ((persistence  > Si2141_44_24_FINE_TUNE_CMD_PERSISTENCE_MAX ) ) {error_code++; SiTRACE("\nOut of range: ");}; SiTRACE("PERSISTENCE %d " , persistence  );
    if ((offset_500hz > Si2141_44_24_FINE_TUNE_CMD_OFFSET_500HZ_MAX)  || (offset_500hz < Si2141_44_24_FINE_TUNE_CMD_OFFSET_500HZ_MIN) ) {error_code++; SiTRACE("\nOut of range: ");}; SiTRACE("OFFSET_500HZ %d ", offset_500hz );
    if (error_code) {
      SiTRACE("%d out of range parameters\n", error_code);
      return ERROR_Si2141_44_24_PARAMETER_OUT_OF_RANGE;
    }
    ...
    SiTRACE("\n");
    cmdByteBuffer[0] = Si2141_44_24_FINE_TUNE_CMD;
    cmdByteBuffer[1] = (unsigned char) ( ( persistence  & Si2141_44_24_FINE_TUNE_CMD_PERSISTENCE_MASK  ) << Si2141_44_24_FINE_TUNE_CMD_PERSISTENCE_LSB );
    cmdByteBuffer[2] = (unsigned char) ( ( offset_500hz & Si2141_44_24_FINE_TUNE_CMD_OFFSET_500HZ_MASK ) << Si2141_44_24_FINE_TUNE_CMD_OFFSET_500HZ_LSB);
    cmdByteBuffer[3] = (unsigned char) ((( offset_500hz & Si2141_44_24_FINE_TUNE_CMD_OFFSET_500HZ_MASK ) << Si2141_44_24_FINE_TUNE_CMD_OFFSET_500HZ_LSB)>>8);
...
/*---------------------------------------------------*/
/* Si2141_44_24_GET_PROPERTY COMMAND                         */
/*---------------------------------------------------*/
                                         ...
                                         unsigned int    prop)
{
    ...
    SiTRACE("Si2141_44_24 GET_PROPERTY ");
  #ifdef   DEBUG_RANGE_CHECK
    if ((reserved > Si2141_44_24_GET_PROPERTY_CMD_RESERVED_MAX) ) {error_code++; SiTRACE("\nOut of range: ");}; SiTRACE("RESERVED %d ", reserved );
    SiTRACE("PROP %d "    , prop     );
    if (error_code) {
      SiTRACE("%d out of range parameters\n", error_code);
      return ERROR_Si2141_44_24_PARAMETER_OUT_OF_RANGE;
    ...
    SiTRACE("\n");
    cmdByteBuffer[0] = Si2141_44_24_GET_PROPERTY_CMD;
    cmdByteBuffer[1] = (unsigned char) ( ( reserved & Si2141_44_24_GET_PROPERTY_CMD_RESERVED_MASK ) << Si2141_44_24_GET_PROPERTY_CMD_RESERVED_LSB);
    cmdByteBuffer[2] = (unsigned char) ( ( prop     & Si2141_44_24_GET_PROPERTY_CMD_PROP_MASK     ) << Si2141_44_24_GET_PROPERTY_CMD_PROP_LSB    );
    cmdByteBuffer[3] = (unsigned char) ((( prop     & Si2141_44_24_GET_PROPERTY_CMD_PROP_MASK     ) << Si2141_44_24_GET_PROPERTY_CMD_PROP_LSB    )>>8);
    ...
    error_code = Si2141_44_24_pollForResponse(api, 4, rspByteBuffer);
    if (error_code) {
      SiTRACE("Error polling GET_PROPERTY response\n");
      return error_code;
    }

    api->rsp->get_property.reserved = Si2141_44_24_convert_to_byte  (&rspByteBuffer[ 1] , Si2141_44_24_GET_PROPERTY_RESPONSE_RESERVED_LSB  , Si2141_44_24_GET_PROPERTY_RESPONSE_RESERVED_MASK );
    api->rsp->get_property.data     = Si2141_44_24_convert_to_uint  (&rspByteBuffer[ 2] , Si2141_44_24_GET_PROPERTY_RESPONSE_DATA_LSB      , Si2141_44_24_GET_PROPERTY_RESPONSE_DATA_MASK     );
...
unsigned char Si2141_44_24_L1_GET_REV         (L1_Si2141_44_24_Context *api)
{
    ...
    SiTRACE("Si2141_44_24 GET_REV ");
    SiTRACE("\n");
    cmdByteBuffer[0] = Si2141_44_24_GET_REV_CMD;
    ...
    error_code = Si2141_44_24_pollForResponse(api, 13, rspByteBuffer);
    if (error_code) {
      SiTRACE("Error polling GET_REV response\n");
      return error_code;
    }

    api->rsp->get_rev.pn       = Si2141_44_24_convert_to_byte  (&rspByteBuffer[ 1] , Si2141_44_24_GET_REV_RESPONSE_PN_LSB        , Si2141_44_24_GET_REV_RESPONSE_PN_MASK       );
    api->rsp->get_rev.fwmajor  = Si2141_44_24_convert_to_byte  (&rspByteBuffer[ 2] , Si2141_44_24_GET_REV_RESPONSE_FWMAJOR_LSB   , Si2141_44_24_GET_REV_RESPONSE_FWMAJOR_MASK  );
    api->rsp->get_rev.fwminor  = Si2141_44_24_convert_to_byte  (&rspByteBuffer[ 3] , Si2141_44_24_GET_REV_RESPONSE_FWMINOR_LSB   , Si2141_44_24_GET_REV_RESPONSE_FWMINOR_MASK  );
    api->rsp->get_rev.patch    = Si2141_44_24_convert_to_uint  (&rspByteBuffer[ 4] , Si2141_44_24_GET_REV_RESPONSE_PATCH_LSB     , Si2141_44_24_GET_REV_RESPONSE_PATCH_MASK    );
    api->rsp->get_rev.cmpmajor = Si2141_44_24_convert_to_byte  (&rspByteBuffer[ 6] , Si2141_44_24_GET_REV_RESPONSE_CMPMAJOR_LSB  , Si2141_44_24_GET_REV_RESPONSE_CMPMAJOR_MASK );
    api->rsp->get_rev.cmpminor = Si2141_44_24_convert_to_byte  (&rspByteBuffer[ 7] , Si2141_44_24_GET_REV_RESPONSE_CMPMINOR_LSB  , Si2141_44_24_GET_REV_RESPONSE_CMPMINOR_MASK );
    api->rsp->get_rev.cmpbuild = Si2141_44_24_convert_to_byte  (&rspByteBuffer[ 8] , Si2141_44_24_GET_REV_RESPONSE_CMPBUILD_LSB  , Si2141_44_24_GET_REV_RESPONSE_CMPBUILD_MASK );
    api->rsp->get_rev.chiprev  = Si2141_44_24_convert_to_byte  (&rspByteBuffer[ 9] , Si2141_44_24_GET_REV_RESPONSE_CHIPREV_LSB   , Si2141_44_24_GET_REV_RESPONSE_CHIPREV_MASK  );
    api->rsp->get_rev.pn2      = Si2141_44_24_convert_to_byte  (&rspByteBuffer[12] , Si2141_44_24_GET_REV_RESPONSE_PN2_LSB       , Si2141_44_24_GET_REV_RESPONSE_PN2_MASK      );
...
/*---------------------------------------------------*/
/* Si2141_44_24_PART_INFO COMMAND                               */
/*---------------------------------------------------*/
unsigned char Si2141_44_24_L1_PART_INFO       (L1_Si2141_44_24_Context *api)
{
    ...
    SiTRACE("Si2141_44_24 PART_INFO ");
    SiTRACE("\n");
    cmdByteBuffer[0] = Si2141_44_24_PART_INFO_CMD;
    ...
    error_code = Si2141_44_24_pollForResponse(api, 14, rspByteBuffer);
    if (error_code) {
      SiTRACE("Error polling PART_INFO response\n");
      return error_code;
    }

    api->rsp->part_info.chiprev  = Si2141_44_24_convert_to_byte  (&rspByteBuffer[ 1] , Si2141_44_24_PART_INFO_RESPONSE_CHIPREV_LSB   , Si2141_44_24_PART_INFO_RESPONSE_CHIPREV_MASK  );
    api->rsp->part_info.part     = Si2141_44_24_convert_to_byte  (&rspByteBuffer[ 2] , Si2141_44_24_PART_INFO_RESPONSE_PART_LSB      , Si2141_44_24_PART_INFO_RESPONSE_PART_MASK     );
    api->rsp->part_info.pmajor   = Si2141_44_24_convert_to_byte  (&rspByteBuffer[ 3] , Si2141_44_24_PART_INFO_RESPONSE_PMAJOR_LSB    , Si2141_44_24_PART_INFO_RESPONSE_PMAJOR_MASK   );
    api->rsp->part_info.pminor   = Si2141_44_24_convert_to_byte  (&rspByteBuffer[ 4] , Si2141_44_24_PART_INFO_RESPONSE_PMINOR_LSB    , Si2141_44_24_PART_INFO_RESPONSE_PMINOR_MASK   );
    api->rsp->part_info.pbuild   = Si2141_44_24_convert_to_byte  (&rspByteBuffer[ 5] , Si2141_44_24_PART_INFO_RESPONSE_PBUILD_LSB    , Si2141_44_24_PART_INFO_RESPONSE_PBUILD_MASK   );
    api->rsp->part_info.reserved = Si2141_44_24_convert_to_uint  (&rspByteBuffer[ 6] , Si2141_44_24_PART_INFO_RESPONSE_RESERVED_LSB  , Si2141_44_24_PART_INFO_RESPONSE_RESERVED_MASK );
    api->rsp->part_info.serial   = Si2141_44_24_convert_to_ulong (&rspByteBuffer[ 8] , Si2141_44_24_PART_INFO_RESPONSE_SERIAL_LSB    , Si2141_44_24_PART_INFO_RESPONSE_SERIAL_MASK   );
    api->rsp->part_info.romid    = Si2141_44_24_convert_to_byte  (&rspByteBuffer[12] , Si2141_44_24_PART_INFO_RESPONSE_ROMID_LSB     , Si2141_44_24_PART_INFO_RESPONSE_ROMID_MASK    );
    api->rsp->part_info.part2    = Si2141_44_24_convert_to_byte  (&rspByteBuffer[13] , Si2141_44_24_PART_INFO_RESPONSE_PART2_LSB     , Si2141_44_24_PART_INFO_RESPONSE_PART2_MASK    );
...
/*---------------------------------------------------*/
/* Si2141_44_24_PART_INFO2 COMMAND                             */
/*---------------------------------------------------*/
unsigned char Si2141_44_24_L1_PART_INFO2      (L1_Si2141_44_24_Context *api)
{
    ...
    SiTRACE("Si2141_44_24 PART_INFO2 ");
    SiTRACE("\n");
    cmdByteBuffer[0] = Si2141_44_24_PART_INFO2_CMD;
    ...
    error_code = Si2141_44_24_pollForResponse(api, 8, rspByteBuffer);
    if (error_code) {
      SiTRACE("Error polling PART_INFO2 response\n");
      return error_code;
    }

    api->rsp->part_info2.romid    = Si2141_44_24_convert_to_byte  (&rspByteBuffer[ 1] , Si2141_44_24_PART_INFO2_RESPONSE_ROMID_LSB     , Si2141_44_24_PART_INFO2_RESPONSE_ROMID_MASK    );
    api->rsp->part_info2.part2    = Si2141_44_24_convert_to_byte  (&rspByteBuffer[ 2] , Si2141_44_24_PART_INFO2_RESPONSE_PART2_LSB     , Si2141_44_24_PART_INFO2_RESPONSE_PART2_MASK    );
    api->rsp->part_info2.reserved = Si2141_44_24_convert_to_byte  (&rspByteBuffer[ 3] , Si2141_44_24_PART_INFO2_RESPONSE_RESERVED_LSB  , Si2141_44_24_PART_INFO2_RESPONSE_RESERVED_MASK );
    api->rsp->part_info2.serial   = Si2141_44_24_convert_to_ulong (&rspByteBuffer[ 4] , Si2141_44_24_PART_INFO2_RESPONSE_SERIAL_LSB    , Si2141_44_24_PART_INFO2_RESPONSE_SERIAL_MASK   );
...
/*---------------------------------------------------*/
/* Si2141_44_24_POWER_DOWN_HW COMMAND                       */
/*---------------------------------------------------*/
                                         ...
                                         unsigned char   reserved9)
{
    ...
    SiTRACE("Si2141_44_24 POWER_DOWN_HW ");
  #ifdef   DEBUG_RANGE_CHECK
    if ((subcode   > Si2141_44_24_POWER_DOWN_HW_CMD_SUBCODE_MAX  ) ) {error_code++; SiTRACE("\nOut of range: ");}; SiTRACE("SUBCODE %d "  , subcode   );
    if ((pdb_xo_osc > Si2141_44_24_POWER_DOWN_HW_CMD_PDB_XO_OSC_MAX) ) {error_code++; SiTRACE("\nOut of range: ");}; SiTRACE("PDB_XO_OSC %d ", pdb_xo_osc );
    if ((reserved1 > Si2141_44_24_POWER_DOWN_HW_CMD_RESERVED1_MAX) ) {error_code++; SiTRACE("\nOut of range: ");}; SiTRACE("RESERVED1 %d ", reserved1 );
    if ((en_xout   > Si2141_44_24_POWER_DOWN_HW_CMD_EN_XOUT_MAX  ) ) {error_code++; SiTRACE("\nOut of range: ");}; SiTRACE("EN_XOUT %d "  , en_xout   );
    if ((reserved2 > Si2141_44_24_POWER_DOWN_HW_CMD_RESERVED2_MAX)  || (reserved2 < Si2141_44_24_POWER_DOWN_HW_CMD_RESERVED2_MIN) ) {error_code++; SiTRACE("\nOut of range: ");}; SiTRACE("RESERVED2 %d ", reserved2 );
    if ((reserved3 > Si2141_44_24_POWER_DOWN_HW_CMD_RESERVED3_MAX) ) {error_code++; SiTRACE("\nOut of range: ");}; SiTRACE("RESERVED3 %d ", reserved3 );
    if ((reserved4  > Si2141_44_24_POWER_DOWN_HW_CMD_RESERVED4_MAX ) ) {error_code++; SiTRACE("\nOut of range: ");}; SiTRACE("RESERVED4 %d " , reserved4  );
    if ((reserved5  > Si2141_44_24_POWER_DOWN_HW_CMD_RESERVED5_MAX ) ) {error_code++; SiTRACE("\nOut of range: ");}; SiTRACE("RESERVED5 %d " , reserved5  );
    if ((reserved6 > Si2141_44_24_POWER_DOWN_HW_CMD_RESERVED6_MAX) ) {error_code++; SiTRACE("\nOut of range: ");}; SiTRACE("RESERVED6 %d ", reserved6 );
    if ((reserved7 > Si2141_44_24_POWER_DOWN_HW_CMD_RESERVED7_MAX) ) {error_code++; SiTRACE("\nOut of range: ");}; SiTRACE("RESERVED7 %d ", reserved7 );
    if ((reserved8 > Si2141_44_24_POWER_DOWN_HW_CMD_RESERVED8_MAX) ) {error_code++; SiTRACE("\nOut of range: ");}; SiTRACE("RESERVED8 %d ", reserved8 );
    if ((reserved9  > Si2141_44_24_POWER_DOWN_HW_CMD_RESERVED9_MAX ) ) {error_code++; SiTRACE("\nOut of range: ");}; SiTRACE("RESERVED9 %d " , reserved9  );
    if (error_code) {
      SiTRACE("%d out of range parameters\n", error_code);
      return ERROR_Si2141_44_24_PARAMETER_OUT_OF_RANGE;
    }
    ...
    SiTRACE("\n");
    cmdByteBuffer[0] = Si2141_44_24_POWER_DOWN_HW_CMD;
    cmdByteBuffer[1] = (unsigned char) ( ( subcode   & Si2141_44_24_POWER_DOWN_HW_CMD_SUBCODE_MASK   ) << Si2141_44_24_POWER_DOWN_HW_CMD_SUBCODE_LSB  );
    cmdByteBuffer[2] = (unsigned char) ( ( pdb_xo_osc & Si2141_44_24_POWER_DOWN_HW_CMD_PDB_XO_OSC_MASK ) << Si2141_44_24_POWER_DOWN_HW_CMD_PDB_XO_OSC_LSB|
                                         ( reserved1 & Si2141_44_24_POWER_DOWN_HW_CMD_RESERVED1_MASK ) << Si2141_44_24_POWER_DOWN_HW_CMD_RESERVED1_LSB|
                                         ( en_xout   & Si2141_44_24_POWER_DOWN_HW_CMD_EN_XOUT_MASK   ) << Si2141_44_24_POWER_DOWN_HW_CMD_EN_XOUT_LSB  |
                                         ( reserved2 & Si2141_44_24_POWER_DOWN_HW_CMD_RESERVED2_MASK ) << Si2141_44_24_POWER_DOWN_HW_CMD_RESERVED2_LSB);
    cmdByteBuffer[3] = (unsigned char) ( ( reserved3  & Si2141_44_24_POWER_DOWN_HW_CMD_RESERVED3_MASK  ) << Si2141_44_24_POWER_DOWN_HW_CMD_RESERVED3_LSB );
    cmdByteBuffer[4] = (unsigned char) ( ( reserved4  & Si2141_44_24_POWER_DOWN_HW_CMD_RESERVED4_MASK  ) << Si2141_44_24_POWER_DOWN_HW_CMD_RESERVED4_LSB );
    cmdByteBuffer[5] = (unsigned char) ( ( reserved5  & Si2141_44_24_POWER_DOWN_HW_CMD_RESERVED5_MASK  ) << Si2141_44_24_POWER_DOWN_HW_CMD_RESERVED5_LSB );
    cmdByteBuffer[6] = (unsigned char) ( ( reserved6  & Si2141_44_24_POWER_DOWN_HW_CMD_RESERVED6_MASK  ) << Si2141_44_24_POWER_DOWN_HW_CMD_RESERVED6_LSB );
    cmdByteBuffer[7] = (unsigned char) ( ( reserved7  & Si2141_44_24_POWER_DOWN_HW_CMD_RESERVED7_MASK  ) << Si2141_44_24_POWER_DOWN_HW_CMD_RESERVED7_LSB );
    cmdByteBuffer[8] = (unsigned char) ( ( reserved8  & Si2141_44_24_POWER_DOWN_HW_CMD_RESERVED8_MASK  ) << Si2141_44_24_POWER_DOWN_HW_CMD_RESERVED8_LSB );
    cmdByteBuffer[9] = (unsigned char) ( ( reserved9  & Si2141_44_24_POWER_DOWN_HW_CMD_RESERVED9_MASK  ) << Si2141_44_24_POWER_DOWN_HW_CMD_RESERVED9_LSB );
    ...
    error_code = Si2141_44_24_pollForResetState(api);
    if (error_code) {
                                         ...
                                         unsigned char   reserved8)
{
    ...
    SiTRACE("Si2141_44_24 POWER_UP ");
  #ifdef   DEBUG_RANGE_CHECK
    if ((subcode     > Si2141_44_24_POWER_UP_CMD_SUBCODE_MAX    ) ) {error_code++; SiTRACE("\nOut of range: ");}; SiTRACE("SUBCODE %d "    , subcode     );
    if ((clock_mode  > Si2141_44_24_POWER_UP_CMD_CLOCK_MODE_MAX )  || (clock_mode  < Si2141_44_24_POWER_UP_CMD_CLOCK_MODE_MIN ) ) {error_code++; SiTRACE("\nOut of range: ");}; SiTRACE("CLOCK_MODE %d " , clock_mode  );
    if ((en_xout     > Si2141_44_24_POWER_UP_CMD_EN_XOUT_MAX    ) ) {error_code++; SiTRACE("\nOut of range: ");}; SiTRACE("EN_XOUT %d "    , en_xout     );
    if ((config_1p8v > Si2141_44_24_POWER_UP_CMD_CONFIG_1P8V_MAX)  || (config_1p8v < Si2141_44_24_POWER_UP_CMD_CONFIG_1P8V_MIN) ) {error_code++; SiTRACE("\nOut of range: ");}; SiTRACE("CONFIG_1P8V %d ", config_1p8v );
    if ((reserved3   > Si2141_44_24_POWER_UP_CMD_RESERVED3_MAX  ) ) {error_code++; SiTRACE("\nOut of range: ");}; SiTRACE("RESERVED3 %d "  , reserved3   );
    if ((reserved4   > Si2141_44_24_POWER_UP_CMD_RESERVED4_MAX  )  || (reserved4   < Si2141_44_24_POWER_UP_CMD_RESERVED4_MIN  ) ) {error_code++; SiTRACE("\nOut of range: ");}; SiTRACE("RESERVED4 %d "  , reserved4   );
    if ((reserved5   > Si2141_44_24_POWER_UP_CMD_RESERVED5_MAX  )  || (reserved5   < Si2141_44_24_POWER_UP_CMD_RESERVED5_MIN  ) ) {error_code++; SiTRACE("\nOut of range: ");}; SiTRACE("RESERVED5 %d "  , reserved5   );
    if ((reserved6   > Si2141_44_24_POWER_UP_CMD_RESERVED6_MAX  )  || (reserved6   < Si2141_44_24_POWER_UP_CMD_RESERVED6_MIN  ) ) {error_code++; SiTRACE("\nOut of range: ");}; SiTRACE("RESERVED6 %d "  , reserved6   );
    if ((reserved7   > Si2141_44_24_POWER_UP_CMD_RESERVED7_MAX  )  || (reserved7   < Si2141_44_24_POWER_UP_CMD_RESERVED7_MIN  ) ) {error_code++; SiTRACE("\nOut of range: ");}; SiTRACE("RESERVED7 %d "  , reserved7   );
    if ((reserved8   > Si2141_44_24_POWER_UP_CMD_RESERVED8_MAX  )  || (reserved8   < Si2141_44_24_POWER_UP_CMD_RESERVED8_MIN  ) ) {error_code++; SiTRACE("\nOut of range: ");}; SiTRACE("RESERVED8 %d "  , reserved8   );
    if (error_code) {
      SiTRACE("%d out of range parameters\n", error_code);
      return ERROR_Si2141_44_24_PARAMETER_OUT_OF_RANGE;
    }
    ...
    SiTRACE("\n");
    cmdByteBuffer[0] = Si2141_44_24_POWER_UP_CMD;
    cmdByteBuffer[1] = (unsigned char) ( ( subcode     & Si2141_44_24_POWER_UP_CMD_SUBCODE_MASK     ) << Si2141_44_24_POWER_UP_CMD_SUBCODE_LSB    );
    cmdByteBuffer[2] = (unsigned char) ( ( clock_mode  & Si2141_44_24_POWER_UP_CMD_CLOCK_MODE_MASK  ) << Si2141_44_24_POWER_UP_CMD_CLOCK_MODE_LSB |
                                         ( en_xout     & Si2141_44_24_POWER_UP_CMD_EN_XOUT_MASK     ) << Si2141_44_24_POWER_UP_CMD_EN_XOUT_LSB    );
    cmdByteBuffer[3] = (unsigned char) ( ( config_1p8v & Si2141_44_24_POWER_UP_CMD_CONFIG_1P8V_MASK ) << Si2141_44_24_POWER_UP_CMD_CONFIG_1P8V_LSB);
    cmdByteBuffer[4] = (unsigned char) ( ( reserved3   & Si2141_44_24_POWER_UP_CMD_RESERVED3_MASK   ) << Si2141_44_24_POWER_UP_CMD_RESERVED3_LSB  );
    cmdByteBuffer[5] = (unsigned char) ( ( reserved4   & Si2141_44_24_POWER_UP_CMD_RESERVED4_MASK   ) << Si2141_44_24_POWER_UP_CMD_RESERVED4_LSB  );
    cmdByteBuffer[6] = (unsigned char) ( ( reserved5   & Si2141_44_24_POWER_UP_CMD_RESERVED5_MASK   ) << Si2141_44_24_POWER_UP_CMD_RESERVED5_LSB  );
    cmdByteBuffer[7] = (unsigned char) ( ( reserved6   & Si2141_44_24_POWER_UP_CMD_RESERVED6_MASK   ) << Si2141_44_24_POWER_UP_CMD_RESERVED6_LSB  );
    cmdByteBuffer[8] = (unsigned char) ( ( reserved7   & Si2141_44_24_POWER_UP_CMD_RESERVED7_MASK   ) << Si2141_44_24_POWER_UP_CMD_RESERVED7_LSB  );
    cmdByteBuffer[9] = (unsigned char) ( ( reserved8   & Si2141_44_24_POWER_UP_CMD_RESERVED8_MASK   ) << Si2141_44_24_POWER_UP_CMD_RESERVED8_LSB  );
...
/*---------------------------------------------------*/
/* Si2141_44_24_POWER_UP8_1 COMMAND                           */
/*---------------------------------------------------*/
                                         ...
                                         unsigned char   reserved6)
{
    ...
    SiTRACE("Si2141_44_24 POWER_UP8_1 ");
  #ifdef   DEBUG_RANGE_CHECK
    if ((subcode     > Si2141_44_24_POWER_UP8_1_CMD_SUBCODE_MAX    ) ) {error_code++; SiTRACE("\nOut of range: ");}; SiTRACE("SUBCODE %d "    , subcode     );
    if ((clock_mode  > Si2141_44_24_POWER_UP8_1_CMD_CLOCK_MODE_MAX )  || (clock_mode  < Si2141_44_24_POWER_UP8_1_CMD_CLOCK_MODE_MIN ) ) {error_code++; SiTRACE("\nOut of range: ");}; SiTRACE("CLOCK_MODE %d " , clock_mode  );
    if ((en_xout     > Si2141_44_24_POWER_UP8_1_CMD_EN_XOUT_MAX    ) ) {error_code++; SiTRACE("\nOut of range: ");}; SiTRACE("EN_XOUT %d "    , en_xout     );
    if ((config_1p8v > Si2141_44_24_POWER_UP8_1_CMD_CONFIG_1P8V_MAX)  || (config_1p8v < Si2141_44_24_POWER_UP8_1_CMD_CONFIG_1P8V_MIN) ) {error_code++; SiTRACE("\nOut of range: ");}; SiTRACE("CONFIG_1P8V %d ", config_1p8v );
    if ((reserved3   > Si2141_44_24_POWER_UP8_1_CMD_RESERVED3_MAX  ) ) {error_code++; SiTRACE("\nOut of range: ");}; SiTRACE("RESERVED3 %d "  , reserved3   );
    if ((reserved4   > Si2141_44_24_POWER_UP8_1_CMD_RESERVED4_MAX  )  || (reserved4   < Si2141_44_24_POWER_UP8_1_CMD_RESERVED4_MIN  ) ) {error_code++; SiTRACE("\nOut of range: ");}; SiTRACE("RESERVED4 %d "  , reserved4   );
    if ((reserved5   > Si2141_44_24_POWER_UP8_1_CMD_RESERVED5_MAX  )  || (reserved5   < Si2141_44_24_POWER_UP8_1_CMD_RESERVED5_MIN  ) ) {error_code++; SiTRACE("\nOut of range: ");}; SiTRACE("RESERVED5 %d "  , reserved5   );
    if ((reserved6   > Si2141_44_24_POWER_UP8_1_CMD_RESERVED6_MAX  )  || (reserved6   < Si2141_44_24_POWER_UP8_1_CMD_RESERVED6_MIN  ) ) {error_code++; SiTRACE("\nOut of range: ");}; SiTRACE("RESERVED6 %d "  , reserved6   );
    if (error_code) {
      SiTRACE("%d out of range parameters\n", error_code);
      return ERROR_Si2141_44_24_PARAMETER_OUT_OF_RANGE;
    }
    ...
    SiTRACE("\n");
    cmdByteBuffer[0] = Si2141_44_24_POWER_UP8_1_CMD;
    cmdByteBuffer[1] = (unsigned char) ( ( subcode     & Si2141_44_24_POWER_UP8_1_CMD_SUBCODE_MASK     ) << Si2141_44_24_POWER_UP8_1_CMD_SUBCODE_LSB    );
    cmdByteBuffer[2] = (unsigned char) ( ( clock_mode  & Si2141_44_24_POWER_UP8_1_CMD_CLOCK_MODE_MASK  ) << Si2141_44_24_POWER_UP8_1_CMD_CLOCK_MODE_LSB |
                                         ( en_xout     & Si2141_44_24_POWER_UP8_1_CMD_EN_XOUT_MASK     ) << Si2141_44_24_POWER_UP8_1_CMD_EN_XOUT_LSB    );
    cmdByteBuffer[3] = (unsigned char) ( ( config_1p8v & Si2141_44_24_POWER_UP8_1_CMD_CONFIG_1P8V_MASK ) << Si2141_44_24_POWER_UP8_1_CMD_CONFIG_1P8V_LSB);
    cmdByteBuffer[4] = (unsigned char) ( ( reserved3   & Si2141_44_24_POWER_UP8_1_CMD_RESERVED3_MASK   ) << Si2141_44_24_POWER_UP8_1_CMD_RESERVED3_LSB  );
    cmdByteBuffer[5] = (unsigned char) ( ( reserved4   & Si2141_44_24_POWER_UP8_1_CMD_RESERVED4_MASK   ) << Si2141_44_24_POWER_UP8_1_CMD_RESERVED4_LSB  );
    cmdByteBuffer[6] = (unsigned char) ( ( reserved5   & Si2141_44_24_POWER_UP8_1_CMD_RESERVED5_MASK   ) << Si2141_44_24_POWER_UP8_1_CMD_RESERVED5_LSB  );
    cmdByteBuffer[7] = (unsigned char) ( ( reserved6   & Si2141_44_24_POWER_UP8_1_CMD_RESERVED6_MASK   ) << Si2141_44_24_POWER_UP8_1_CMD_RESERVED6_LSB  );
...
/*---------------------------------------------------*/
/* Si2141_44_24_POWER_UP8_2 COMMAND                           */
/*---------------------------------------------------*/
                                         ...
                                         unsigned char   reserved8)
{
    ...
    SiTRACE("Si2141_44_24 POWER_UP8_2 ");
  #ifdef   DEBUG_RANGE_CHECK
    if ((subcode   > Si2141_44_24_POWER_UP8_2_CMD_SUBCODE_MAX  )  || (subcode   < Si2141_44_24_POWER_UP8_2_CMD_SUBCODE_MIN  ) ) {error_code++; SiTRACE("\nOut of range: ");}; SiTRACE("SUBCODE %d "  , subcode   );
    if ((reserved7 > Si2141_44_24_POWER_UP8_2_CMD_RESERVED7_MAX)  || (reserved7 < Si2141_44_24_POWER_UP8_2_CMD_RESERVED7_MIN) ) {error_code++; SiTRACE("\nOut of range: ");}; SiTRACE("RESERVED7 %d ", reserved7 );
    if ((reserved8 > Si2141_44_24_POWER_UP8_2_CMD_RESERVED8_MAX)  || (reserved8 < Si2141_44_24_POWER_UP8_2_CMD_RESERVED8_MIN) ) {error_code++; SiTRACE("\nOut of range: ");}; SiTRACE("RESERVED8 %d ", reserved8 );
    if (error_code) {
      SiTRACE("%d out of range parameters\n", error_code);
      return ERROR_Si2141_44_24_PARAMETER_OUT_OF_RANGE;
    }
    ...
    SiTRACE("\n");
    cmdByteBuffer[0] = Si2141_44_24_POWER_UP8_2_CMD;
    cmdByteBuffer[1] = (unsigned char) ( ( subcode   & Si2141_44_24_POWER_UP8_2_CMD_SUBCODE_MASK   ) << Si2141_44_24_POWER_UP8_2_CMD_SUBCODE_LSB  );
    cmdByteBuffer[2] = (unsigned char) ( ( reserved7 & Si2141_44_24_POWER_UP8_2_CMD_RESERVED7_MASK ) << Si2141_44_24_POWER_UP8_2_CMD_RESERVED7_LSB);
    cmdByteBuffer[3] = (unsigned char) ( ( reserved8 & Si2141_44_24_POWER_UP8_2_CMD_RESERVED8_MASK ) << Si2141_44_24_POWER_UP8_2_CMD_RESERVED8_LSB);
...
/*---------------------------------------------------*/
/* Si2141_44_24_REREAD_REPLY COMMAND                         */
/*---------------------------------------------------*/
                                         ...
                                         unsigned char   offset)
{
    ...
    SiTRACE("Si2141_44_24 REREAD_REPLY ");
  #ifdef   DEBUG_RANGE_CHECK
    if ((offset > Si2141_44_24_REREAD_REPLY_CMD_OFFSET_MAX) ) {error_code++; SiTRACE("\nOut of range: ");}; SiTRACE("OFFSET %d ", offset );
    if (error_code) {
      SiTRACE("%d out of range parameters\n", error_code);
      return ERROR_Si2141_44_24_PARAMETER_OUT_OF_RANGE;
    }
    ...
    SiTRACE("\n");
    cmdByteBuffer[0] = Si2141_44_24_REREAD_REPLY_CMD;
    cmdByteBuffer[1] = (unsigned char) ( ( offset & Si2141_44_24_REREAD_REPLY_CMD_OFFSET_MASK ) << Si2141_44_24_REREAD_REPLY_CMD_OFFSET_LSB);
    ...
    error_code = Si2141_44_24_pollForResponse(api, 16, rspByteBuffer);
    if (error_code) {
      SiTRACE("Error polling REREAD_REPLY response\n");
      return error_code;
    }

    api->rsp->reread_reply.reply1  = Si2141_44_24_convert_to_byte  (&rspByteBuffer[ 1] , Si2141_44_24_REREAD_REPLY_RESPONSE_REPLY1_LSB   , Si2141_44_24_REREAD_REPLY_RESPONSE_REPLY1_MASK  );
    api->rsp->reread_reply.reply2  = Si2141_44_24_convert_to_byte  (&rspByteBuffer[ 2] , Si2141_44_24_REREAD_REPLY_RESPONSE_REPLY2_LSB   , Si2141_44_24_REREAD_REPLY_RESPONSE_REPLY2_MASK  );
    api->rsp->reread_reply.reply3  = Si2141_44_24_convert_to_byte  (&rspByteBuffer[ 3] , Si2141_44_24_REREAD_REPLY_RESPONSE_REPLY3_LSB   , Si2141_44_24_REREAD_REPLY_RESPONSE_REPLY3_MASK  );
    api->rsp->reread_reply.reply4  = Si2141_44_24_convert_to_byte  (&rspByteBuffer[ 4] , Si2141_44_24_REREAD_REPLY_RESPONSE_REPLY4_LSB   , Si2141_44_24_REREAD_REPLY_RESPONSE_REPLY4_MASK  );
    api->rsp->reread_reply.reply5  = Si2141_44_24_convert_to_byte  (&rspByteBuffer[ 5] , Si2141_44_24_REREAD_REPLY_RESPONSE_REPLY5_LSB   , Si2141_44_24_REREAD_REPLY_RESPONSE_REPLY5_MASK  );
    api->rsp->reread_reply.reply6  = Si2141_44_24_convert_to_byte  (&rspByteBuffer[ 6] , Si2141_44_24_REREAD_REPLY_RESPONSE_REPLY6_LSB   , Si2141_44_24_REREAD_REPLY_RESPONSE_REPLY6_MASK  );
    api->rsp->reread_reply.reply7  = Si2141_44_24_convert_to_byte  (&rspByteBuffer[ 7] , Si2141_44_24_REREAD_REPLY_RESPONSE_REPLY7_LSB   , Si2141_44_24_REREAD_REPLY_RESPONSE_REPLY7_MASK  );
    api->rsp->reread_reply.reply8  = Si2141_44_24_convert_to_byte  (&rspByteBuffer[ 8] , Si2141_44_24_REREAD_REPLY_RESPONSE_REPLY8_LSB   , Si2141_44_24_REREAD_REPLY_RESPONSE_REPLY8_MASK  );
    api->rsp->reread_reply.reply9  = Si2141_44_24_convert_to_byte  (&rspByteBuffer[ 9] , Si2141_44_24_REREAD_REPLY_RESPONSE_REPLY9_LSB   , Si2141_44_24_REREAD_REPLY_RESPONSE_REPLY9_MASK  );
    api->rsp->reread_reply.reply10 = Si2141_44_24_convert_to_byte  (&rspByteBuffer[10] , Si2141_44_24_REREAD_REPLY_RESPONSE_REPLY10_LSB  , Si2141_44_24_REREAD_REPLY_RESPONSE_REPLY10_MASK );
    api->rsp->reread_reply.reply11 = Si2141_44_24_convert_to_byte  (&rspByteBuffer[11] , Si2141_44_24_REREAD_REPLY_RESPONSE_REPLY11_LSB  , Si2141_44_24_REREAD_REPLY_RESPONSE_REPLY11_MASK );
    api->rsp->reread_reply.reply12 = Si2141_44_24_convert_to_byte  (&rspByteBuffer[12] , Si2141_44_24_REREAD_REPLY_RESPONSE_REPLY12_LSB  , Si2141_44_24_REREAD_REPLY_RESPONSE_REPLY12_MASK );
    api->rsp->reread_reply.reply13 = Si2141_44_24_convert_to_byte  (&rspByteBuffer[13] , Si2141_44_24_REREAD_REPLY_RESPONSE_REPLY13_LSB  , Si2141_44_24_REREAD_REPLY_RESPONSE_REPLY13_MASK );
    api->rsp->reread_reply.reply14 = Si2141_44_24_convert_to_byte  (&rspByteBuffer[14] , Si2141_44_24_REREAD_REPLY_RESPONSE_REPLY14_LSB  , Si2141_44_24_REREAD_REPLY_RESPONSE_REPLY14_MASK );
    api->rsp->reread_reply.reply15 = Si2141_44_24_convert_to_byte  (&rspByteBuffer[15] , Si2141_44_24_REREAD_REPLY_RESPONSE_REPLY15_LSB  , Si2141_44_24_REREAD_REPLY_RESPONSE_REPLY15_MASK );
                                         ...
                                         unsigned char   reserved2)
{
    ...
    SiTRACE("Si2141_44_24 RESET_HW ");
  #ifdef   DEBUG_RANGE_CHECK
    if ((subcode   > Si2141_44_24_RESET_HW_CMD_SUBCODE_MAX  )  || (subcode   < Si2141_44_24_RESET_HW_CMD_SUBCODE_MIN  ) ) {error_code++; SiTRACE("\nOut of range: ");}; SiTRACE("SUBCODE %d "  , subcode   );
    if ((reserved1 > Si2141_44_24_RESET_HW_CMD_RESERVED1_MAX) ) {error_code++; SiTRACE("\nOut of range: ");}; SiTRACE("RESERVED1 %d ", reserved1 );
    if ((reserved2 > Si2141_44_24_RESET_HW_CMD_RESERVED2_MAX) ) {error_code++; SiTRACE("\nOut of range: ");}; SiTRACE("RESERVED2 %d ", reserved2 );
    if (error_code) {
      SiTRACE("%d out of range parameters\n", error_code);
      return ERROR_Si2141_44_24_PARAMETER_OUT_OF_RANGE;
    }
    ...
    SiTRACE("\n");
    cmdByteBuffer[0] = Si2141_44_24_RESET_HW_CMD;
    cmdByteBuffer[1] = (unsigned char) ( ( subcode   & Si2141_44_24_RESET_HW_CMD_SUBCODE_MASK   ) << Si2141_44_24_RESET_HW_CMD_SUBCODE_LSB  );
    cmdByteBuffer[2] = (unsigned char) ( ( reserved1 & Si2141_44_24_RESET_HW_CMD_RESERVED1_MASK ) << Si2141_44_24_RESET_HW_CMD_RESERVED1_LSB);
    cmdByteBuffer[3] = (unsigned char) ( ( reserved2 & Si2141_44_24_RESET_HW_CMD_RESERVED2_MASK ) << Si2141_44_24_RESET_HW_CMD_RESERVED2_LSB);
...
/*---------------------------------------------------*/
/* Si2141_44_24_SET_PROPERTY COMMAND                         */
/*---------------------------------------------------*/
                                         ...
                                         unsigned int    data)
{
    ...
    res = Si2141_44_24_PackProperty(api,api->propShadow, prop, (int *) &shadowData);

    SiTRACE("Si2141_44_24 SET_PROPERTY ");
  #ifdef   DEBUG_RANGE_CHECK
    ...
    if ((data != shadowData)  || (api->propertyWriteMode == DOWNLOAD_ALWAYS) || (res== ERROR_Si2141_44_24_UNKNOWN_PROPERTY) )
    {
        SiTRACE("\n");
        cmdByteBuffer[0] = Si2141_44_24_SET_PROPERTY_CMD;
        cmdByteBuffer[1] = (unsigned char) ( ( reserved & Si2141_44_24_SET_PROPERTY_CMD_RESERVED_MASK ) << Si2141_44_24_SET_PROPERTY_CMD_RESERVED_LSB);
        cmdByteBuffer[2] = (unsigned char) ( ( prop     & Si2141_44_24_SET_PROPERTY_CMD_PROP_MASK     ) << Si2141_44_24_SET_PROPERTY_CMD_PROP_LSB    );
        cmdByteBuffer[3] = (unsigned char) ((( prop     & Si2141_44_24_SET_PROPERTY_CMD_PROP_MASK     ) << Si2141_44_24_SET_PROPERTY_CMD_PROP_LSB    )>>8);
        cmdByteBuffer[4] = (unsigned char) ( ( data     & Si2141_44_24_SET_PROPERTY_CMD_DATA_MASK     ) << Si2141_44_24_SET_PROPERTY_CMD_DATA_LSB    );
        cmdByteBuffer[5] = (unsigned char) ((( data     & Si2141_44_24_SET_PROPERTY_CMD_DATA_MASK     ) << Si2141_44_24_SET_PROPERTY_CMD_DATA_LSB    )>>8);
        ...
        api->rsp->set_property.reserved = Si2141_44_24_convert_to_byte  (&rspByteBuffer[ 1] , Si2141_44_24_SET_PROPERTY_RESPONSE_RESERVED_LSB  , Si2141_44_24_SET_PROPERTY_RESPONSE_RESERVED_MASK );
        api->rsp->set_property.data     = Si2141_44_24_convert_to_uint  (&rspByteBuffer[ 2] , Si2141_44_24_SET_PROPERTY_RESPONSE_DATA_LSB      , Si2141_44_24_SET_PROPERTY_RESPONSE_DATA_MASK     );
       /* Update propShadow with latest data if correctly set */
        Si2141_44_24_UnpackProperty(api,api->propShadow, prop, data);
    }
    else
    {
                                         ...
                                         unsigned char   type)
{
    ...
    SiTRACE("Si2141_44_24 STANDBY ");
  #ifdef   DEBUG_RANGE_CHECK
    if ((type > Si2141_44_24_STANDBY_CMD_TYPE_MAX) ) {error_code++; SiTRACE("\nOut of range: ");}; SiTRACE("TYPE %d ", type );
    if (error_code) {
      SiTRACE("%d out of range parameters\n", error_code);
      return ERROR_Si2141_44_24_PARAMETER_OUT_OF_RANGE;
    }
    ...
    SiTRACE("\n");
    cmdByteBuffer[0] = Si2141_44_24_STANDBY_CMD;
    cmdByteBuffer[1] = (unsigned char) ( ( type & Si2141_44_24_STANDBY_CMD_TYPE_MASK ) << Si2141_44_24_STANDBY_CMD_TYPE_LSB);
...
/*---------------------------------------------------*/
/* Si2141_44_24_TUNER_STATUS COMMAND                         */
/*---------------------------------------------------*/
unsigned char Si2141_44_24_L1_TUNER_STATUS    (L1_Si2141_44_24_Context *api)
{
    ...
    SiTRACE("Si2141_44_24 TUNER_STATUS ");
    SiTRACE("\n");
    cmdByteBuffer[0] = Si2141_44_24_TUNER_STATUS_CMD;
    ...
    error_code = Si2141_44_24_pollForResponse(api, 12, rspByteBuffer);
    if (error_code) {
      SiTRACE("Error polling TUNER_STATUS response\n");
      return error_code;
    }

    api->rsp->tuner_status.rsvd1    = Si2141_44_24_convert_to_byte  (&rspByteBuffer[ 1] , Si2141_44_24_TUNER_STATUS_RESPONSE_RSVD1_LSB     , Si2141_44_24_TUNER_STATUS_RESPONSE_RSVD1_MASK    );
    api->rsp->tuner_status.rsvd2    = Si2141_44_24_convert_to_byte  (&rspByteBuffer[ 2] , Si2141_44_24_TUNER_STATUS_RESPONSE_RSVD2_LSB     , Si2141_44_24_TUNER_STATUS_RESPONSE_RSVD2_MASK    );
    api->rsp->tuner_status.rssi     = Si2141_44_24_convert_to_byte  (&rspByteBuffer[ 3] , Si2141_44_24_TUNER_STATUS_RESPONSE_RSSI_LSB      , Si2141_44_24_TUNER_STATUS_RESPONSE_RSSI_MASK     );
    api->rsp->tuner_status.freq     = Si2141_44_24_convert_to_ulong (&rspByteBuffer[ 4] , Si2141_44_24_TUNER_STATUS_RESPONSE_FREQ_LSB      , Si2141_44_24_TUNER_STATUS_RESPONSE_FREQ_MASK     );
    api->rsp->tuner_status.mode     = Si2141_44_24_convert_to_byte  (&rspByteBuffer[ 8] , Si2141_44_24_TUNER_STATUS_RESPONSE_MODE_LSB      , Si2141_44_24_TUNER_STATUS_RESPONSE_MODE_MASK     );
    api->rsp->tuner_status.wb_att_status = Si2141_44_24_convert_to_byte  (&rspByteBuffer[ 8] , Si2141_44_24_TUNER_STATUS_RESPONSE_WB_ATT_STATUS_LSB  , Si2141_44_24_TUNER_STATUS_RESPONSE_WB_ATT_STATUS_MASK );
    api->rsp->tuner_status.vco_code = Si2141_44_24_convert_to_int   (&rspByteBuffer[10] , Si2141_44_24_TUNER_STATUS_RESPONSE_VCO_CODE_LSB  , Si2141_44_24_TUNER_STATUS_RESPONSE_VCO_CODE_MASK );
...
/*---------------------------------------------------*/
/* Si2141_44_24_TUNER_TUNE_FREQ COMMAND                   */
/*---------------------------------------------------*/
                                         ...
                                         unsigned long   freq)
{
    ...
    SiTRACE("Si2141_44_24 TUNER_TUNE_FREQ ");
  #ifdef   DEBUG_RANGE_CHECK
    if ((mode > Si2141_44_24_TUNER_TUNE_FREQ_CMD_MODE_MAX) ) {error_code++; SiTRACE("\nOut of range: ");}; SiTRACE("MODE %d ", mode );
    if ((freq > Si2141_44_24_TUNER_TUNE_FREQ_CMD_FREQ_MAX)  || (freq < Si2141_44_24_TUNER_TUNE_FREQ_CMD_FREQ_MIN) ) {error_code++; SiTRACE("\nOut of range: ");}; SiTRACE("FREQ %d ", freq );
    if (error_code) {
      SiTRACE("%d out of range parameters\n", error_code);
      return ERROR_Si2141_44_24_PARAMETER_OUT_OF_RANGE;
    }
    ...
    SiTRACE("\n");
    cmdByteBuffer[0] = Si2141_44_24_TUNER_TUNE_FREQ_CMD;
    cmdByteBuffer[1] = (unsigned char) ( ( mode & Si2141_44_24_TUNER_TUNE_FREQ_CMD_MODE_MASK ) << Si2141_44_24_TUNER_TUNE_FREQ_CMD_MODE_LSB);
    cmdByteBuffer[2] = (unsigned char)0x00;
    cmdByteBuffer[3] = (unsigned char)0x00;
    cmdByteBuffer[4] = (unsigned char) ( ( freq & Si2141_44_24_TUNER_TUNE_FREQ_CMD_FREQ_MASK ) << Si2141_44_24_TUNER_TUNE_FREQ_CMD_FREQ_LSB);
    cmdByteBuffer[5] = (unsigned char) ((( freq & Si2141_44_24_TUNER_TUNE_FREQ_CMD_FREQ_MASK ) << Si2141_44_24_TUNER_TUNE_FREQ_CMD_FREQ_LSB)>>8);
    cmdByteBuffer[6] = (unsigned char) ((( freq & Si2141_44_24_TUNER_TUNE_FREQ_CMD_FREQ_MASK ) << Si2141_44_24_TUNER_TUNE_FREQ_CMD_FREQ_LSB)>>16);
    cmdByteBuffer[7] = (unsigned char) ((( freq & Si2141_44_24_TUNER_TUNE_FREQ_CMD_FREQ_MASK ) << Si2141_44_24_TUNER_TUNE_FREQ_CMD_FREQ_LSB)>>24);
                                         ...
                                         unsigned char   wake_up)
{
  ...
  #ifdef   DEBUG_RANGE_CHECK
    if ((subcode     > Si2141_44_24_WAKE_UP_CMD_SUBCODE_MAX    )  || (subcode     < Si2141_44_24_WAKE_UP_CMD_SUBCODE_MIN    ) ) {error_code++; SiTRACE("\nOut of range: ");}; SiTRACE("SUBCODE %d "    , subcode     );
    if ((reset       > Si2141_44_24_WAKE_UP_CMD_RESET_MAX      )  || (reset       < Si2141_44_24_WAKE_UP_CMD_RESET_MIN      ) ) {error_code++; SiTRACE("\nOut of range: ");}; SiTRACE("RESET %d "      , reset       );
    if ((clock_freq  > Si2141_44_24_WAKE_UP_CMD_CLOCK_FREQ_MAX )  || (clock_freq  < Si2141_44_24_WAKE_UP_CMD_CLOCK_FREQ_MIN ) ) {error_code++; SiTRACE("\nOut of range: ");}; SiTRACE("CLOCK_FREQ %d " , clock_freq  );
    if ((reserved9   > Si2141_44_24_WAKE_UP_CMD_RESERVED9_MAX  ) ) {error_code++; SiTRACE("\nOut of range: ");}; SiTRACE("RESERVED9 %d "  , reserved9   );
    if ((func        > Si2141_44_24_WAKE_UP_CMD_FUNC_MAX       ) ) {error_code++; SiTRACE("\nOut of range: ");}; SiTRACE("FUNC %d "       , func        );
    if ((wake_up     > Si2141_44_24_WAKE_UP_CMD_WAKE_UP_MAX    )  || (wake_up     < Si2141_44_24_WAKE_UP_CMD_WAKE_UP_MIN    ) ) {error_code++; SiTRACE("\nOut of range: ");}; SiTRACE("WAKE_UP %d "    , wake_up     );
    if (error_code) {
      SiTRACE("%d out of range parameters\n", error_code);
      return ERROR_Si2141_44_24_PARAMETER_OUT_OF_RANGE;
    }
    ...
    SiTRACE("\n");
    cmdByteBuffer[0] = Si2141_44_24_WAKE_UP_CMD;
    cmdByteBuffer[1] = (unsigned char) ( ( subcode     & Si2141_44_24_WAKE_UP_CMD_SUBCODE_MASK     ) << Si2141_44_24_WAKE_UP_CMD_SUBCODE_LSB    );
    cmdByteBuffer[2] = (unsigned char) ( ( reset       & Si2141_44_24_WAKE_UP_CMD_RESET_MASK       ) << Si2141_44_24_WAKE_UP_CMD_RESET_LSB      );
    cmdByteBuffer[3] = (unsigned char) ( ( clock_freq  & Si2141_44_24_WAKE_UP_CMD_CLOCK_FREQ_MASK  ) << Si2141_44_24_WAKE_UP_CMD_CLOCK_FREQ_LSB );
    cmdByteBuffer[4] = (unsigned char) ( ( reserved9   & Si2141_44_24_WAKE_UP_CMD_RESERVED9_MASK   ) << Si2141_44_24_WAKE_UP_CMD_RESERVED9_LSB  );
    cmdByteBuffer[5] = (unsigned char) ( ( func       & Si2141_44_24_WAKE_UP_CMD_FUNC_MASK       ) << Si2141_44_24_WAKE_UP_CMD_FUNC_LSB         );
    cmdByteBuffer[6] = (unsigned char) ( ( wake_up     & Si2141_44_24_WAKE_UP_CMD_WAKE_UP_MASK     ) << Si2141_44_24_WAKE_UP_CMD_WAKE_UP_LSB    );
...
unsigned char   Si2141_44_24_L1_SendCommand2(L1_Si2141_44_24_Context *api, unsigned int cmd_code) {
    switch (cmd_code) {
     ...
     case         Si2141_44_24_AGC_OVERRIDE_CMD_CODE:
       return Si2141_44_24_L1_AGC_OVERRIDE (api, api->cmd->agc_override.force_max_gain, api->cmd->agc_override.force_top_gain );
     break;
    #endif /*     Si2141_44_24_AGC_OVERRIDE_CMD */
    #ifdef        Si2141_44_24_CONFIG_CLOCKS_CMD
     case         Si2141_44_24_CONFIG_CLOCKS_CMD_CODE:
       return Si2141_44_24_L1_CONFIG_CLOCKS (api, api->cmd->config_clocks.subcode, api->cmd->config_clocks.clock_mode, api->cmd->config_clocks.en_xout );
     break;
    #endif /*     Si2141_44_24_CONFIG_CLOCKS_CMD */
    #ifdef        Si2141_44_24_CONFIG_I2C_CMD
     case         Si2141_44_24_CONFIG_I2C_CMD_CODE:
       return Si2141_44_24_L1_CONFIG_I2C (api, api->cmd->config_i2c.subcode, api->cmd->config_i2c.i2c_broadcast );
     break;
    #endif /*     Si2141_44_24_CONFIG_I2C_CMD */
    #ifdef        Si2141_44_24_CONFIG_PINS_CMD
     case         Si2141_44_24_CONFIG_PINS_CMD_CODE:
       return Si2141_44_24_L1_CONFIG_PINS (api, api->cmd->config_pins.gpio1_mode, api->cmd->config_pins.gpio1_read, api->cmd->config_pins.gpio2_mode, api->cmd->config_pins.gpio2_read, api->cmd->config_pins.agc1_mode, api->cmd->config_pins.agc1_read, api->cmd->config_pins.agc2_mode, api->cmd->config_pins.agc2_read, api->cmd->config_pins.xout_mode );
     break;
    #endif /*     Si2141_44_24_CONFIG_PINS_CMD */
    #ifdef        Si2141_44_24_EXIT_BOOTLOADER_CMD
     case         Si2141_44_24_EXIT_BOOTLOADER_CMD_CODE:
       return Si2141_44_24_L1_EXIT_BOOTLOADER (api, api->cmd->exit_bootloader.func, api->cmd->exit_bootloader.ctsien );
     break;
    #endif /*     Si2141_44_24_EXIT_BOOTLOADER_CMD */
    #ifdef        Si2141_44_24_FINE_TUNE_CMD
     case         Si2141_44_24_FINE_TUNE_CMD_CODE:
       return Si2141_44_24_L1_FINE_TUNE (api, api->cmd->fine_tune.persistence, api->cmd->fine_tune.offset_500hz );
     break;
    #endif /*     Si2141_44_24_FINE_TUNE_CMD */
    #ifdef        Si2141_44_24_GET_PROPERTY_CMD
     case         Si2141_44_24_GET_PROPERTY_CMD_CODE:
       return Si2141_44_24_L1_GET_PROPERTY (api, api->cmd->get_property.reserved, api->cmd->get_property.prop );
     break;
    #endif /*     Si2141_44_24_GET_PROPERTY_CMD */
    #ifdef        Si2141_44_24_GET_REV_CMD
     case         Si2141_44_24_GET_REV_CMD_CODE:
       return Si2141_44_24_L1_GET_REV (api );
     break;
    #endif /*     Si2141_44_24_GET_REV_CMD */
    #ifdef        Si2141_44_24_PART_INFO_CMD
     case         Si2141_44_24_PART_INFO_CMD_CODE:
       return Si2141_44_24_L1_PART_INFO (api );
     break;
    #endif /*     Si2141_44_24_PART_INFO_CMD */
    #ifdef        Si2141_44_24_PART_INFO2_CMD
     case         Si2141_44_24_PART_INFO2_CMD_CODE:
       return Si2141_44_24_L1_PART_INFO2 (api );
     break;
    #endif /*     Si2141_44_24_PART_INFO2_CMD */
    #ifdef        Si2141_44_24_POWER_DOWN_HW_CMD
     case         Si2141_44_24_POWER_DOWN_HW_CMD_CODE:
       return Si2141_44_24_L1_POWER_DOWN_HW (api, api->cmd->power_down_hw.subcode, api->cmd->power_down_hw.pdb_xo_osc, api->cmd->power_down_hw.reserved1, api->cmd->power_down_hw.en_xout, api->cmd->power_down_hw.reserved2, api->cmd->power_down_hw.reserved3, api->cmd->power_down_hw.reserved4, api->cmd->power_down_hw.reserved5, api->cmd->power_down_hw.reserved6, api->cmd->power_down_hw.reserved7, api->cmd->power_down_hw.reserved8, api->cmd->power_down_hw.reserved9 );
     break;
    #endif /*     Si2141_44_24_POWER_DOWN_HW_CMD */
    #ifdef        Si2141_44_24_POWER_UP_CMD
     case         Si2141_44_24_POWER_UP_CMD_CODE:
       return Si2141_44_24_L1_POWER_UP (api, api->cmd->power_up.subcode, api->cmd->power_up.clock_mode, api->cmd->power_up.en_xout, api->cmd->power_up.config_1p8v, api->cmd->power_up.reserved3, api->cmd->power_up.reserved4, api->cmd->power_up.reserved5, api->cmd->power_up.reserved6, api->cmd->power_up.reserved7, api->cmd->power_up.reserved8 );
     break;
    #endif /*     Si2141_44_24_POWER_UP_CMD */
    #ifdef        Si2141_44_24_POWER_UP8_1_CMD
     case         Si2141_44_24_POWER_UP8_1_CMD_CODE:
       return Si2141_44_24_L1_POWER_UP8_1 (api, api->cmd->power_up8_1.subcode, api->cmd->power_up8_1.clock_mode, api->cmd->power_up8_1.en_xout, api->cmd->power_up8_1.config_1p8v, api->cmd->power_up8_1.reserved3, api->cmd->power_up8_1.reserved4, api->cmd->power_up8_1.reserved5, api->cmd->power_up8_1.reserved6 );
     break;
    #endif /*     Si2141_44_24_POWER_UP8_1_CMD */
    #ifdef        Si2141_44_24_POWER_UP8_2_CMD
     case         Si2141_44_24_POWER_UP8_2_CMD_CODE:
       return Si2141_44_24_L1_POWER_UP8_2 (api, api->cmd->power_up8_2.subcode, api->cmd->power_up8_2.reserved7, api->cmd->power_up8_2.reserved8 );
     break;
    #endif /*     Si2141_44_24_POWER_UP8_2_CMD */
    #ifdef        Si2141_44_24_REREAD_REPLY_CMD
     case         Si2141_44_24_REREAD_REPLY_CMD_CODE:
       return Si2141_44_24_L1_REREAD_REPLY (api, api->cmd->reread_reply.offset );
     break;
    #endif /*     Si2141_44_24_REREAD_REPLY_CMD */
    #ifdef        Si2141_44_24_RESET_HW_CMD
     case         Si2141_44_24_RESET_HW_CMD_CODE:
       return Si2141_44_24_L1_RESET_HW (api, api->cmd->reset_hw.subcode, api->cmd->reset_hw.reserved1, api->cmd->reset_hw.reserved2 );
     break;
    #endif /*     Si2141_44_24_RESET_HW_CMD */
    #ifdef        Si2141_44_24_SET_PROPERTY_CMD
     case         Si2141_44_24_SET_PROPERTY_CMD_CODE:
       return Si2141_44_24_L1_SET_PROPERTY (api, api->cmd->set_property.reserved, api->cmd->set_property.prop, api->cmd->set_property.data );
     break;
    #endif /*     Si2141_44_24_SET_PROPERTY_CMD */
    #ifdef        Si2141_44_24_STANDBY_CMD
     case         Si2141_44_24_STANDBY_CMD_CODE:
       return Si2141_44_24_L1_STANDBY (api, api->cmd->standby.type );
     break;
    #endif /*     Si2141_44_24_STANDBY_CMD */
    #ifdef        Si2141_44_24_TUNER_STATUS_CMD
     case         Si2141_44_24_TUNER_STATUS_CMD_CODE:
       return Si2141_44_24_L1_TUNER_STATUS (api );
     break;
    #endif /*     Si2141_44_24_TUNER_STATUS_CMD */
    #ifdef        Si2141_44_24_TUNER_TUNE_FREQ_CMD
     case         Si2141_44_24_TUNER_TUNE_FREQ_CMD_CODE:
       return Si2141_44_24_L1_TUNER_TUNE_FREQ (api, api->cmd->tuner_tune_freq.mode, api->cmd->tuner_tune_freq.freq );
     break;
    #endif /*     Si2141_44_24_TUNER_TUNE_FREQ_CMD */
    #ifdef        Si2141_44_24_WAKE_UP_CMD
     case         Si2141_44_24_WAKE_UP_CMD_CODE:
       return Si2141_44_24_L1_WAKE_UP (api, api->cmd->wake_up.subcode, api->cmd->wake_up.reset, api->cmd->wake_up.clock_freq, api->cmd->wake_up.reserved9, api->cmd->wake_up.func, api->cmd->wake_up.wake_up );
     break;
    #endif /*     Si2141_44_24_WAKE_UP_CMD */
   default : break;
    }
...
unsigned char   Si2141_44_24_L1_GetCommandResponseString(L1_Si2141_44_24_Context *api, unsigned int cmd_code, const char *separator, char *msg) {
    ...
    #ifdef        Si2141_44_24_POWER_UP8_1_CMD
     case         Si2141_44_24_POWER_UP8_1_CMD_CODE:
      sprintf(msg,"POWER_UP8_1 ");
       sprintf(msg,"%s%s",msg,separator); strcat(msg,"-TUNINT ");
           if  (api->rsp->power_up8_1.STATUS->tunint ==     0) strcat(msg,"NOT_TRIGGERED");
      else if  (api->rsp->power_up8_1.STATUS->tunint ==     1) strcat(msg,"TRIGGERED    ");
      else                                                    sprintf(msg,"%s%d", msg, api->rsp->power_up8_1.STATUS->tunint);
       sprintf(msg,"%s%s",msg,separator); strcat(msg,"-ATVINT ");
           if  (api->rsp->power_up8_1.STATUS->atvint ==     0) strcat(msg,"NOT_TRIGGERED");
      else if  (api->rsp->power_up8_1.STATUS->atvint ==     1) strcat(msg,"TRIGGERED    ");
      else                                                    sprintf(msg,"%s%d", msg, api->rsp->power_up8_1.STATUS->atvint);
       sprintf(msg,"%s%s",msg,separator); strcat(msg,"-DTVINT ");
           if  (api->rsp->power_up8_1.STATUS->dtvint ==     0) strcat(msg,"NOT_TRIGGERED");
      else if  (api->rsp->power_up8_1.STATUS->dtvint ==     1) strcat(msg,"TRIGGERED    ");
      else                                                    sprintf(msg,"%s%d", msg, api->rsp->power_up8_1.STATUS->dtvint);
       sprintf(msg,"%s%s",msg,separator); strcat(msg,"-ERR    ");
           if  (api->rsp->power_up8_1.STATUS->err    ==     1) strcat(msg,"ERROR   ");
      else if  (api->rsp->power_up8_1.STATUS->err    ==     0) strcat(msg,"NO_ERROR");
      else                                                    sprintf(msg,"%s%d", msg, api->rsp->power_up8_1.STATUS->err);
       sprintf(msg,"%s%s",msg,separator); strcat(msg,"-CTS    ");
           if  (api->rsp->power_up8_1.STATUS->cts    ==     1) strcat(msg,"COMPLETED");
      else if  (api->rsp->power_up8_1.STATUS->cts    ==     0) strcat(msg,"WAIT     ");
      else                                                    sprintf(msg,"%s%d", msg, api->rsp->power_up8_1.STATUS->cts);
     break;
    #endif /*     Si2141_44_24_POWER_UP8_1_CMD */

    #ifdef        Si2141_44_24_POWER_UP8_2_CMD
     case         Si2141_44_24_POWER_UP8_2_CMD_CODE:
      sprintf(msg,"POWER_UP8_2 ");
       sprintf(msg,"%s%s",msg,separator); strcat(msg,"-TUNINT ");
           if  (api->rsp->power_up8_2.STATUS->tunint ==     0) strcat(msg,"NOT_TRIGGERED");
      else if  (api->rsp->power_up8_2.STATUS->tunint ==     1) strcat(msg,"TRIGGERED    ");
      else                                                    sprintf(msg,"%s%d", msg, api->rsp->power_up8_2.STATUS->tunint);
       sprintf(msg,"%s%s",msg,separator); strcat(msg,"-ATVINT ");
           if  (api->rsp->power_up8_2.STATUS->atvint ==     0) strcat(msg,"NOT_TRIGGERED");
      else if  (api->rsp->power_up8_2.STATUS->atvint ==     1) strcat(msg,"TRIGGERED    ");
      else                                                    sprintf(msg,"%s%d", msg, api->rsp->power_up8_2.STATUS->atvint);
       sprintf(msg,"%s%s",msg,separator); strcat(msg,"-DTVINT ");
           if  (api->rsp->power_up8_2.STATUS->dtvint ==     0) strcat(msg,"NOT_TRIGGERED");
      else if  (api->rsp->power_up8_2.STATUS->dtvint ==     1) strcat(msg,"TRIGGERED    ");
      else                                                    sprintf(msg,"%s%d", msg, api->rsp->power_up8_2.STATUS->dtvint);
       sprintf(msg,"%s%s",msg,separator); strcat(msg,"-ERR    ");
           if  (api->rsp->power_up8_2.STATUS->err    ==     1) strcat(msg,"ERROR   ");
      else if  (api->rsp->power_up8_2.STATUS->err    ==     0) strcat(msg,"NO_ERROR");
      else                                                    sprintf(msg,"%s%d", msg, api->rsp->power_up8_2.STATUS->err);
       sprintf(msg,"%s%s",msg,separator); strcat(msg,"-CTS    ");
           if  (api->rsp->power_up8_2.STATUS->cts    ==     1) strcat(msg,"COMPLETED");
      else if  (api->rsp->power_up8_2.STATUS->cts    ==     0) strcat(msg,"WAIT     ");
      else                                                    sprintf(msg,"%s%d", msg, api->rsp->power_up8_2.STATUS->cts);
     break;
    #endif /*     Si2141_44_24_POWER_UP8_2_CMD */
