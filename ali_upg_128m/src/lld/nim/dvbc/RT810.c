...
//----------------------------------------------------------//
UINT8 RT810_Arry[50];
RT810_SectType IMR_Data[6] = {{0,0,0}};//Please keep this array data for standby mode.
I2C_TYPE  RT810_I2C;
...
RT810_ErrCode RT810_Muti_Read(UINT8 IMR_Reg, UINT16* IMR_Data);
...
RT810_ErrCode RT810_InitReg(void)
{
    ...
    if(I2C_Write_Len(&RT810_I2C_Len) != RT_Success)
        return RT_Fail;
...
RT810_ErrCode RT810_IMR_Prepare(void)
{
    ...
    if(I2C_Write(&RT810_I2C) != RT_Success)
        return RT_Fail;
    //ring_mux= from ring
...
RT810_ErrCode RT810_IMR(UINT8 IMR_MEM, BOOL IM_Flag)
{
    ...
    for(n=0;n<16;n++)
    {
        if((UINT32)(2*(2+n)*RT810_Xtal)>=RingRef)
        {
        n_ring=n;
        break;
        }
    ...
    case 4:
        ...
        break;
    default:
        IMR_Data[5].Gain_X  = IMR_POINT.Gain_X;
        IMR_Data[5].Phase_Y = IMR_POINT.Phase_Y;
        IMR_Data[5].Value   = IMR_POINT.Value;
        break;
    }
...
RT810_ErrCode RT810_PLL(UINT32 LO_Freq)
{
    ...
    UINT32 VCO_Freq = 0;
    UINT32 Bry_M    = 1;
    //UINT32 Bry_L    = 0;//dev->channel_search = nim_c3505_channel_search;
    UINT32 Si       = 0;
    ...
    SDM_B  = 0x80;
    Bry_M  = 50000;
    //Bry_L  = 0; //dev->channel_search = nim_c3505_channel_search;
    ...
    RT810_I2C_Len.Len     = 3;
    if(I2C_Read_Len(&RT810_I2C_Len) != RT_Success)
        return RT_Fail;
...
RT810_ErrCode RT810_MUX(UINT32 RF_KHz)
{
    ...
    //Set_IMR
    RT_Reg08 = IMR_Data[Freq_Info1.IMR_MEM].Gain_X;
    RT_Reg09 = IMR_Data[Freq_Info1.IMR_MEM].Phase_Y;
