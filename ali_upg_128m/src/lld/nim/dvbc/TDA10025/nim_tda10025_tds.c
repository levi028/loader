...
static INT32 nim_tda10025_open(struct nim_device *dev)
{
    ...
    if (priv->i2c_mutex == OSAL_INVALID_ID)
    {
...
#if 1   //judge  funllnim online and fullnim api params(id & addr)
		if (SUCCESS != nim_i2c_read( priv->tuner_config_ext.i2c_type_id,
									 priv->tuner_config_ext.c_tuner_base_addr, 
									 &data, 
									 1))
		{	
			TDA10025_PRINTF("[%s]line=%d,nim_tda10025_open fail! data = %d\n",__FUNCTION__,__LINE__,data);

			if (RET_SUCCESS != os_delete_mutex(priv->i2c_mutex))
			{
					TDA10025_PRINTF("nim_tda10025_open: Delete mutex failed!\n");
			}
		...
		{
			TDA10025_PRINTF("[%s]line=%d,nim_tda10025_open success! data = %d\n",__FUNCTION__,__LINE__,data);
		}
#endif
