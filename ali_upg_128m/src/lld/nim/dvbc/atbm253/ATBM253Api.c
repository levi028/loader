...
#define ATBM253_DEFAULT_I2C_ADDR (0xF8)
...
ATBM253_ERROR_e ATBM253Init(ATBM_U32 DevId,ATBM253InitConfig_t *pConfig)
{
    ...
    memset(&ATBM253Tuner[DevId],0,sizeof(ATBM253_t));
...
ATBM253_ERROR_e ATBM253DefaultCfgGet(ATBM253InitConfig_t *pConfig)
{
    ...
    pConfig->Mode = ATBM253_SIGNAL_MODE_DVBC;
    ...
    pConfig->PinsCfg[0].PinState = ATBM253_GPO_STATE_LOW;
    pConfig->PinsCfg[1].PinName = ATBM253_PIN_NAME_GPO2;
    pConfig->PinsCfg[1].PinState = ATBM253_GPO_STATE_LOW;
    pConfig->LTAOut = ATBM253_RF_LTA_OUT_ENABLE;
...
ATBM253_ERROR_e ATBM253Process(ATBM_U32 DevId)
{
    ...
    if(ATBM253MutexInitFlag == FALSE)
    {
       ATBM253Print("[Error]ATBM253Process(%d),NO INIT!\n",DevId);
       return ATBM253_ERROR_NOT_INIT;
    }
    ret = ATBM253MutexLock();
    pTuner = &ATBM253Tuner[DevId];    
...
int tun_atbm253_get_status(unsigned int *tuner_id, unsigned char lock_status)
{
	int ret = 0;
	...
	ret = ATBM253AGCLockingFreeze(dev_id, lock_status);
	return ret;
}
