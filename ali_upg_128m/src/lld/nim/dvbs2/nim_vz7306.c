...
*    File:    nim_vz7306.c
*
*    Description:    This file contains SHARP BS2S7VZ7306 tuner basic function.
*    History:
...
struct QPSK_TUNER_CONFIG_EXT * vz7306_dev_id[MAX_TUNER_SUPPORT_NUM] = {NULL};
...
INT32 nim_vz7306_init(UINT32* tuner_id, struct QPSK_TUNER_CONFIG_EXT * ptrTuner_Config)
{
	...
	if ((ptrTuner_Config == NULL)||(vz7306_tuner_cnt>=MAX_TUNER_SUPPORT_NUM) || (tuner_id == NULL))
        return ERR_FAILUE;
    vz7306_ptr = (struct QPSK_TUNER_CONFIG_EXT *)MALLOC(sizeof(struct QPSK_TUNER_CONFIG_EXT));
    if(!vz7306_ptr)
          return ERR_FAILUE;
    ...
    *tuner_id = vz7306_tuner_cnt;
    vz7306_tuner_cnt++;
    if ((result = i2c_write(vz7306_ptr->i2c_type_id, vz7306_ptr->c_tuner_base_addr, init_data1, sizeof(init_data1))) != SUCCESS)
    ...
    if ((result = i2c_write(vz7306_ptr->i2c_type_id, vz7306_ptr->c_tuner_base_addr, init_data2, sizeof(init_data2))) != SUCCESS)
        return result;


    osal_delay(10000);

    if ((result = i2c_write(vz7306_ptr->i2c_type_id, vz7306_ptr->c_tuner_base_addr, init_data3, sizeof(init_data3))) != SUCCESS)
        return result;
...
INT32 nim_vz7306_control(UINT32 tuner_id,UINT32 freq, UINT32 sym)
{
    ...
    data[2] = 0xE1;
    data[3] = (BA<<5) | (DIV<<1);
    if ((result = i2c_write(vz7306_ptr->i2c_type_id, vz7306_ptr->c_tuner_base_addr, data+0, 4)) != SUCCESS)
    {
    ...
    data[2] = 0xE5;
    if ((result = i2c_write(vz7306_ptr->i2c_type_id, vz7306_ptr->c_tuner_base_addr, data+2, 1)) != SUCCESS)
        return result;
    osal_delay(10000);

    data[2] |= ((LPF & 0x01) << 4) | ((LPF & 0x02) << 2);
    data[3] |= ((LPF & 0x04) << 1) | ((LPF & 0x08) >> 1);
    if ((result = i2c_write(vz7306_ptr->i2c_type_id, vz7306_ptr->c_tuner_base_addr, data+2, 2)) != SUCCESS)
        return result;
