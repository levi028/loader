...
INT32 nim_rt710_init(UINT32* tuner_id, struct QPSK_TUNER_CONFIG_EXT * ptrTuner_Config)
{
	...
	*tuner_id = rt710_tuner_cnt;
	...
	/*Output Signal Mode*/
	if(rt710_ptr->c_tuner_out_s_d_sel != 0)  // 1--> Single end, 0--> Differential
	{
...
static int Tuner_I2C_write(UINT32 tuner_id, unsigned char reg_start, unsigned char* buff, unsigned char length)
{
        ...
        while((rd+15)<length)
        {
                ...
                i2c_result = i2c_write(rt710_ptr->i2c_type_id, rt710_ptr->c_tuner_base_addr, data, 16);
                rd+=15;
                data[0] += 15;
        ...
        i2c_result = i2c_write(rt710_ptr->i2c_type_id, rt710_ptr->c_tuner_base_addr, data, length-rd+1);
        return i2c_result;
}
...
static int Tuner_I2C_read(UINT32 tuner_id, unsigned char reg_start, unsigned char* buff, unsigned char length)
{
        ...
        while((rd+15)<length)
        {
                i2c_result = i2c_write_read(rt710_ptr->i2c_type_id, rt710_ptr->c_tuner_base_addr, data, 1, 15);
        ...
        i2c_result = i2c_write_read(rt710_ptr->i2c_type_id, rt710_ptr->c_tuner_base_addr, data, 1, length-rd);
        memcpy(&buff[rd], &data[0], 15);
		return i2c_result;
}
