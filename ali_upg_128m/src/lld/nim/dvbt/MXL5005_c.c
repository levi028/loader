...
#define MXL5005_ADDR            0xC6
#define DEM_WRITE                1
#define DEM_READ                0

extern int  Write_I2C(_u8 *Reg_Addr, _u8 *Reg_Data, int BytesNo,UINT32 tuner_id);
//Tuner_struct MXL5005_Tuner;

#if 0

//////////////////////////////////////////////////////////////////////////////////////////
unsigned char ZL10353_I2CPassThrough(unsigned char mode);
int I2cReadWrite(int mode,unsigned char ChipAddress,unsigned char *Data,int NbData);
//int I2cReadWrite35X(int mode,unsigned char ChipAddress,unsigned char *Data,int NbData);
void ZL10353_Dly1(unsigned int i);
//////////////////////////////////////////////////////////////////////////////////////////


//Penghui Added 20070801
...
int I2cReadWrite(int mode,unsigned char ChipAddress,unsigned char *Data,int NbData)
{
    ...
    if (mode == DEM_WRITE)
        ret = i2c_write(I2C_TYPE_SCB, ChipAddress, Data, NbData);
    else
        ret = SUCCESS;
    return ret;
...
void ZL10353_Dly1(unsigned int i)
{
    osal_task_sleep((unsigned long)i);
}



int MXL5005_I2C_ReadWrite(_u8 bIndex, _u8 bValue)
{
    _u8 szBuffer[4];
    int status;
    ...
    status = I2cReadWrite(DEM_WRITE,(char )MXL5005_ADDR, szBuffer, 3);
    if(status != 0)
    {
        //printf("MXL5005 i2c error code : %d\r\n", status);
        return 0;
    }
  return 1;
}
...
int  Write_I2C(_u8 *Reg_Addr, _u8 *Reg_Data, int BytesNo)
{
   int status = 0;
   int i;
   ...
   for( i = 0 ; i < BytesNo; i++)
   {
     if(MXL5005_I2C_ReadWrite(Reg_Addr[i], Reg_Data[i]) == 0)
     {
         status =  1;
         break;
     }
...
int  Write_I2C(_u8 *Reg_Addr, _u8 *Reg_Data, int BytesNo)
{
    ...
    for (i = 0; i<BytesNo; i++)
        {
            szBuffer[0] = Reg_Addr[i];
            szBuffer[1] = Reg_Data[i];
            szBuffer[2] = 254;
            status= i2c_write(I2C_TYPE_SCB, 0xC6, szBuffer, 3);
            //libc_printf("b0 = %d,b1=%d,b2=%d\n",szBuffer[0],szBuffer[1],szBuffer[2]);
            osal_task_sleep(10);
        }
    //libc_printf("\n");
}
        ...
        )
{
    ...
    if (Tuner->bMxl_Tuner_Inited == 0)
    {
//        libc_printf("init tuner...\n");
        InitTunerControls (Tuner) ;
        Tuner->bMxl_Tuner_Inited = 1;
    }
...
_u16 MXL_ControlWrite(Tuner_struct *Tuner, _u16 ControlNum, _u32 value)
{
    ...
    _u16 status = 0 ;
    status = MXL5005_ControlSet (Tuner,  ControlNum, value);
...
_u32    if_divval_temp ;
    ...
    rf_in    = dwRF_Freq_KHz*1000UL;
    bandwidth = bBandWidthMHz*1000000L;
    if_out = (_u32)(dwIF_Freq_KHz * 1000UL);//36000000;//4570000;
    fxtal = (_u32)(crystal_KHz * 1000UL);
    top  = (_u16)(dw_TOP);
    ...
    //if_out = 4570000UL;//36000000;//4570000;
    //fxtal = 16000000UL;
    ...
    mod_type    = MXL_DVBT ;              // Options - MXL_DEFAULT_MODULATION = 0
                                          ...
                                          //           MXL_QAM = 3
                                          //           MXL_ANALOG_CABLE = 4
    tf_type     = MXL_TF_OFF ;            // Options - MXL_TF_DEFAULT = 0
                                          //           MXL_TF_OFF = 1
                                          //           MXL_TF_C = 2
                                          //           MXL_TF_C_H = 3
                                          //           MXL_TF_D = 4
    ...
    status += Write_I2C( Reg_Addr, Reg_Data, 1,tuner_id) ;
    ...
    //
    status += Write_I2C( Reg_Addr, Reg_Data, count,tuner_id) ;
    ...
    if (if_mode == MXL_ZERO_IF) // Analog or Digital Zero IF mode
    {
        ...
        status += Write_I2C( Reg_Addr, Reg_Data, count+1,tuner_id) ;
