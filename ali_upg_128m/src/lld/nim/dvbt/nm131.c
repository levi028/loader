...
static const uint32 rf_freq_offset_lut[] = { 
	/*      nNTSC */  2220000, 
	/*     nPAL_G */  2720000, 
	/*     nPAL_M */  2220000, 
	/*     nPAL_N */  2220000, 
...
static const  uint32 adec_ddfs_fq /*(0x230)*/[] = {
	/*      nNTSC */ 0x00015555, 
	/*     nPAL_G */ 0x0001a12f, 
	/*     nPAL_M */ 0x00015555, 
	/*     nPAL_N */ 0x00015555, 
...
static const  uint32 cvbs_ddfs_audio_out_lut /*(0x234)*/[] = {
	/*      nNTSC */ 0x00015555, 
	/*     nPAL_G */ 0x0001a12f, 
	/*     nPAL_M */ 0x00015555, 
	/*     nPAL_N */ 0x00015555, 
...
static const  uint32 ddfs_lut /*(0x1bc)*/[] = {
	/*      nNTSC */ 0x0002a190, 
	/*     nPAL_G */ 0x00033945, 
	/*     nPAL_M */ 0x0002a190, 
	/*     nPAL_N */ 0x0002a190, 
...
static const uint32 lo_freq_lut[] = { 
	/*               0 */        0, 
	/*               1 */        0, 
	/*               2 */ 434000000, 
	/*               3 */ 237000000, 
...
static const  uint32 avsep_comp /*(0x104)*/[] = {
	/*      nNTSC */ 0x00000004, 
	/*     nPAL_G */ 0x00000007, 
	/*     nPAL_M */ 0x00000004, 
	/*     nPAL_N */ 0x00000004, 
...
static void nm131_demod_config(uint32 rffrequency, tNmiTvStd std, 
							   tNmiOutput output, uint32 aif, uint32 vif, uint32 clk_off_f, 
							   bool_t is_stereo, tTnrTuneMode tuneMode, tNmiDacSel dacSel)
{
	uint32 val32;
	uint32 audio_offset;
...

	{
		...
		**/
		...
		ddfs =	(ddfs << 14)/((1 << 14) + (clk_off_f/(216000 >> 5)));
		wReg32(0x1bc, ddfs);
	...
	**/
...

	{
		if ((std == nNTSC)||(std == nFM_RADIO)) {
			...
			val32 |= 0x3ul;
		} else if ((std == nPAL_L) || (std == nSECAM_L)) {
...
static void nm131_invert_spectum(int invert)
{
	...
	if(pp->output == nIf) {
		...
		if(invert == 0) {
			uint32 vif;
			/* vddfs = (rf_freq_offset_lut[std]*2)/(1+clk_offset); */
...
static void nm131_tuner_apply_vhf_filter(uint32 rffrequency)
{
	size_t i;
...
static void nm131_tuner_config(uint32 rffrequency, tNmiTvStd std, tNmiOutput output, uint32 *clk_off_f, tNmiDacSel dac_sel)
{
	tNmiPriv 		*pp = (tNmiPriv *)&chip.priv;
	uint32			xo_f;
	uint16_t n0_f;
	...
	if(chip.inp.xo_out_en) {
		...
		if(std == nFM_RADIO) {
			...
			if((std ==nPAL_K) &&
				...
				if(ISATV(std)) {
					r8 = 0x05;  //Dont use 0x85 forever. LLLL
					pp->cvbsMaxVol = 15;
				} else {
					if( (rffrequency >= (506000000-150000)) && (rffrequency <= (506000000+150000)) ) {
						r8 = 0x05;
					} else {
						r8 = 0x85;
					}
	...
	**/
	*clk_off_f = ( (vco_f << 9)/((uint32)w_f) ) - ((1ul << 9)*216000);
...
static void nm131_tune_internal(tTnrTune *tune, tTnrTuneMode tuneMode)
{
	...
	nm131_tuner_config(tune->rf, tune->std, tune->output, &clk_off_f, tune->dacSel);
	nm131_demod_config(tune->rf, tune->std, tune->output, tune->aif, tune->vif,
		clk_off_f, tune->is_stereo, tuneMode, tune->dacSel);
...
static uint32 nm131_get_chipid(void) 
{
	uint32 chipid;
...
static void nm131_notch_ctl(int en, int width)
{
	...
	if (en) {
		if (
			...
			pp->dconotch_ctl |= ((((uint32)width) & 0x7ul) << 12);
			pp->dconotch_ctl &= ~(1ul << 1);			
...
#if !defined USE_NM131_NOTCH_LUT
static void nm131_notch_set(int en, uint32 frequency, int width)
{
	tNmiPriv 	*pp = (tNmiPriv *)&chip.priv;
	...
	**/
	...
	if(ISDTV(pp->std)  || (pp->std == nFM_RADIO)) {
		...
		if(			
			...
			uint32 xo, bh, bl, hrmnch, hrmncl;

			xo = (uint32)chip.inp.xo * 1000;
...
static void wReg32Debug(uint16_t adr, uint32 val32)
{
	tNmiPriv * pp = &chip.priv;
