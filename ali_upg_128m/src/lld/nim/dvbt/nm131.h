...
typedef struct {
    ...
    uint32 len; /*!< [IN] Temporary Buffer length. */
} tNmiBusTest;
...
typedef struct {
    struct control{
        int (*read)(uint8_t, uint8_t *, uint32);
        int (*write)(uint8_t, uint8_t *, uint32);
        uint32 (*getreadblksz)(void);
        uint32 (*getwriteblksz)(void);
    } c;
    void (*delay)(uint32);
    uint32 (*gettick)(void);
    void (*log)(char *);
