...
static void nmi_debug(uint32 zone, char *fmt,...)
{
	char buf[256];
	va_list args;
	int len;
	...
	if (zone & chip.inp.zone) {
		...
		len = vsnprintf(&buf[12], 243, fmt, args);
		va_end(args);
...
**
*******************************************************************************/
static void nmi_delay(uint32 msec)
{
...
static uint32 nmi_get_tick(void)
{
	uint32 tick = 0;
...
static uint32 rReg32(uint16_t adr)
{
	uint8_t b[4];
...
void wReg32(uint16_t adr, uint32 val)
{
	uint8_t b[6];
...
static void wRegBurst(uint32 adr, uint8_t *bd, int sz)
{
#define OVERHEAD (2)

	int retVal;	
	int i;
	uint32 sz2, sz3, blksz = 4;
