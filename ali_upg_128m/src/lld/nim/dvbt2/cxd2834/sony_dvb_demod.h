...
------------------------------------------------------------------------------*/
#define SONY_DVB_DEMOD_ADDRESS                   0xC8 //0xD8    /**< I2C address. */


#define LOG_STRING_BUFFER_SIZE    255

#if 1
#define CXD2834_LOG(param, fmt, args...) \
    do{}while(0)
...
//#define CXD2834_LOG_I2C        cxd2834_log_i2c
#define CXD2834_LOG_I2C(...)         do{}while(0)
...
#define DVB_DEMOD_DVBT2_WAIT_LOCK           3500    /**< DVB-T2 3500ms for demod lock indication. */
#define DVB_DEMOD_DVBT2_WAIT_TS_LOCK        1500    /**< DVB-T2 1500ms for TS lock. Assumes that demod core is locked. */
#define DVB_DEMOD_TUNE_T2_L1POST_TIMEOUT    600 //300     /**< DVB-T2 300ms timeout for L1Post Valid loop */
...
//    UINT32 bandwidth;
    ...
    UINT8 all_plp_id[255];      //all plp_id.
    ...
    */
    ...
    /**
     @brief RF level monitor enable/disable.
            ...
            Use ::sony_dvb_demod_SetConfig to configure this flag.
    */
    int enable_rflvmon;
...
/**
 @brief Tuning parameters used in ::sony_dvb_integration_Tune.
*/
typedef struct sony_dvb_tune_params_t {

/* Comments are different between CXD2834 and CXD2835/36 for centreFreqkHz */
#ifdef SONY_DVB_SUPPORT_DVBC2
    /**
     @brief The function of this parameter is system dependant:
            - DVB-C / T / T2
                - The centre frequency of the channel to tune to.
            - DVB-C2
                - Normal DS
                @n Either provide the RF tuning frequency to compensate for IF frequency offset
                or set to 0 and ::sony_dvbc2_tune_params_t::c2TuningFrequencykHz will be used for
                RF tune.
                - Dependant Static DS
                @n Either provide the RF tuning frequency for the second tune to compensate IF
                frequency offset or set to 0 and ::sony_dvbc2_tune_params_t::c2TuningFrequencyDSDSkHz
                will be used for 2nd RF tune.
    */
#else
    /**
     @brief The centre frequency of the channel to tune to.
    */
#endif
    ...
    */
    ...
    /**
     @brief If tuneResult == SONY_RESULT_OK, then this field contains the
            ...
            with the ::sony_dvb_demod_monitor_CarrierOffset function.
    */
    uint32_t centreFreqkHz;
...
/**
 @brief Finalize the demodulator.
        ...
        ::sony_dvb_demod_Initialize must be called to re-initialise the
...
typedef enum {
    ...
    */
    ...
    /**
     @brief RF AGC PWM enable/disable.
            ...
            GPIO1 configured with ::sony_dvb_demod_GPIOSetting.
    ...
    */
    ...
    /**
     @brief Set Shared IF mode. Shared IF mode is available once
            demodulator is in sleep mode (::sony_dvb_demod_Sleep).
    ...
    */
    ...
    /**
     @brief Enable/Disable TS error output. Configure when TSERR is active
            with ::DEMOD_CONFIG_TSERR_MASK.

            Note, this will overwrite the port settings for GPIO2.

            - 0 : TS Error output is disabled.
            - 1 : TS Error output is enabled.
    */
    ...
    */
    ...
    /**
     @brief Set the measurment period for Pre-BCH BER (DVB-T/C) and
    ...
    */
    ...
    /**
     @brief Disable the internal pull-up resistor for GPIO0
    ...
    */
    ...
    /**
     @brief Disable the internal pull-up resistor for GPIO1
    ...
    */
    ...
    /**
     @brief Disable the internal pull-up resistor for GPIO2
    ...
    */
    ...
    /**
     @brief Enable the internal pull-up resistor for TS Clk / TS Sync
            / TS Valid / TS Data pins
