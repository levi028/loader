...
sony_result_t sony_dvb_demodT2_CheckTSLock (sony_dvb_demod_t * pDemod, sony_dvb_demod_lock_result_t * pLock)
{
    ...
    *pLock = DEMOD_LOCK_RESULT_NOTDETECT;
    ...
    if (result != SONY_RESULT_OK) {
        CXD2834_LOG(pDemod, "%s()=%d: sync=0x%X, tslock=0x%X, unlockDetected=0x%X.\r\n", __FUNCTION__, result, sync, tslock, unlockDetected);
        SONY_TRACE_RETURN (result);
    }
...
sony_result_t sony_dvb_demodT2_OptimizeMISO (sony_dvb_demod_t * pDemod, sony_dvbt2_ofdm_t *ofdm)
{
    sony_result_t result = SONY_RESULT_OK;
...
static sony_result_t SL2NOG_BandSetting (sony_dvb_demod_t * pDemod)
{
    ...
    /* slave    Bank    Addr    Bit     default   Value     Name
     * -----------------------------------------------------------------------------------------------------------------------------
     * <SLV-T>  2Bh     A5h     [6:0]   8'h00     8'h11     OREG_TITP_STEPSIZE_SHIFT_PP1[2:0],1'b0,OREG_TITP_STEPSIZE_SHIFT_PP2[2:0]
     */
