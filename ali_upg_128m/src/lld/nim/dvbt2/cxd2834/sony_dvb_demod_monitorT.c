...
sony_result_t sony_dvb_demod_monitorT_SyncStat (sony_dvb_demod_t * pDemod, uint8_t * pSyncStat, uint8_t * pTSLockStat, uint8_t* pUnlockDetected)
{
    ...
    SONY_TRACE_ENTER ("sony_dvb_demod_monitorT_SyncStat");
...
/*--------------------------------------------------------------------
  ...
  sony_dvb_demod_t *pDemod      Instance of demod control struct
  int32_t          *pOffset     Carrier offset value(Hz)
--------------------------------------------------------------------*/
...
sony_result_t sony_dvb_demod_monitorT_SNR (sony_dvb_demod_t * pDemod, int32_t * pSNR)
{
    ...
    /*
     * SNR = 10 * log10 (REG / (13000 - REG)) + 32.2
     *     = 10 * (log10(REG) - log10(13000 - REG)) + 32.2
     * sony_log10 returns log10(x) * 100
     * Therefore SNR(dB) * 1000 :
     *     = 10 * 10 * (sony_log10(REG) - sony_log10(13000 - REG) + 32200
     */
...
sony_result_t sony_dvb_demod_monitorT_PER (sony_dvb_demod_t * pDemod, uint32_t * pPER)
{
    ...
    /*  slave    Bank    Addr    Bit     Name
     * ---------------------------------------------------------------------------------
     * <SLV-T>   00h     5Ch    [3:0]    OREG_PER_MES[3:0]
     * <SLV-T>   00h     5Dh      [0]    IREG_PER_VALID
     * <SLV-T>   00h     5Eh    [7:0]    IREG_PER_PKTERR[15:8]
     * <SLV-T>   00h     5Fh    [7:0]    IREG_PER_PKTERR[7:0]
     *
