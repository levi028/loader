...
sony_result_t sony_dvb_demod_monitorT2_CarrierOffset (sony_dvb_demod_t * pDemod, int32_t * pOffset)
{
    ...
    /* Read crl_ctlval 27:0 */
    {
        ...
        for ( retry=0; retry<30; ++retry )
        {
            ...
            if (syncState == 6) //the target.
            {
                if (retry>0)
                {
                    CXD2834_LOG(pDemod, "%s: Lock. retry %d times.\r\n", __FUNCTION__, retry);
                }
                break;
            }
        }
        ...
        /*
         * Carrier Offset [Hz] = -(IREG_CRCG_CTLVAL * (2^-30) * 8 * BW) / (7E-6)
         *
...
sony_result_t sony_dvb_demod_monitorT2_SNR (sony_dvb_demod_t * pDemod, int32_t * pSNR)
{
...

    {
        ...
        /*
         * SNR = 10 * log10 (REG / (24000 - REG)) + 34.8
         *     = 10 * (log10(REG) - log10(24000 - REG)) + 34.8
         * sony_log10 returns log10(x) * 100
         * Therefore SNR(dB) * 1000 :
         *     = 10 * 10 * (sony_log10(REG) - sony_log10(24000 - REG) + 34800
         */
...
sony_result_t sony_dvb_demod_monitorT2_TsRate (sony_dvb_demod_t * pDemod, uint32_t * pTsRateKbps)
{
    ...
    if (issy_on_data) {
        if ((ep_ck_deno == 0) || (ep_ck_nume == 0) || (ep_ck_deno >= ep_ck_nume)) {
            /* This case is never occured. */
            SONY_TRACE_RETURN (SONY_RESULT_ERROR_HW_STATE);
        }
    }

    /*
    If IREG_SP_ISSY_ON_DATA         TS Data output rate[Mbps]
    ----------------------------------------------------------------------------------------------------------------
    ...
    */
    {
        ...
        /* Multiply by 320 to retain accuracy in 32bit range.
           Result divided by 32 before returning */
        R *= 320U;
        Q = Q * 320 + R / div;
        R = R % div;
    ...
    if (issy_on_data) {
        /*  TSRate *= D/N
            TSRate *= 1 - (N-D)/N  (NOTE: D < N is always true) */
        uint32_t div = 0;
        uint32_t Q = 0;
        uint32_t R = 0;
        uint32_t tsRateComp = 0;

        /*TSRate * div/N  (div = N-D)
          ...
          TSRate * div/N
            = TSRate / (Q + R/div)
            = (TSRate * 32) / (Q*32 + (R*32)/div)
            = (TSRate * 32) / (Q*32 + ((R*32) + div/2)/div) (Add rounding) */
