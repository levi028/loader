...
sony_result_t sony_i2c_CommonReadRegister(sony_i2c_t* pI2c, uint8_t deviceAddress, uint8_t subAddress, uint8_t* pData, uint32_t size)
{
    ...
    sony_dvb_demod_t *pDemod = NULL;//(sony_dvb_demod_t *)pI2c->pDemod;
    ...
    pDemod = (sony_dvb_demod_t *)pI2c->pDemod;
...
//    }
    ...
    CXD2834_LOG_I2C(pDemod, result, 1, deviceAddress, &subAddress, 1);
    CXD2834_LOG_I2C(pDemod, result, 0, deviceAddress, pData, size);
...
sony_result_t sony_i2c_CommonWriteRegister(sony_i2c_t* pI2c, uint8_t deviceAddress, uint8_t subAddress, const uint8_t* pData, uint32_t size)
{
    ...
    {
        CXD2834_LOG(pDemod, "s%(): buffer[%d] is too small: pData[%d]\r\n", __FUNCTION__, BURST_WRITE_MAX, size);
        SONY_TRACE_I2C_RETURN(SONY_RESULT_ERROR_ARG);
    }
...
//    result = pI2c->Write(pI2c, deviceAddress, buffer, size+1, SONY_I2C_START_EN | SONY_I2C_STOP_EN);
    ...
    CXD2834_LOG_I2C(pDemod, result, 1, deviceAddress, buffer, size+1);
...
sony_result_t sony_i2c_TunerGateway(void* nim_dev_priv, UINT8    tuner_address , UINT8* wdata , int wlen , UINT8* rdata , int rlen)
{
    uint8_t buffer[BURST_WRITE_MAX + 2];//Protect against overflow: GW Sub Adress (1) + Tuner address (1)
    sony_dvb_demod_t* pDemod = (sony_dvb_demod_t*)nim_dev_priv;
    sony_result_t result = SONY_RESULT_OK;
    ...
    if( (0 == wlen) && (0 == rlen) )
    {
        CXD2834_LOG(pDemod, "s%(): No data for read/write.\r\n", __FUNCTION__);
        SONY_TRACE_I2C_RETURN(SONY_RESULT_ERROR_ARG);
    }

//    if( (0 != wlen) && (0 != rlen) )    //i2c_write_read
//    {
//        CXD2834_LOG(pDemod, "s%(): No data.\r\n", __FUNCTION__, BURST_WRITE_MAX);
//        SONY_TRACE_I2C_RETURN(SONY_RESULT_ERROR_ARG);
//    }
    ...
    if ( wlen != 0 ) //write to tuner
    {
        ...
        {
            CXD2834_LOG(pDemod, "s%(): buffer[%d] is too small: Write wdata[%d]\r\n", __FUNCTION__, BURST_WRITE_MAX, wlen);
            SONY_TRACE_I2C_RETURN(SONY_RESULT_ERROR_ARG);
        }
        ...
        if ( SUCCESS != i2c_write(pDemod->pI2c->i2c_type_id, pDemod->i2cAddress, buffer, wlen+2) )
            result = SONY_RESULT_ERROR_I2C;
        CXD2834_LOG_I2C(pDemod, result, 1, pDemod->i2cAddress, buffer, wlen+2);
    ...
    if ( rlen != 0 ) //read from tuner
    {
        ...
        {
            CXD2834_LOG(pDemod, "s%(): buffer[%d] is too small: Read rdata[%d]\r\n", __FUNCTION__, BURST_WRITE_MAX, rlen);
            SONY_TRACE_I2C_RETURN(SONY_RESULT_ERROR_ARG);
        }
...
//        hostI2C->Read (hostI2C, pI2c->gwAddress, pData, size, SONY_I2C_START_EN | SONY_I2C_STOP_EN);
        ...
        CXD2834_LOG_I2C(pDemod, result, 1, pDemod->i2cAddress, buffer, 2);
        if ( SUCCESS != i2c_write_read(pDemod->pI2c->i2c_type_id, pDemod->i2cAddress, buffer, 2, rlen) )
            result = SONY_RESULT_ERROR_I2C;
        CXD2834_LOG_I2C(pDemod, result, 0, pDemod->i2cAddress, buffer, rlen);
