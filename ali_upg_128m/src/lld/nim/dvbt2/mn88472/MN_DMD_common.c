...
DMD_u32_t    DMD_calc_SQI( DMD_PARAMETER_t* param)
{
    ...
    if( param->system == DMD_E_DVBT )
    {
            ...
            mod = param->info[DMD_E_INFO_DVBT_CONSTELLATION];
               ...
               /* 0 : DMD_E_DVBT_HIER_SEL_LP(default),    1: DMD_E_DVBT_HIER_SEL_HP
                ...
                D_device_set_info(param , DMD_E_INFO_DVBT_HIERARCHY_SELECT , DMD_u32_t val ) // val -> 1 :  HP ; 0 : LP
              */
    ...
    else
    {
        ...
        //if( beri < 1000 )        //BER>10-3  --> This is Nordig rule !
        //troy, 20130131, for DVB-T, video will not have mosaic until BER > 2* 10^(-3) ;
        //--> This is not Nordig rule, but it will make customer feel better ! Because when video moves, SQI should not be Zero.
        if( beri < 50 ) // 2* 10^(-2), mosaic happen, but video moves
        {
            ber_sqi = 0;
        }
...
DMD_ERROR_t DMD_trans_reg(DMD_PARAMETER_t* param, DMD_I2C_Register_t* t)
{
    int i=0;
    while(i<DMD_REGISTER_MAX)
    {
        ...
        if( DMD_I2C_Write(param,  slvadr , t[i].adr , t[i].data ) == DMD_E_ERROR ){
            DMD_I2C_Write(param,  slvadr , t[i].adr , t[i].data ); //for debug
            return DMD_E_ERROR;
        }
        i++;
...
/* Register  */
/* **************************************************** */
DMD_ERROR_t DMD_send_registers( DMD_PARAMETER_t* param, DMD_u8_t    *slaveset , DMD_u8_t* regset )
{
    DMD_u32_t    i;
    for(i=0;;)
    {
        if( regset[i] == 0xff ) break;
        DMD_I2C_Write( param, slaveset[regset[i]] , regset[i+1] , regset[i+2] );
        i=i+3;
    }
    return DMD_E_OK;
}
