...
UINT16 key_cnt = 0;
static UINT32 ms_tick = 0;
static struct pan_hw_info *local_hw_info = NULL;
...
void irc_m6303irc_init(struct pan_hw_info *hw_info)
{
    ...
    {
        osal_interrupt_register_lsr(20, irc_m6303irc_lsr, 0);
    }
    else
    {
...
void irc_m6303irc_close(void)
{
	...
	else if(sys_ic_get_chip_id() == ALI_C3702)
    {
        osal_interrupt_unregister_lsr(20, irc_m6303irc_lsr);
...
#else
    irc_lab_pulse_to_code((tick - irc_last_tick) * 1000);    /* The time in idle */
#endif
#endif
    irc_nec_pulse_to_code((tick - irc_last_tick) * 1000);    /* The time in idle */
            //        irc_RC6_pulse_to_code((tick - last_tick) * 1000);    /* The time in idle */
#else
    irc_pulse_to_code((tick - irc_last_tick) * 1000, 1);    /* The time in idle */
#endif
    irc_last_tick = tick;
...
#if (((SYS_SDRAM_SIZE != 2)  && (SYS_PROJECT_FE != PROJECT_FE_ATSC)) || (PORTING_ATSC == 1))
#if (SYS_CHIP_MODULE == ALI_S3602) && (SYS_PROJECT_FE == PROJECT_FE_DVBS)
		code = irc_nec_pulse_to_code(pulse_width);
#else
		code = irc_lab_pulse_to_code(pulse_width);
#endif
#else
		code = irc_nec_pulse_to_code(pulse_width);
#endif
...
#endif
        if (code != PAN_KEY_INVALID)
        {
            ms_tick = 0;
            last_width = tick - last_act_tick;
//            PRINTF("\t\t\tlast_width=%d\n", last_width);
            if(0 == key_cnt)        /* Receive a new key */
            {
                last_act_code = PAN_KEY_INVALID;
            }
            else if(1 == key_cnt)    /* Receive a continous key */
            {
                ir_repeat_width = local_hw_info->intv_repeat_first;
            }
            else
            ...
            if ((last_act_code != code) || (last_width > ir_repeat_width))
            {
                ...
                key.count = key_cnt++;
                key.code = code;
                if((pan_buff_get_repeat()) ||( 1 == key_cnt))
                {
...
static void irc_m6303irc_lsr(UINT32 param)
{
    ...
    param = param;
    while ((status=((READ_INF_BYTE(INFRA_ISR)) & 0x3)))
    {
        ...
        switch (status)
        {
            ...
            case 0x01:        /* If FIFO trigger, copy data to buffer */
                do
                {
                    num = READ_INF_BYTE(INFRA_FIFOCTRL) & 0x7F;
                    num1 = num;
                    while (num > 0)
                    {
                        /* Put RLC to buffer */
