...
#if ((defined PMU_MCU_M3702) || (defined PMU_MCU_M3922))
#define SYS_I2C_SDA                                                    XPMU_GPIO_1
#define SYS_I2C_SCL                                                    XPMU_GPIO_0
#else/*M3921 project.*/
...
static void show_off(void)
{
	...
	UINT8 led_map[4] = {0x00, 0x3F, 0x71, 0x1};
#else
	UINT8 led_map[4] = {0x00, 0x5F, 0x65, 0x1};
#endif
...
{
	INT8 ret = SUCCESS;
	UINT8 led_map[4] = {0x0, 0x0, 0x0, 0x1};
...
static void show_time(pRTC_TIMER rtc)
{
	...
	UINT8 show_colon = 0;
#ifndef FD650_PAN_SUPPORT
	UINT8 led_map[11] = {0x3F, 0x06, 0x5B, 0x4F, 0x66, 0x6D, 0x7D, 0x07, 0x7F, 0x6F, 0x1};/*ASCII of data 0~9.*/
#else
	UINT8 led_map[11] = {0x5F, 0x12, 0x2F, 0x3B, 0x72, 0x79, 0x7D, 0x53, 0x7F, 0x7B, 0x1};/*ASCII of data 0~9.*/
#endif
...
{
	INT8 ret = SUCCESS;
	UINT8 led_map[5] = {0x00, 0x3F, 0x37, 0x0, 0x1};

	ret |= i2c_gpio_write(SETING_ADDR, &led_map[4], 1);/*Enable CH455 Panel.*/
	ret |= i2c_gpio_write(DIG0_ADDR, &led_map[0], 1);
	ret |= i2c_gpio_write(DIG1_ADDR, &led_map[1], 1);
	ret |= i2c_gpio_write(DIG2_ADDR, &led_map[2], 1);
	ret |= i2c_gpio_write(DIG3_ADDR, &led_map[3], 1);
}
...
unsigned char pannel_scan(void)
{
	...
	{
		if(i2c_gpio_read(KEY_ADDR, &Detected_Panel_Key1, 1) != SUCCESS) return ERROR;
		if((Detected_Panel_Key1 != Received_Panel_Key1) && (Detected_Panel_Key1 != Received_Panel_Key2))
		{
	...
	{
		if(i2c_gpio_read(KEY_ADDR, &Detected_Panel_Key2, 1) != SUCCESS) return ERROR;

	#if 0/*Add only for debug.*/
		if((0x0 != Detected_Panel_Key2) && (0xFF != Detected_Panel_Key2))
		{
			PMU_WRITE_BYTE(0x3f00, Detected_Panel_Key1);
			PMU_WRITE_BYTE(0x3f01, Detected_Panel_Key1);
			PMU_WRITE_BYTE(0x3f02, Detected_Panel_Key1);
			PMU_WRITE_BYTE(0x3f03, Detected_Panel_Key1);
			PMU_WRITE_BYTE(0x3f04, Detected_Panel_Key2);
			PMU_WRITE_BYTE(0x3f05, Detected_Panel_Key2);
			PMU_WRITE_BYTE(0x3f06, Detected_Panel_Key2);
			PMU_WRITE_BYTE(0x3f07, Detected_Panel_Key2);
		}
	#endif
