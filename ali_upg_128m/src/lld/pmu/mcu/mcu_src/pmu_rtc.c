...
void pmu_timer1_set_time(unsigned char autoload)
{
	...
	if(autoload)
	{
		...
		PMU_WRITE_BYTE(PMU_RTC1_CTRL, PMU_READ_BYTE(PMU_RTC1_CTRL) | AUTO_LOAD_EN);/*AUTO LOAD EN.*/
	}
...
static void pmu_timer1_int_en(UINT8 en)
{
	if(en)
	{
		PMU_WRITE_BYTE(PMU_RTC1_CTRL, PMU_READ_BYTE(PMU_RTC1_CTRL) | TIMER1_INT_ENABLE);
	}
	else
	{
		PMU_WRITE_BYTE(PMU_RTC1_CTRL, PMU_READ_BYTE(PMU_RTC1_CTRL) & TIMER1_INT_DISABLE);
	}
}
		...
		if(rtc->min>59)
		{
			...
			if(rtc->hour>23)
			{
				...
				if((rtc->month == 1) || (rtc->month == 3) || (rtc->month == 5) \
					|| (rtc->month == 7) || (rtc->month == 8) || (rtc->month == 10) \
					|| (rtc->month == 12))
				{
					if(rtc->day>31)
					{
